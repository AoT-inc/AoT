...
-   **モバイルで全幅** — ウィジェットの設定ドロワーの基本設定にあるトグルです。オンにするとスマートフォンの画面(2列配置)でそのウィジェットが1行をまるごと使い、オフにすると1行に2つずつ並びます。マップ・カレンダー・シーケンスのように狭い幅では読みにくいウィジェットはオンの状態で追加され、その他はオフの状態で追加されます。デスクトップの配置には影響しません。
...
*特徴:* サーバー側のバックグラウンドワーカーが実行/休止サイクルを進めるため、ダッシュボードのタブを閉じても止まりません。アプリが再起動されても（アップデートなど）、タイマーはその時点であるべき位置から続きを実行します。予約開始を待っていたタイマーはその予約を保ち、実行中または休止中だったサイクルも続行されます。再起動の間に終わっているはずだった区間は飛ばされ、その間にすべて終わっていた場合は完了として表示されます。

### AoT グラフ { #widget-graph }

複数の測定値を1つの時系列グラフ(Apache EChartsベース)に重ねて表示し、それらの関係を確認できます。たとえば、温室の1棟の気温・湿度(入力の測定値)を、そのVPD(機能の測定値)や灌水バルブの稼働時間(出力の測定値)と一緒にプロットすれば、気温の急上昇が灌水サイクルと重なっているかどうかを一目で確認できます。
