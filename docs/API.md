...
알림을 위한 자격 증명으로 이메일을 보냅니다.

매개변수:

-  **recipients** - 이메일을 보낼 이메일 주소(문자열) 또는 주소 목록(문자열 리스트)입니다.
-  **message** - 이메일 본문입니다.
-  **subject** - 이메일 제목입니다.
...
함수 액션을 실행하라는 지시를 내립니다.

매개변수:

-  **action_id** - 함수 액션의 고유 ID입니다.
-  **value** - 최소한 'message' 키가 포함된 딕셔너리로, 액션에 메시지를 추가하는 데 사용됩니다. 이 딕셔너리는 액션에 의해 반환되어야 합니다.
-  **debug** - 디버그 로깅 메시지를 표시할지 여부입니다.

### trigger_all_actions()

**trigger_all_actions**\ (*function_id*, *message=''*, *debug=False*)

함수 컨트롤러의 모든 함수 액션을 순차적으로 실행하라는 지시를 내립니다.

매개변수:

-  **function_id** - 컨트롤러의 고유 ID입니다.
-  **message** - 액션과 함께 보낼 메시지로, 액션에서 사용할 수 있습니다.
-  **debug** - 디버그 로깅 메시지를 표시할지 여부입니다.
