...
- **Google → AoT (pull)**: 이미 동기화된 이벤트의 수정·시간 변경·취소는 해당 AoT 작업에 반영되며, 사용자/장치 캘린더의 새 이벤트는 위 표대로 새 작업을 만듭니다. 이 방향은 AoT 작업을 바꾸므로 연결한 AoT 계정이 편집자 역할(컨트롤러 편집 권한)일 때만 실행되고, 그 밖의 역할에서는 AoT → Google 만 실행됩니다.
