...
**온도축을 되돌릴 때 고정하는 양은 RH 가 아니라 수증기압 ea** (2026-08-20 확정):

`VPD = SVP(T)×(1−RH/100)` 은 정의이지만, "온도를 얼마나 올리면 목표 VPD 가
되는가" 를 풀 때 그 식을 **RH 를 상수로 두고** 미분하면 안 된다. 밀폐 공간을
가열해도 물이 늘지 않으므로 보존되는 것은 ea 이고 RH 는 따라 내려간다.

```
ea = RH/100 · SVP(T_int)                  (고정)
SVP(T_needed) = vpd_target + ea           → Magnus 역함수로 닫힌 해
```

RH 를 고정으로 두면 `(1−RH/100)` 인자가 남아 **RH=100% 에서 온도가 VPD 에
아무 영향이 없다**는 결론이 나온다 — 포화 상태(VPD=0)에서 난방 요구가 0 이
되어 가열이 영영 일어나지 않는다. RH=99.9% 에서는 반대로 필요 온도가 상한
(50°C)까지 튄다. 같은 인자가 `effect_functions.make_vpd_effect` 의
`∂VPD/∂T` 에도 있었고 난방기의 VPD 효과를 0 으로 만들었다.

회귀: `tests/test_vpd_saturation_regression.py`.

**그런데 분무에는 `(1−RH/100)` 이 필요하다** — 위와 형태만 같고 성격이 다르다:

| 자리 | `(1−RH/100)` | 이유 |
|------|--------------|------|
| `_invert_svp_for_T` · `make_vpd_effect` 의 `∂VPD/∂T` | **없어야 함** | "온도를 바꾸는 동안 RH 고정" 은 물을 넣지 않는 한 성립하지 않는 가정 |
| `fogger_*_effect` 의 증발 가용도 | **있어야 함** | 증발을 미는 힘이 곧 VPD — 포화 공기는 수증기를 더 받지 못한다 |

분무한 물이 실제로 증발해야 잠열을 가져가고(냉각) 수증기가 된다(가습). 포화
공기에서는 둘 다 일어나지 않고 표면만 젖는다. 그런데 모델은 RH 와 무관하게
같은 ΔT·ΔRH 를 신고해, RH 100% 인 육묘장에서 분무가 냉각 효과를 주장했다.

```
가용도 = clamp( (1−RH/100) / (1−RH_ref/100), 0, 1 )      RH_ref = 70%
```

`RH_ref` 이하에서 1.0 에 거는 이유: `K_FOG_*` 는 이미 보수적인 실사용 계수라,
건조할 때 그 값을 넘겨 키우면 근거가 없다. 가용도 0 이면 `EffectResult('0', 0)`
을 돌려 제어 축에서 아예 빠진다. **한쪽을 고쳤다고 다른 쪽까지 지우지 말 것.**

**증발 유량은 관수 유량이 아니다** (같은 날 함께 고침):

`irrigation_flow_lpm` 하나를 성격이 다른 둘이 나눠 쓰고 있었다 —
투여량 환산(`VolumetricAdapter`, 드립 포함 총유량)과 증발 효과(공기 중으로
나가는 유량). 게다가 액추에이터 값이 없으면 **시설 합계로 폴백**했다. 노즐
정보가 없는 분무기가 시설 전체 관수 216 L/min(드립 에미터 324개)을 물려받아
9448 m³ 온실에서 증발냉각 **45.6 °C/사이클** 이 나왔고, 결합 drive 에서 그 한
항의 가중치가 나머지의 20배가 되어 다른 축을 전부 무의미하게 만들었다.

지금은 키가 갈라져 있다. `fog_flow_lpm` 은 그 액추에이터 자기 노즐 중 드립이
아닌 것들의 합(`summarize_nozzles.sprinkler_flow_lph`/60)이고 **폴백하지
않는다.** 모르면 키를 넣지 않고, 효과 쪽이 물리 계산을 포기하고 보수적
`K_FOG_*` 로 떨어진다 — 모르는 값을 참조값으로 메우면 그것이 실측처럼
흘러다닌다.
...
### 3.1a 적분의 레일 고착과 회복 경로 (2026-08-20 확정)

L3 의 적분은 정의상 **평형 개도 기억**이다 — 그 액추에이터가 평형에서 서 있어야
할 자리(%). 그런데 포화 중에는 그 뜻이 깨진다. back-calculation 의 부호 때문이다.

| 레일 | back-calculation 동작 | 남는 값 |
|------|----------------------|---------|
| 아래(u<0) | `I -= (u−c)·β` 가 I 를 **위로** 민다 | 요구값 −P 가 100 을 넘으면 상한에 굳음 |
| 위(u>100) | 같은 식이 I 를 **아래로** 깎는다 | 0 근처까지 내려감 |

둘 다 dispatch 와 **정반대** 값이 남는다. 그리고 편차가 풀리는 순간
`cmd = P + I` 의 I 가 통째로 계단으로 튀어나온다. 두 번 났다:

- **2026-07-29 aot-005** — 폭염 중 dispatch 100% 인데 I 는 0 근처로 깎여 있었고,
  무구배로 전환되는 순간 보온커튼이 급락(오폐쇄)했다.
- **2026-08-20 로컬 육묘장** — 냉방 I 가 100 에 굳은 채 버텼다. 그때 명령을
  붙잡고 있던 것은 I 가 아니라 P 였지만, 계단은 그대로 장전돼 있었다.

**회복 경로**: 직전 dispatch 가 이미 그 레일이면(= 한 사이클 이상 눌러붙어
있었다) 적분을 **실제 개도 쪽으로** `RELAX_FACTOR` 기하 감쇠시킨다. 포화 직후
사이클은 위쪽 레일이면 back-calculation 에 맡겨 빠른 anti-windup 을 유지하고,
아래쪽 레일이면 **적분을 동결**한다(아래).

```
if cmd_unclamped != cmd_raw:
    if abs(prev_val - cmd_raw) <= RAIL_EPS:   # 레일에 눌러붙어 있음
        I = cmd_raw + (I - cmd_raw) * RELAX_FACTOR
    elif cmd_unclamped < 0:                    # 아래쪽 갓 포화 — 동결
        I = I_entry                            # 이번 누적·슬루 되먹임 모두 버림
    else:                                      # 위쪽 갓 포화
        I = clamp(I - (cmd_unclamped - cmd_raw) * AW_BETA, 0, 100)
```

**아래쪽 레일 갓 포화 = 동결 (2026-09-19)**: 아래쪽 back-calculation 은 I 를
`−P` 쪽으로 **올린다**. P 가 크면 그대로 드러난다 — 로컬 섀도 실행(하드 온도
상한, 실외가 더 더워 개구부를 닫는 중)에서 개도 0 %·10 % 인 개구부 둘의 적분이
한 사이클에 0→100, 3.53→100 이 됐다. 현실적인 초과(하드 32 °C 를 4 °C 넘음,
P ≈ −54 %)에서도 0→27 이었다. 풀린 뒤 '조금 더 닫아라' 면 방향 전환 되앉힘이
돌지 않아 `cmd = P + I` 가 창을 **열었다**.

세 규칙을 합성 폐루프(1차 지연, ki 0.2 %/사이클)로 비교해 동결을 골랐다.

| 경우 | 종전 | 동결 | I ≤ 실제 개도 |
|------|-----:|-----:|-----:|
| 개도 60 · 추움 · 필요 20 % (IAE) | 295 | 187 | 110 |
| 평형 60 에서 짧고 강한 냉각 외란 (IAE) | 96 | 105 | 284 |
| 위쪽 레일 사례 9종 | — | 종전과 동일 | — |

'I ≤ 실제 개도' 는 평형 기억까지 지워 외란 뒤 100 사이클 넘게 2.6 °C 벗어난
채 고착됐다. 위쪽 레일까지 동결하면 짧은 고온 스파이크 뒤 회복이 나빠져
(IAE 108 → 138) 위쪽은 그대로 두었다.

**새 상태를 만들지 않는다.** "몇 사이클째 포화인가" 를 세려면 액추에이터별
카운터를 영속시켜야 하는데, `prev_val` 이 이미 그 정보를 갖고 있다 — slew 가
걸린 액추에이터는 레일에 닿기까지 여러 사이클이 걸리므로, `prev_val` 이 레일에
있다는 것 자체가 "최소 한 사이클 이상 여기 있었다" 는 뜻이다. 짧은 포화는
감쇠를 아예 트리거하지 않고, 트리거되더라도 기하 감쇠라 몇 사이클로는 조금만
움직인다.

효과(실측, 편차 −10°C 로 아래쪽 레일에 고착 후 편차 +1.5°C 로 반전):

```
회복 경로 있음   I → 0.2    이후 명령 16.7%   (비례항이 요구하는 값 = 16.7%)
회복 경로 없음   I → 90.4   이후 명령 20 → 40 → 60 → 80 → 100%
```

회귀: `tests/test_coordinator_rail_recovery.py`, `tests/test_ceiling_rail_windup.py`.
`RELAX_FACTOR` 는 무구배·파킹 분기와 공유한다 — 둘 다 "알려진 자리로 수렴" 이라
같은 비율을 쓰는 것이 맞다.

**덮지 못하는 경우 — 범위 안에 앉은 낡은 적분** (실측으로 확인):

적분이 100 인데 P 가 −17 이면 명령은 82.6% 로 **포화되지 않는다.** 회복 경로는
발동하지 않고, 낡은 적분이 액추에이터를 82.6% 로 끌어올린 채 `ki·e_eff`(사이클당
0.03%p)로만 풀린다. 로컬에서 그 상태를 일부러 만들어 재현했다.

이건 고칠 수 없는 것이 아니라 **고치면 안 되는 것**이다. 포화라는 증거가 없으면
"낡은 값" 과 "학습된 평형 개도" 를 구분할 방법이 없고, 후자를 임의로 감쇠시키면
적분이 존재하는 이유(정상상태 droop 제거)가 사라진다. 회복 경로가 레일 조건으로
좁게 스코프된 이유가 이것이다 — 레일에 눌러붙은 값만이 **뜻이 없다고 증명된**
값이다.

그래서 제어 법칙을 고치는 배포에서는 **해당 코디네이터의 PI 상태를 한 번
비우는 것**이 여전히 필요하다. 회복 경로는 운전 중에 생기는 고착을 막을 뿐,
이전 regime 이 남긴 값을 소급해서 정리하지는 않는다.
...
- 작성 언어: **한국어 + 영어 병기**
- 위치: `docs/ai_guide.ko.md`, `docs/ai_guide.md`
