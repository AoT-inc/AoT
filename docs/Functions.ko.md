...
!!! 참고
    "마지막"은 기능이 데이터베이스에서 마지막(최신) 측정값만 가져온다는 것을 의미합니다. "과거"는 기능이 설정된 "최대 수명(초)"까지 현재부터 모든 측정값을 가져온다는 것을 의미합니다(예: 측정값이 10초마다 수집되고 최대 수명이 60초로 설정된 경우, 평균적으로 기능이 작동하는 데 6개의 측정값이 반환됩니다).

### 목록에서 함수 찾기

페이지 상단의 드롭다운은 함수를 제어·자동화, 트리거, 사용자 함수로 묶어 보여줍니다. 검색창은 표시 이름과 모듈 이름 양쪽으로 걸립니다.

## 사용자 정의 기능 { #custom-functions }

AoT에는 사용자 생성 기능을 AoT 시스템에서 사용할 수 있도록 하는 사용자 정의 기능 가져오기 시스템이 있습니다. 사용자 정의 기능은 `[톱니바퀴 아이콘] -> 구성 -> 사용자 정의 기능` 페이지에서 업로드할 수 있습니다. 가져온 후에는 `설정 -> 함수` 페이지에서 사용할 수 있습니다.
...
적절한 포맷팅 예제를 보려면 디렉토리 [AoT/aot/functions](https://github.com/AoT-inc/AoT/tree/main/aot/functions/)에 있는 기본 제공 모듈을 열어보세요.

또한, 디렉토리 [AoT/aot/functions/examples](https://github.com/AoT-inc/AoT/tree/main/aot/functions/examples)에는 사용자 정의 기능 예제가 포함되어 있습니다.
...
<tr>
<td>측정값 (단일, 최신)</td>
<td>입력 또는 장치에서 최신 측정값을 가져옵니다. 최대 수명(초)을 설정하여 값을 허용할 시간을 제한할 수 있습니다. 최신 값이 이 기간보다 오래된 경우, "None"이 반환됩니다.</td>
</tr>
<tr>
<td>측정값 (단일, 과거, 평균)</td>
<td>입력 또는 장치에서 과거 측정값을 가져온 후 평균을 계산합니다. 최대 수명(초)을 설정하여 값을 허용할 시간을 제한할 수 있습니다. 모든 값이 이 기간보다 오래된 경우, "None"이 반환됩니다.</td>
</tr>
<tr>
<td>측정값 (단일, 과거, 합계)</td>
<td>입력 또는 장치에서 과거 측정값을 가져온 후 합계를 계산합니다. 최대 수명(초)을 설정하여 값을 허용할 시간을 제한할 수 있습니다. 모든 값이 이 기간보다 오래된 경우, "None"이 반환됩니다.</td>
</tr>
<tr>
<td>측정값 (다중, 과거)</td>
<td>입력 또는 장치에서 과거 측정값을 가져옵니다. 최대 수명(초)을 설정하여 값을 허용할 시간을 제한할 수 있습니다. 이 기간 내에 값을 찾을 수 없는 경우, "None"이 반환됩니다. 이는 "측정값 (단일)" 조건과 다르게 'time' 및 'value' 키 쌍을 포함한 딕셔너리 목록을 반환합니다.</td>
</tr>
<tr>
<td>GPIO 상태</td>
...
<tr>
<td>태양: 주간/야간</td>
<td>해당 위치가 지금 주간이면 True, 야간이면 False를 반환합니다. 위치는 이 조건부 기능이 지도에서 갖는 위치(없으면 소속 구역·사이트)를 상속하므로 좌표를 따로 설정하지 않습니다. 일출·일몰 오프셋(분)으로 주간 구간의 양 끝을 밀 수 있습니다 — 예를 들어 일몰 오프셋 -30은 일몰 30분 전부터 야간으로 봅니다. 극지에서 백야면 항상 True, 극야면 항상 False입니다.</td>
</tr>
<tr>
<td>태양: 이벤트까지 남은 시간</td>
<td>선택한 다음 태양 이벤트(일출·일몰·남중·시민박명 시작/끝)까지 남은 시간을 초 단위로 반환합니다. 이벤트 오프셋(분)을 음수로 주면 이벤트 이전 시점까지의 시간을 셉니다 — 예를 들어 -60은 일몰 1시간 전까지 남은 초입니다. 위치는 조건부 기능에서 상속합니다. 해당 날짜에 그 이벤트가 없으면(극지) "None"이 반환됩니다.</td>
</tr>
<tr>
<td>최대 수명 (초)</td>
<td>측정값이 가질 수 있는 최대 수명(초)입니다. 마지막 측정값이 이보다 오래된 경우, 측정값 대신 "None"이 반환됩니다.</td>
</tr>
</tbody>
</table>
...
다음은 조건부 함수에서 사용할 수 있는 몇 가지 예제입니다. 각 `self.condition("ID")`은 해당 조건의 최신 측정값을 반환하며, 설정된 최대 수명(Max Age) 내에 있는 경우에만 값을 반환합니다.
...
if measurement is not None:
    ...
    self.stored_measurement = measurement  # 측정값 저장
```
"측정값 (다중)" 조건은 설정된 최대 수명(Max Age) 내에서 마지막 측정값뿐만 아니라 과거의 특정 값이 저장되었는지 확인하려는 경우 유용합니다. 이는 각 숫자 값이 확인해야 할 서로 다른 경고를 나타내는 경고 시스템이 있는 경우, 과거 값 중 특정 값이 발생했는지 확인하는 데 사용할 수 있습니다. 아래는 지난 30분(최대 수명: 1800초) 동안의 모든 측정값을 가져와 반환된 목록에서 "119"와 같은 값이 있는지 확인하는 예제입니다. "119"가 존재하면 작업이 실행되고 `break`를 사용하여 `for` 루프를 종료합니다.

```python
# 예제 1: 지난 30분 동안의 측정값에서 특정 값 찾기 (최대 수명: 1800초)
...
!!! 참고
    "활성화 시 트리거"는 활성화나 설정 재로드 시점이 아니라 트리거의 **다음
    점검**에서 발화합니다 — 최대 한 표본 주기(데몬의 일반 트리거·조건부 표본
    주기) 만큼 늦어질 수 있습니다. 그 자리에서 곧바로(동기적으로) 발화시키면
    설정을 다시 읽는 중에 드물게 교착이 걸려, 트리거가 그대로 멈춰 다시는
    발화하지 않을 위험이 있었습니다.

    선택한 지속 시간 메서드가 아직 값을 내지 못하는 상태라면(예: 활성화 직후
    시작 시각이 아직 기록되지 않은 경우) 잘못된 명령을 보내는 대신 PWM 출력을
    건드리지 않고 다음 주기를 기다립니다. 계산 자체는 됐는데 출력이 명령을
    거부한 경우(무응답·시간 초과)는 이제 오류로 남습니다 — 설정점 곡선은 출력에
    닿았는지와 무관하게 계속 진행됩니다.
...
**시퀀스**는 여러 개의 출력 장치(밸브·펌프·조명 등)를 정해진 순서와 시간표에 따라 자동으로 켜고 끄는 기능입니다. "밸브 A를 30분 열고, 그다음 밸브 B를 30분 열고…" 처럼 여러 단계로 이어지는 작업을 사람이 일일이 조작하지 않고 반복 실행하도록 자동화할 때 사용합니다. 관수 순환, 다단계 환기(창문 열기 → 대기 → 팬 가동) 등에 특히 유용합니다.

### 예시로 알아보기: 급수 펌프 + 밸브 3개 순차 관수

가장 흔한 사용 사례로 살펴보겠습니다. 메인 급수 펌프 1대와, 구역별로 순서대로 물을 주는 밸브 A·B·C가 있다고 가정합니다. 원하는 동작은 다음과 같습니다.

- **펌프**는 관수가 진행되는 동안 처음부터 끝까지 계속 켜져 있어야 합니다.
- **밸브 A → B → C** 순서로 각각 30분씩 급수한 뒤 자동으로 꺼집니다.

설정 방법:

1. 함수 목록에서 **시퀀스**를 새로 추가합니다.
2. **액션 추가**에서 "출력: 켜기/끄기/지속시간"을 선택하고 메인 펌프의 출력을 골라 추가합니다. 모드는 **전체(Total)**로 지정합니다 — 이 모드는 "시퀀스가 도는 내내 계속 켜져 있어야 하는 장치"에 씁니다.
3. 밸브 A, B, C를 이 순서로 추가합니다. 각각 모드는 **단일(Single)**, 지속시간은 1800초(30분)로 지정합니다.
4. 저장하면 등록한 순서(위젯에서는 드래그로 재배열 가능)대로 실행됩니다.

시간이 흐르는 모습을 표로 보면 다음과 같습니다.

| 경과 시간 | 0~30분 | 30~60분 | 60~90분 |
| :--- | :---: | :---: | :---: |
| 펌프 (전체) | ON | ON | ON |
| 밸브 A (단일) | ON → OFF | | |
| 밸브 B (단일) | | ON → OFF | |
...
**전체(Total)** 모드는 이렇게 "다른 단계가 전부 끝날 때까지 켜져 있어야 하는" 장치 한 개(보통 메인 펌프)에 사용하고, 나머지 단계들은 **단일(Single)** 모드로 순서대로 실행되는 것이 기본 패턴입니다.

기본값 그대로 두면 펌프는 밸브 A와 **같은 순간**에 켜지고 밸브 C와 같은 순간에 꺼집니다. 닫힌 밸브를 상대로 펌프가 먼저 돌거나, 펌프가 도는 채로 밸브가 닫히면 배관에 무리가 가므로, 펌프 단계에 **여유 시간(시작 지연 / 조기 정지)** 을 주어 펌프 구간을 밸브 구간 안쪽으로 넣는 것을 권장합니다. 아래 [여유 시간](#total-margins)을 참고하세요.

### 예시로 알아보기: 밸브 여러 개를 동시에 열기

위 예시에서 밸브 B와 C를 순서대로가 아니라 **동시에** 열고 싶다면 어떻게 할까요? 이럴 때는 두 단계를 **장치 그룹**으로 묶습니다. 위젯이나 통합 모달에서 밸브 B·C의 그룹 칸에 같은 이름(예: `구역2`)을 입력하면, 두 밸브가 하나의 슬롯으로 묶여 같은 시간에 함께 켜지고 꺼집니다.

| 경과 시간 | 0~30분 | 30~60분 |
| :--- | :---: | :---: |
| 펌프 (전체) | ON | ON |
| 밸브 A (단일) | ON → OFF | |
| 밸브 B + C (그룹 `구역2`) | | ON → OFF (동시) |

이렇게 순차 실행(펌프+단일 단계)과 동시 실행(장치 그룹)을 섞어서 쓸 수 있습니다. 그룹으로 묶인 밸브들은 같은 지속시간을 공유하며, 자세한 규칙은 아래 [장치 그룹](#device-groups)을 참고하세요.

### 실행 모드: 단일 vs 전체

- **단일 (Single, 기본값)**: 각 단계마다 독립적인 실행 시간을 적용합니다.
    - **계산식**: `총 활성 시간 = Head Overlap + Base Duration + Tail Overlap`
    - **동작**: 이전 단계가 끝나기 `Overlap` 초 전에 다음 단계가 시작되어 부드러운 전환을 지원합니다(예: 밸브를 닫기 몇 초 전에 다음 밸브를 미리 열어 배관 압력이 갑자기 떨어지지 않게 함).
- **전체 (Total, Full-span)**: 위 펌프 예시처럼, 전체 시퀀스가 실행되는 동안 계속 켜져 있어야 하는 단계에 사용합니다. 시퀀스의 시작부터 마지막 `단일` 단계가 종료될 때까지 유지되며, **여유 시간**으로 그 구간을 양쪽에서 좁힐 수 있습니다(아래 참조).

### 여유 시간: 펌프를 밸브 안쪽에 두기 { #total-margins }

**전체(Total)** 모드 단계는 기본적으로 사이클의 0초에 켜져 마지막 단계와 동시에 꺼집니다. 즉 첫 밸브와 펌프가 같은 순간에 켜지고, 마지막 밸브와 펌프가 같은 순간에 꺼집니다. 실제 관수에서는 이 순서가 중요합니다.

- 밸브가 아직 열리지 않았는데 펌프가 돌면 펌프가 압력을 그대로 받습니다.
- 펌프가 도는 상태에서 밸브가 닫히면 수격(water hammer)이 발생합니다.

이를 위해 전체 모드 단계에 두 값을 줄 수 있습니다. 단위는 초이고, 기본값은 둘 다 0(기존 동작 그대로)입니다.

| 값 | 하는 일 |
| :--- | :--- |
| **시작 지연** (`total_lead`) | 시퀀스가 시작되고 이 초만큼 지난 뒤에 켭니다. 그 사이 밸브가 먼저 열립니다. |
| **조기 정지** (`total_lag`) | 시퀀스가 끝나기 이 초 전에 끕니다. 밸브가 닫히기 전에 압력이 빠집니다. |

예를 들어 시작 지연 10초, 조기 정지 15초를 주면 90분 사이클에서 펌프는 다음과 같이 동작합니다.

| | 시작 | 종료 |
| :--- | :--- | :--- |
| 밸브 (단일 단계 전체) | 0초 | 5400초 |
| 펌프 (전체) | 10초 | 5385초 |

펌프 구간이 밸브 구간 안쪽에 완전히 들어가므로, 항상 "밸브 열림 → 펌프 켜짐 … 펌프 꺼짐 → 밸브 닫힘" 순서가 됩니다. 정지할 때도 전체 모드 단계를 먼저 끄고 나머지를 끕니다.

설정 위치는 두 곳입니다. 어느 쪽이든 **전체(Total)** 모드를 고른 단계에서만 보입니다.

- 함수 설정 화면의 단계 행: 모드를 `전체`로 바꾸면 `시작 지연`·`조기 정지` 칸이 나타납니다.
- 대시보드 시퀀스 위젯: 단계 이름을 눌러 열리는 설정 창의 **여유 시간 (초)**.

여유 시간이 사이클 길이보다 커서 펌프가 아예 못 도는 설정이 되면, 그 값은 무시하고 전체 구간을 그대로 사용하며 로그에 경고를 남깁니다.

### 그 밖의 주요 개념

- **동적 기간 (Dynamic Duration)**: `action_duration_id` 옵션을 통해 특정 입력(Input)의 측정값을 실행 시간으로 사용할 수 있습니다.
    - 형식: `Input_UUID` 또는 `Input_UUID,Measurement_UUID`.
    - 유효성: `time_offset_minutes`(UI에는 **입력 유효성 (초)**로 표시) 내의 최신 측정값만 사용하며, 없을 경우 설정된 기본 `action_duration`을 사용합니다.
- **오버랩 (Overlaps)**: `output_duration` 설정값을 사용하여 단계 사이의 전환 시간을 결정합니다. 첫 번째 액션은 `Tail Overlap`만, 중간 액션은 `Head & Tail Overlap`을, 마지막 액션은 `Head Overlap`만 가집니다.
...
| `time_offset_minutes` | 동적 기간 측정값의 최대 유효 시간 (필드명과 달리 초 단위 — UI에는 **입력 유효성 (초)**로 표시). |
| `enabled` | 개별 액션의 활성화 여부. |
| `sequence_mode` | 'single' 또는 'total' 선택. |
| `total_lead` | (전체 모드 전용) 시퀀스 시작 후 이 초만큼 지나서 켭니다. 기본 0. |
| `total_lag` | (전체 모드 전용) 시퀀스 종료 이 초 전에 끕니다. 기본 0. |
| `action_duration` | 해당 단계의 기본 실행 시간 (초 단위). |
| `action_duration_id` | 동적 실행 시간을 가져올 장치/측정값 ID. |
| `group_name` | 장치 그룹 이름. 같은 이름을 가진 단계들은 하나의 슬롯으로 묶여 동시에 작동합니다(아래 참조). 비어 있으면 단독 작동합니다. |
| `display_name` | 위젯 목록에 표시할 사용자 지정 라벨. 비우면 장치 이름으로 대체됩니다. |

### 장치 그룹 (동시 작동) { #device-groups }

앞의 예시에서 본 것처럼, 여러 단계를 **하나의 장치 그룹**으로 묶으면 순차 실행이 아니라 **같은 시간 창에서 동시에** 켜지고 꺼집니다. 상세 규칙은 다음과 같습니다.

- **묶는 방법**: 위젯 또는 통합 모달에서 단계 이름을 클릭 → 그룹 칸에 같은 그룹 이름을 입력합니다. 같은 이름을 가진 단계들이 하나의 슬롯으로 접혀 표시됩니다. 그룹 칸을 비우면 해당 단계가 그룹에서 빠져 단독 작동으로 돌아갑니다.
- **공통 기간 (리더 상속)**: 그룹은 하나의 공통 작동 시간을 공유합니다. 어떤 단계가 기존 그룹에 합류하면 그 그룹의 공통 `action_duration`(및 동적 기간 리더 `action_duration_id`)을 **자동으로 상속**합니다. 슬롯의 대표(가장 앞선 위치의 멤버)가 그룹 전체의 기간·동적 리더를 결정합니다.
- **실행 순서**: 슬롯 순서는 각 그룹이 처음 등장한 위치를 따릅니다. 이미 존재하는 그룹의 뒤쪽 멤버는 앞선 슬롯으로 folding(접힘)되어 새 슬롯을 만들지 않습니다.
- **제약**: `total`(Full-span) 모드 단계는 그룹으로 묶을 수 없습니다. 그룹은 본질적으로 `single`(동시-단일) 모드입니다.

### 요일별 스케줄 (Weekly Schedule)

기존의 단일 시작·종료 시간 대신, **요일마다 다른 작동 시간·주기**를 설정할 수 있습니다. 스케줄은 `Trigger.timer_schedule`에 JSON으로 저장되며 두 가지 모드를 지원합니다.

- **`shared` (공유 모드)**: 모든 요일이 하나의 시작·종료·주기를 공유합니다. 활성 요일만 선택합니다(기존 방식과 동일).
- **`per_day` (요일별 모드)**: 월~일 각 요일이 독립적인 시작(`start`)·종료(`end`)·주기(`period`)와 활성 여부(`enabled`)를 가집니다. 나아가 요일별로 다음을 개별 오버라이드할 수 있습니다.
    - **`actions`**: 요일별 단계 활성/비활성 (전역 `enabled` 플래그를 덮어씀).
    - **`groups`**: 요일별 장치 그룹 소속 (전역 `group_name`을 덮어씀). 빈 문자열은 "그 요일에는 그룹에서 제외"를 의미합니다.
    - **`durations`**: 요일별 작동 시간 (전역 `action_duration`을 덮어씀).

시간 규칙:

- 요일 인덱스는 Python 관례를 따릅니다: `0`=월, `1`=화, … `6`=일.
- 시간은 **장치 로컬 타임존**의 벽시계 `HH:MM` 형식입니다.
- 종료 시간 `24:00`은 "하루의 끝"을 의미하며 내부적으로 1440분으로 저장됩니다.
- 자정을 넘기는 창(`start >= end`)은 허용되지 않습니다. 각 요일은 `00:00`~`24:00` 범위 안에서만 작동합니다.
- **자정 연속성**: 앞 요일의 종료가 `24:00`이고 다음 요일의 시작이 `00:00`이며 두 요일이 모두 활성이고 인접해 있으면, 데몬은 자정에 사이클을 리셋하지 않고 연속으로 이어서 실행합니다.

!!! 참고
    주기(`period`)가 그 요일의 작동 창 길이보다 길면 사이클이 창 안에서 잘립니다. 설정 화면은 이 경우 경고를 표시합니다.

    창이 닫힐 때까지 남은 시간이 한 바퀴를 다 돌기에 모자라면, 그 사이클은 **시작하지 않고 건너뜁니다**. 예전에는 그런 경우에도 사이클을 시작해 밸브가 열리자마자 창이 닫히며 강제로 끊겼는데, 그렇게 끊긴 단계는 개방 시간이 기록되지 않아 얼마나 물을 주었는지 확인할 수 없었습니다.

    창 길이와 한 바퀴가 딱 맞아떨어지는 설정에서는 마지막 단계의 종료와 창 닫힘이 같은 순간에 옵니다. 이때 창 판정이 먼저 이기면 단계가 스스로 끝나기 전에 꺼져 개방 시간이 남지 않으므로, 종료가 코앞인 단계는 몇 초 기다려 정상적으로 끝냅니다. 아직 한참 남은 단계는 기다리지 않습니다 — 그건 자연 종료가 아니라 실제 중단이고, 기다려 봐야 창만 넘깁니다.

### 시퀀스를 끄면 바로 멈춥니다 { #sequence-deactivate }

함수 목록이나 위젯에서 시퀀스를 비활성화하면 **돌고 있던 제어가 그 자리에서
정지**하고, 그때 열려 있던 단계는 정상적으로 닫힙니다. 다음 작동 시간이 와도
다시 시작하지 않습니다.

끄는 조작이 제어 데몬까지 닿지 못했다면 화면이 **"여전히 동작 중"** 이라고
알려 줍니다. 그때는 설정이 저장됐더라도 밸브가 계속 돌고 있을 수 있으니, 잠시
뒤 다시 시도하거나 출력을 직접 확인하십시오. 예전에는 이 경우에도 "저장됨"으로
보여, 화면은 꺼진 것으로 보이는데 실제로는 관수가 계속되는 상태가 만들어졌습니다.

!!! 주의
    **요일별 모드에서 위젯의 주기를 바꾸면 그날 요일에만 적용됩니다.** 위젯이
    보여주는 주기는 오늘의 값이므로, 다른 요일의 주기는 그대로 남습니다. 요일마다
    다르게 정하려면 각 요일을 선택한 상태에서 바꾸거나 통합 모달의 요일별 설정을
    쓰십시오.

### 다시 켤 때: 이어서 vs 처음부터 { #sequence-resume-on-activate }

시퀀스를 껐다가 다시 켜면 기본적으로 **멈춘 지점부터 이어서** 진행합니다. 잠깐
멈췄다 재개하는 경우를 위한 동작입니다. 설정을 고치고 처음부터 새로 돌리고
싶다면 시퀀스 위젯 설정의 **다시 켤 때** 항목을 「처음부터 시작」으로 바꾸십시오.

| 값 | 다시 켰을 때 |
|---|---|
| **멈춘 지점부터 이어서** (기본) | 중단된 사이클의 남은 단계부터 계속합니다 |
| **처음부터 시작** | 첫 단계부터 새로 실행합니다 |

「이어서」로 두어도 중단된 지 한 주기가 넘었으면 처음부터 시작합니다 — 그만큼
지났다면 이어 붙일 사이클이 남아 있지 않기 때문입니다.

!!! 참고
    **데몬 재시작은 이 설정과 무관합니다.** 시스템 업데이트나 재부팅으로 제어가
    잠시 멈춘 것은 사용자가 끈 것이 아니므로, 진행 중이던 관수는 언제나 이어서
    진행합니다. 이 설정이 정하는 것은 사람이 직접 껐다가 켜는 경우뿐입니다.

!!! 참고
    **진짜 처음 켜는 경우, 첫 사이클은 정해진 눈금이 아니라 지금 이 순간부터
    시작합니다.** 이어받을 이전 사이클이 전혀 없을 때만(신규 시퀀스이거나
    「처음부터 시작」으로 이전 사이클을 비운 뒤) 그렇습니다. 예전에는 이 경우도
    데몬이 같은 날의 사이클이 표류하지 않게 쓰는 고정 눈금(위 "시작하지 않고
    건너뜁니다" 항목의 그 눈금)을 그대로 기준으로 삼아서, 창 중간에 시퀀스를
    켜면 새 사이클이 이미 몇 시간 지난 것으로 계산돼 그 안의 단계가 전부 이미
    끝난 것처럼 처리되는 경우가 있었습니다. 이전 사이클의 상태가 남아 있다면
    (너무 오래돼 이어받지 못하더라도) 여전히 그 눈금을 기준으로 삼습니다 —
    그 경우는 사용자가 방금 켠 것이 아니라 데몬이 내려가 있던 것이기 때문입니다.

### 공통 ↔ 요일별 전환 { #sequence-schedule-mode-switch }

요일별 모드에서 공통 모드로 되돌리면, **첫 활성 요일**의 설정이 공통값으로
올라옵니다 — 작동 시간(시작·종료·주기)뿐 아니라 그 요일의 **장치 그룹 구성과
단계별 작동 시간**도 함께 옮겨집니다. 공통 모드는 요일별 맵을 읽지 않으므로,
옮기지 않으면 요일마다 짜 둔 구성이 버튼 한 번에 사라집니다.

기준이 되는 요일은 작동 시간과 같은 규칙(첫 활성 요일)입니다. 여러 요일을 서로
다르게 구성해 두었다면 전환 뒤 나머지 요일의 차이는 남지 않으니, 어느 요일을
공통으로 삼을지 정한 뒤에 전환하십시오.

### 설정 모달은 항상 지금 값을 보여줍니다 { #sequence-widget-live-values }

시작·종료 시간, 주기, 교차 시간, 재개 방식은 요일별 시간 편집기(시간휠)나
함수 설정 화면 등 **이 위젯이 아닌 다른 경로**로도 바뀔 수 있습니다. 그렇게
바뀐 값은 관수에 즉시 반영되고, 이 위젯의 설정 모달도 대시보드를 열 때마다
그 값을 다시 읽어 보여줍니다 — 옛 값을 저장해 둔 채 있다가 다음에 이 위젯을
저장할 때에야 맞춰지는 일이 없습니다. 예전에는 함수 설정 화면에서 시작·종료
시간이나 주기를 고치면, 이미 요일별 스케줄을 쓰고 있는 시퀀스에서는 저장은
성공했다고 나오면서 데몬이 실제로 따르는 스케줄은 그대로인 경우가 있었습니다 —
지금은 어느 저장 경로를 쓰든 같은 정본 스케줄에 기록됩니다.

!!! 참고
    요일별 모드에서는 함수 자체의 설정 화면에 있는 시작·종료·주기 칸도
    반영되지 않습니다 — 전역 값 하나가 요일별로 짜 둔 구성을 통째로 지워버리기
    때문에 같은 이유로 막습니다. 이제는 저장할 때 조용히 성공이라고 답하는
    대신 그 사실을 알려줍니다 — 요일별 시간은 시퀀스 위젯의 단계 목록에서
    고치십시오.

시퀀스가 작동하는 중에 **단계**의 설정 — 작동 시간, 장치 그룹, 요일별 켬/끔 —
을 바꾸면 그 변경도 다음 사이클이 아니라 **지금 돌고 있는 사이클**에 바로
반영됩니다. 예전에는 화면에는 곧바로 저장됐다고 나와도, 실제 반영은 다음
사이클이 시작될 때까지(주기가 길면 몇 시간 뒤까지) 미뤄졌습니다. 작동 중 편집이
엉뚱하게 작동하지 않도록 두 가지 안전장치를 함께 넣었습니다 — 이번 사이클에
이미 끝난 단계는, 앞 단계를 줄여 슬롯 경계가 당겨져서 그 창 안에 지금 경과
시간이 다시 들어오더라도 다시 켜지지 않습니다. 그리고 오늘 계획에서 빠진 단계
(오늘 요일에서 끄거나 아예 지운 단계)는 추적만 끊는 것이 아니라 **명시적으로
끕니다** — 그래야 그 출력이 켜진 채 아무도 모르게 남지 않습니다.

자정을 넘는 창(같은 날 안에서 종료 시각이 시작 시각보다 이른 경우)은 저장을
시도하는 시점에 이유와 함께 거부됩니다. 예전에는 함수 설정 화면에서 그런 창도
저장할 수 있었는데, 그러면 데몬이 창을 그날 안으로 잘라 쓰면서 나머지 요일별
구성을 조용히 버렸습니다.

!!! 참고
    시퀀스 위젯 자체의 설정 대화상자에는 더 이상 시작 시각·종료 시각 텍스트
    입력칸이 없습니다 — 요일별 스케줄을 쓰는 순간부터는 그 두 칸이 애초에
    제대로 동작하지 않았으므로, 시작·종료 시각은 아래에서 설명하는 통합
    모달의 시간휠로만 정합니다.

### 통합 모달 · 시간휠

시퀀스 위젯의 단계 목록에서 시간·그룹·이름·요일 스케줄을 **하나의 통합 모달**에서 편집합니다.

- 단계 이름을 클릭하면 표시 이름(`display_name`)·모드(`single`/`total`)·장치 그룹을 함께 수정하는 모달이 열립니다.
- 시작·종료 시간은 **시간휠(time wheel)** 컴포넌트로 입력하여 모바일에서도 정확하게 시:분을 선택할 수 있습니다.
- 레거시 시작·종료·요일 컬럼은 스케줄 저장 시 자동으로 동기화되어(하위 호환), 스케줄을 사용하지 않는 구형 뷰에서도 대표 값이 유지됩니다.

## 통합 환경 제어 - 육묘장 모드 { #nursery-mode }

통합 환경 제어 기능은 등록된 모든 액추에이터를 VPD 목표에 맞춰 조율합니다. 이 절은 육묘장 모드 옵션만 다루며, 나머지 옵션은 기능 설정 화면에서 설명합니다.

### 육묘 모종을 다르게 다뤄야 하는 이유

분무는 VPD를 가장 빠르게 낮추는 수단입니다. 온도를 내리면서 습도를 동시에 올리기 때문입니다. 그래서 조율기는 VPD가 오르면 분무를 가장 먼저 고릅니다.

문제는 VPD가 가장 높아지는 시각이 곧 햇빛이 가장 강한 시각이라는 점입니다. 성체 작물이라면 괜찮습니다. 잎이 젖어도 견디고 증발냉각이 오히려 도움이 됩니다. 그러나 상토를 막 뚫고 나온 모종은 아직 큐티클이 없어서, 강한 햇빛 아래 자엽에 맺힌 물방울이 빛을 잎면에 모으고 마르면서 녹아 있던 염류를 농축시킵니다. 그 자리가 탑니다.

육묘장 모드는 VPD 목표를 바꾸지 않습니다. 목표에 도달하는 수단의 순서를 바꿉니다. 햇빛이 강한 동안 분무를 뒤로 미루고 차광과 환기를 먼저 쓰게 합니다.
...
<tr>
<td>육묘장(모종) 모드</td>
<td>아래 보호 기능을 켭니다. 어떤 노즐이 잎을 적시는 형태인지는 시설 도면의 노즐 배치(유량·살수 반경·분사 방향)에서 자동으로 판정하므로 노즐 사양을 따로 입력하지 않습니다. 점적 배관과 진짜 고압 미세포그는 대상이 아닙니다.</td>
</tr>
<tr>
<td>분무 잠금 일사량 (W/m²)</td>
<td>실내 광량이 이 값 이상이면 습윤형 분무를 완전히 차단합니다. 실내 추정 광량을 기준으로 하므로 차광막을 닫으면 잠금이 완화됩니다.</td>
</tr>
<tr>
<td>분무 해제 일사량 (W/m²)</td>
<td>광량이 이 값 아래로 내려가면 분무를 다시 허용하고, 이 값과 잠금 임계 사이에서는 선형으로 줄입니다. 두 값의 간격이 구름이 지나갈 때마다 분무가 켜졌다 꺼졌다 하는 것을 막습니다.</td>
</tr>
<tr>
<td>1회 분무 최대 시간 (초)</td>
<td>한 번에 분무할 수 있는 최대 시간입니다. 가습량은 얼마나 오래 뿌리느냐가 아니라 얼마나 자주 뿌리느냐로 조절됩니다. 관수가 일정 간격으로 정량을 주는 방식과 같습니다.</td>
</tr>
<tr>
<td>강제 건조 간격 (초)</td>
<td>한 번 분무가 끝난 뒤 이 시간 동안은 전혀 분무하지 않아 잎이 마를 시간을 줍니다.</td>
</tr>
<tr>
<td>일몰 전 분무 허용</td>
<td>관수는 보통 일출과 일몰 무렵에 하는데, 저녁 분무는 잎이 젖은 채로 밤을 넘기게 만듭니다. 잎이 젖어 있는 시간이 길수록 잿빛곰팡이와 노균병 위험이 커집니다. 끄면 일몰 전에 분무를 중단합니다. 저녁 관수가 필요한 작물도 있으므로 선택은 재배자 몫입니다. 이 설정은 습도 관리용 분무만 제어하며, 같은 밸브에 걸린 별도 관수 일정에는 영향을 주지 않습니다.</td>
</tr>
<tr>
<td>일몰 전 분무 중단 시간 (분)</td>
<td>위 옵션을 껐을 때 일몰 몇 분 전부터 분무를 중단할지 정합니다. 중단은 다음 일출까지 이어집니다. 보통 2시간이면 어두워지기 전에 잎이 마릅니다. 일몰 시각은 지도상 시설 위치에서 계산하며, 좌표를 해석하지 못하면 차단하지 않습니다.</td>
</tr>
<tr>
<td>분무 원수</td>
<td>처리하지 않은 지하수는 대개 경도가 높고 수온이 낮아, 물방울이 마르면서 염류가 농축되어 남고 햇빛을 받는 잎에 저온 충격을 줄 수 있습니다. 지하수를 선택하면 잠금 임계가 자동으로 낮아집니다.</td>
</tr>
</tbody>
</table>
...
분무기를 개도(%)로 조절하면 매 사이클의 그 비율만큼 계속 뿌리게 되어 잎이 마를 틈이 없습니다. 그래서 잎을 적시는 형태의 분무기는 육묘장 모드와 무관하게 항상 짧은 펄스와 강제 휴지로 나뉩니다. 연속 분무는 성체 작물에서도 병해를 부르기 때문입니다. 육묘장 모드는 그 수치를 더 조일 뿐입니다. 잎에 닿기 전에 증발하는 고압 미세포그와 점적 배관은 기존의 연속 조절을 그대로 유지합니다.

### 참고

분무 잠금은 폭염 비상 대응과 최저 습도 하한보다 우선합니다. 한여름 정오는 폭염 비상이 발동하는 시각이자 일소 위험이 최대인 시각이고, 습도 목표를 채우자고 모종을 잃을 수는 없기 때문입니다. 멈추는 것은 분무뿐이며 창호·커튼·팬은 정상 운전합니다.

순화가 진행되면 보호를 한 번에 끄지 말고 점진적으로 완화하십시오. 한낮 분무를 전혀 하지 않다가 하루아침에 전면 허용으로 바꾸면, 이 모드가 막으려던 조건을 조금 자란 모종에 그대로 재현하게 됩니다.

일소로 보이는 피해 중 일부는 빛이 아니라 물에서 옵니다. 경수는 광물 성분을 남기고, 찬물은 햇빛 받는 잎에 충격을 주며, 철분 침착은 일소와 거의 구별되지 않습니다. 반점을 묽은 산으로 닦아 지워지면 철분입니다. 물 자체가 나쁘면 어떤 제어 설정으로도 해결되지 않습니다.

### 관수와 가습을 한 장치로 처리하는 경우

육묘장에서는 머리 위 스프링클러 한 벌로 관수와 가습을 함께 해결하는 일이 흔합니다. 이때 통합 환경 제어에 그 노즐을 맡기면 관수 쪽과 부딪힙니다. 조율기는 지금 쓰지 않는 액추에이터에도 "꺼 두라"는 명령을 매 주기 내보내기 때문에, 별도로 돌리는 관수 제어가 밸브를 열어도 다음 주기에 닫힙니다. 가습이 필요 없는 새벽일수록 이 명령은 더 확실하게 나갑니다.

물이 남는 방식도 다릅니다. 관수용으로 고른 스프링클러는 가습에 필요한 양보다 훨씬 굵게 뿌리므로, 가습용으로 짧게 끊어 쓰면 잎에 얇은 수막만 남습니다. 이 수막은 흘러내리지 못하고 그 자리에서 마르며 물에 녹아 있던 성분을 잎에 남깁니다. 반대로 아침에 흠뻑 주는 관수는 대부분 흘러내리면서 앞서 남은 것까지 씻어 냅니다. 같은 물, 같은 노즐인데 결과가 갈리는 이유가 이것입니다.

**가습에 습윤형 분무 사용**을 끄면 통합 환경 제어가 그 노즐에 아무 명령도 내리지 않습니다. 관수 일정이나 토양 수분 제어에 온전히 맡기고, 습도는 스크린·개구부·팬으로 관리합니다. 잎에 닿기 전에 증발하는 고압 미세포그는 이 설정과 무관하게 계속 가습에 쓰이며, 가습기와 점적 배관을 비롯한 다른 액추에이터도 영향을 받지 않습니다.

한낮 습도 관리를 분무에 의존하지 못하게 되지만, 대개는 손해가 아닙니다. 차광 스크린을 한 겹 더 치는 것이 분무와 비슷한 냉각 효과를 내면서 물도 염류도 쓰지 않기 때문입니다.
