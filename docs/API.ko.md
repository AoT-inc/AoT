...
한 사용자가 **여러 개의 API 키를 각각 이름을 붙여** 가질 수 있습니다. **관리 → 시스템 관리 → 사용자**에서 사용자를 편집한 뒤 **API 키** 단락에서 발급합니다. 키를 쓸 클라이언트를 알아볼 수 있게 이름을 붙이고(예: "ChatGPT", "Claude Desktop") **생성**을 누르면 됩니다 — 새로 발급해도 기존 키를 덮어쓰지 않으므로, 같은 사용자가 여러 연동을 동시에 쓸 수 있고 키 하나만 따로 폐기해도 나머지에는 영향이 없습니다.

발급할 때 **권한**을 고릅니다: **전체 권한**(기본값 — 그 사용자 계정과 같은 권한)과 **읽기 전용**(데이터 조회는 되지만 상태를 바꾸는 요청은 모두 `403 Forbidden`으로 거부) 중 하나입니다. 권한은 발급 전에 정해야 하며, 이미 발급된 키의 권한은 바꿀 수 없고 폐기 후 다시 발급해야 합니다.

키는 128바이트 난수값이며, **생성되는 그 순간 딱 한 번만** base64로 인코딩된 문자열로 표시됩니다 — AoT는 키 자체가 아니라 단방향 해시만 저장하므로 이후에는 다시 표시할 수 없습니다. 분실했다면 새로 발급하십시오. 자세한 내용은 [보안](Security.ko.md#api-keys)을 참조하십시오.
...
프로그램적으로 처리할 수 있는 4xx 오류(예: 카드가 거절됨)에는 보고된 오류를 간략하게 설명하는 오류 코드가 포함됩니다. 이 API에 특유한 예시 하나: **읽기 전용** API 키로 상태를 바꾸는 요청(`GET`/`HEAD`/`OPTIONS` 이외의 모든 요청)을 보내면, 그 사용자에게는 원래 허용되는 요청이라도 `{"error": "Forbidden", "message": "..."}` 와 함께 `403 Forbidden` 이 돌아옵니다.
