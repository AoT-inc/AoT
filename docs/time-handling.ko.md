...
- **호스트 OS 시계 = "지금"의 유일한 공급원. tz가 아니다.**
  예약 코드는 OS(컨테이너)의 로컬 tz에 의존해서는 안 된다.
  역할은 오직 `timekit.utc_now()`로 **현재 UTC 순간**을 주는 것. 스케줄러 발화는
  `fire_utc <= utc_now()` 비교로만 이뤄지며 여기에는 tz가 개입하지 않는다.
- **컨테이너 `TZ` 환경변수는 해석 체인에 들어가지 않는다.** 하는 일은 **첫 실행 기본값**
  하나뿐이다. 데이터베이스를 처음 만들 때 `TZ`가 시스템 tz(`misc.timezone`)로 한 번
  복사되고, 이후에는 *시스템 시간대* 설정이 정본이라 이미 설치된 곳에서 `TZ`를 바꿔도
  달라지지 않는다. 로그 시각도 `TZ`가 아니라 시스템 tz를 따른다. 정리하면 `TZ`는 첫 실행
  기본값용이고, 예약은 *시스템 시간대*와 지도의 사이트·구역 위치(또는 장치 시간대)로 정한다.
...
사용자(서울, +9)가 방글라데시(다카, +6)의 밸브에 "관수 06:00"을 예약한다고 하자.
