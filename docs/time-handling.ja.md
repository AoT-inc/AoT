...
- **ホストOSの時計 = 「今」の唯一の供給源。tzではありません。**
  スケジュールのコードは、OS(コンテナ)のローカルtzに依存してはいけません。
  その役割は、`timekit.utc_now()`によって**現在のUTC時刻**を提供することだけです。
  スケジューラの発火は`fire_utc <= utc_now()`の比較だけで行われ、ここにtzは関与しません。
- **コンテナの環境変数`TZ`は解決チェーンに含まれません。** 役割は**初回起動時の既定値**だけです。データベースの初回作成時に`TZ`がシステムtz(`misc.timezone`)へ一度コピーされ、以後は*システムタイムゾーン*設定が正本となるため、既存の環境で`TZ`を変えても何も変わりません。ログの時刻も`TZ`ではなくシステムtzに従います。つまり、`TZ`は初回の既定値のために設定し、スケジュールは*システムタイムゾーン*と地図上のサイト・ゾーンの位置(またはデバイスのタイムゾーン)で決めます。
