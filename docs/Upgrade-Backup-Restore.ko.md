...
!!! 참고
    웹 화면에서 백업을 내려받거나 복원하는 것은 **관리자만** 할 수 있습니다. 백업에는 사용자 계정·역할·비밀번호 해시가 든 설정 데이터베이스가 들어 있고, 복원하면 사용자 계정과 역할도 백업 시점으로 돌아가기 때문입니다. 다른 역할에게는 Download·Restore 버튼이 보이지 않습니다.
