...
센서로 환경을 관측하고 장치를 원격 제어하는 오픈소스 시스템이며, 특정 용도나 장소에 매이지 않습니다.

[라즈베리 파이](https://en.wikipedia.org/wiki/Raspberry_Pi) 같은 단일 보드 컴퓨터에 직접 설치하거나, 일반 서버·PC에서 Docker로 실행할 수 있습니다.

제공하는 것은 다음과 같습니다.

- 검증된 **입력·출력·함수** 제어 모델. AoT 가 출발점으로 삼은 Mycodo 프로젝트에서 이어받았습니다([유래](#origins) 참고).
- **GIS 지도** — 장치·센서·구조물을 지도 위 실제 위치에 놓고, 거기서 보고 조작할 수 있습니다.
- **MCP(Model Context Protocol) 기반 AI** — 시스템을 읽고 진단하며, 장비를 움직이는 동작은 사용자 승인을 받아 실행하는 어시스턴트입니다.

무엇을 쓸지는 사용자가 정합니다.

## GIS — 지도·시설·정보

지도 위젯에는 장치·센서·구조물·구역이 실제 위치대로 놓입니다. 쓸지 말지는 사용자가 정하며, 지도 없이 구성해도 나머지는 같게 동작합니다.

- **공간 계층** — 사이트 > 구역 > 시설 > 식생 구획이 실제 계층을 이룹니다. "동쪽 하우스 상황이 어떤가" 같은 질문에 정해진 답이 나옵니다.
- **3D 시설** — 시설 외형을 폴리곤으로 정의해 3D로 시각화하고, 창호·커튼 등 구성 요소를 형상에 연결해 그 자리에서 제어합니다.
