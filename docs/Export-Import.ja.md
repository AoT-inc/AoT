...
!!! note
    設定のエクスポートとインポートは**管理者のみ**が行えます(Webインターフェース・APIとも)。設定データベースにはユーザーアカウント・ロール・パスワードハッシュが含まれているため、管理者以外がエクスポートするとアカウント情報が漏れ、インポートすると自分の権限を変更できてしまうためです。他のロールには設定のエクスポート・インポート欄が表示されません。測定値のエクスポートはこれまでどおり使えます。
