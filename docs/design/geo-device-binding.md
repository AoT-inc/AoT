...
- `check_geo_integrity.py` 에 3검사 추가: `orphan-device-shape`(도형이 실존하지
  ...
  `binding-drift` 가 세는 것은 **"자리에 현재 바인딩이 아예 없는데 레거시
  컬럼에만 연결이 있는"** 경우다. 구역 폴리곤(`role='area'`)에서 자리는
  매여 있고 컬럼만 옛 장치를 가리키는 것은 세지 않는다 — 그 컬럼은 장치를
  갈아끼울 때 **일부러** 두는 것이라(`_move_marker_column` 이 마커만 따라가고
  `TestLegacyColumnStaysInStep` 이 그 결정을 못 박는다) 세면 구역 장치를 한
  번이라도 교체한 설치에서 숫자가 영영 0 이 되지 않는다. 아래 게이팅이
  "0 건"인 이상 그 상수 잡음은 곧 게이트를 영구히 닫아 두는 것이고, 진짜
  드리프트도 그 뒤에 묻힌다(2026-09-06 로컬 실측: 4건 중 1건이 이 경우).
  마커는 계속 센다 — 마커 컬럼은 따라가기로 돼 있으므로 어긋나 있으면 그
  동기화가 실패했다는 뜻이고 실제로 고장이다(`place_device` 가 컬럼으로
  마커를 찾는다).
...
**끝난 바인딩은 폴백하지 않는다 — 빈 슬롯이다** (2026-09-25 정정).
"지금 유효한 바인딩이 없다" 는 두 가지이고 처방이 정반대다.

| 원장 상태 | 뜻 | 리졸버 |
|---|---|---|
| 행이 하나도 없음 | 백필 전 레거시 데이터 | 저장값으로 폴백 |
| 끝난 행만 있음 | 사람(또는 게이트웨이)이 비운 슬롯 | **빈 슬롯**(`VACATED`) |

처음 구현은 유효한 행만 조회해서 둘을 구분하지 못했고, 테스트
(`test_ended_binding_is_not_current`)가 그 동작을 "레거시로 폴백해야 한다" 로
고정하고 있었다. 레거시 컬럼은 바인딩이 끝날 때 정리되지 않으므로, 폴백은
**끊은 연결을 되살렸다.** 실측(로컬): イチゴ 의 측창 슬롯이 9/6 에 끊긴
`측창: 좌/우`(육묘장3 소유)를 되돌려 받아, 쿠마모토 코디네이터가 19일간
육묘장3 의 측창을 함께 제어했다 — 두 코디네이터가 같은 장치를 반대로 밀어
10분 주기 톱니가 났고, 양쪽 로그에는 정상 근거만 남았다.

함께 고친 것:

- `build_facility_index()` 가 끝난 슬롯도 `VACATED` 로 담는다(목록·단건 경로가
  같은 답을 내도록).
- 레거시 컬럼을 **직접 읽던 8곳**을 `resolved_refs()` 로 옮겼다 — 시설 위젯·관수
  상태 2곳·일지 지도·AI 요약, 그리고 **장치를 실제로 움직이는** 시운전·시설 일괄
  명령·안전 상태 전환. `test_stale_legacy_ref.py` 가 AST 로 재발을 막는다.
- 무결성 검사 `stale-legacy-ref`(severe) — 원장이 끊은 연결이 레거시 컬럼에 남은
  자리. 이전에는 `binding-drift` 가 지도 도형만 봐서 시설 쪽은 검사가 없었다.
- 코디네이터가 다른 시설 코디네이터와 같은 장치를 잡으면 error 로 알린다
  (빼지는 않는다 — 주인은 사람이 정한다). 건강 점검에도 같은 항목.
...
### Phase C-3 — 쓰기 전환 ✅ 구현 완료 (2026-08-08), 폴백 제거만 남음

**읽기 이전.** "이 장치의 도형은 어느 것인가"를 다섯 곳이 각자
`GeoShape.query.filter_by(device_id=...)` 로 풀고 있었다(이름 동기화·시간대
상속·AI 좌표 동기화·위성 좌표 폴백·조율기 프로필 적재). 다섯 벌이면 다섯
번 다르게 틀린다 — 어떤 곳은 `type` 을 걸고 어떤 곳은 안 걸었다.
`shapes_for_device()`/`shapes_for_devices()` 하나로 모았다.

도형 JSON 을 **고치던** 두 곳(이름 동기화·AI 좌표 동기화)은 geo 로 옮겼다
(`sync_device_name`·`move_device_markers`). 옮기면서 버그가 드러났다: AI
좌표 동기화는 `.first()` 로 마커 하나만 고쳐, 채널이 여럿이거나 여러 지도에
놓인 나머지 마커는 옛 자리에 남았다.

**쓰기 전환.** `save_overlays`(전체·델타 양쪽)와 `facility_io.save_facility`
가 바인딩을 기록한다. 시설 쪽은 편의가 아니라 **버그 수정**이었다: Phase B-1
이 읽기를 바인딩으로 옮긴 뒤로, 편집기에서 fitting 의 장치를 비워도 바인딩이
그대로라 화면에는 계속 옛 장치가 보였다 — 저장이 읽기에게 지고 있었다.

동기화는 **페이로드가 아니라 저장된 값**을 기준으로 한다. 부분 저장에서
페이로드를 기준으로 삼으면 "빠진 것 = 지운 것"이 되어 멀쩡한 배정이 끊긴다
(`save_overlays` 가 정확히 그 프로토콜로 도형을 잃었다). 단일 점유는
`rebind`(사유 `replaced`), 다중 점유는 집합 차이로 처리한다.

**GB-5** 적용(`p6_29`). 저장 경로가 `properties.device_id`/`channel_id` 를
제거하고 트리거가 봉인한다. 로컬 실측 사본 20건 → 0건, 읽기는 리졸버 주입
으로 종전과 동일. `properties.unique_id` 는 제외(GB-5b).

**GB-6** 명부에서 geo 밖 소비처가 전부 빠졌다. 남은 것은 geo 패키지 자신
(폴백)과 진단 도구뿐이다.

### Phase C-4 — 폴백·레거시 컬럼 제거 ⏳ 배포에 걸려 있다

남은 것은 셋뿐이고 서로 묶여 있다: 리졸버의 레거시 폴백 제거,
`GeoShape.device_id` 컬럼 쓰기 제거, 컬럼 자체 제거.

**지금 하면 안 된다.** 게이팅 조건 "폴백 로그 0" 은 **운영 서버에서** 0 이어야
하는데, 운영 3서버는 아직 `p6_27`(테이블)조차 올라가지 않았고 백필도 돌지
않았다. 그 상태에서 폴백을 지우면 바인딩이 하나도 없는 서버에서 **모든 장치
연결이 즉시 사라진다.** 로컬이 깨끗한 것은 여기서만 백필을 돌렸기 때문이다.

순서: 배포 → `backfill_geo_binding --apply`(서버마다 dry-run 확인 후) →
며칠간 폴백 로그·`binding-drift` 관찰 → 그 다음이 C-4 다.

⚠ 그 백필은 2026-09-06 까지 **자리의 임자가 다르면 통째로 죽었다.** 멱등
키에 `device_id` 가 들어 있어서 "이 자리는 이미 다른 장치가 쓰고 있다" 를
알아보지 못하고, GB-1 단일점유 인덱스가 거부할 행을 밀어 넣었다. 전부 한
트랜잭션이라 **정당한 나머지가 하나도 반영되지 않은 채 종료코드 2 로
끝난다**(로컬 실측: 유효 170건이 그 한 줄 때문에 전멸). 구역 장치를 한 번이라도
교체한 서버에서는 반드시 재현되므로, 위 순서를 운영에서 밟기 전에 이 수정이
배포돼 있어야 한다. 지금은 그런 자리를 건너뛰고 보고한다 — 죽은 참조를
바인딩으로 승격시키지 않는 정책과 같은 논리다(갈아끼워진 옛 참조도 정본으로
되살리지 않는다).

컬럼 쓰기를 폴백보다 먼저 끊어도 안 된다. 둘을 같은 값으로 유지해야
`check_geo_integrity` 의 binding-drift 가 의미를 갖는다 — 한쪽만 끊으면
감시자가 먼저 눈이 먼다.

`channel_id` 컬럼은 device_id 보다 늦게 죽는다. I2(마커 유일성)가
`(geo_id, device_id, channel_id)` 인덱스로 걸려 있어, 그 불변식을 바인딩
기준으로 다시 표현하기 전에는 컬럼을 뺄 수 없다.

### Phase D — 교체 UX + AI + 시계열 접합

- **"장치 교체" 플로우** ✅ (2026-08-08): 접속정보(DevEUI·주소)만 갈아끼우는
  경로를 1안으로 제시(도형·이력·함수 연결 전부 무변 — 사실 고장 교체의 정답),
  새 장치로의 리바인딩을 2안으로. 사용자가 "새로 만들고 옛것 삭제"로 가던
  것은 이 경로가 UI에 없었기 때문이다.
  구현: `aot/aot_flask/utils/utils_device_connection.py` +
  `/api/device/<id>/connection`(GET/POST) + 교체 폼.
  진입점 둘: 지도의 슬롯 배정 모달 「교체」, 시설 편집기 설비 인스펙터의
  「하드웨어 교체」(장치가 물려 있을 때만 보인다).
  - 폼은 `static/js/common/aot-device-connection.js` 한 벌이 소유하고 두 번들이
    함께 넣는다. **모달 셸은 각자 소유한다** — 셸까지 공용화하면 두 페이지의
    모달 관례를 하나로 맞춰야 하고 그건 이 기능이 요구하는 것보다 크다.
  - 결과는 **전역 `window.showToast`**(`layout_default.html`)로 알린다.
    `AoTGlobalSettings` 의 표시 여부 설정이 거기서 지켜지므로, 화면마다 자기
    알림을 만들면 사용자가 끈 알림이 어떤 화면에서만 계속 뜬다.
  - 시설에서도 **바인딩은 건드리지 않는다.** 설비의 장치 배정은 시설 JSON 이
    정본이라 여기서 바인딩을 만들면 다음 시설 저장이 지운다. 접속정보 갱신은
    장치 자신을 고치는 일이라 그 함정에 원리적으로 걸리지 않는다.
  - 편집 대상은 **드라이버 선언**(`options_enabled`/`options_disabled`)과
    `_OPTION_COLUMNS` 의 교집합 + `_CONNECTION_OPTION_TYPES` 명부의
    custom_option 이다. 컬럼을 훑는 방식이 아니라 선언이 정본이다.
  - **값의 종류는 드라이버 선언이 아니라 명부가 정한다.** 드라이버의 `type` 은
    폼 위젯을 정할 뿐이라 포트를 `'text'` 로 선언한 모듈이 흔하고, 그것을
    믿었더니 포트에 한글이 저장됐다(브라우저 실증에서 실제 발생·수정).
  - 자격증명(키·토큰·비밀번호)은 응답에 실리지 않는다. 명부에 실수로 들어와도
    `_is_secret_key()` 가 막는다.
  - 활성 상태 Input/Function 은 서버가 비활성화 → 수정 → 재활성화를 대신
    밟는다. **재활성화 실패는 성공 메시지에 묻히지 않고 경고로 올라온다** —
    저장은 됐는데 장치가 꺼진 채 남는 것이 가장 위험한 결말이다.
  - 교체는 `category='maintenance'` 노트로 남는다. 장치 uuid 가 그대로라
    바인딩은 이 사건을 기록하지 못하므로, "이 시점부터 값이 왜 달라졌나"에
    답할 수 있는 유일한 기록이다.
- 미배정 슬롯 뷰 ✅ (지도 `#btn-unbound-slots`, 시설 `#fac-unbound-box`) —
  장치를 지워도 도형·설비는 남으므로(B4), 남은 자리가 보여야 그 정책이
  성립한다. 안 보이면 사람은 설비를 새로 그린다.
  - **시설 목록에서는 바인딩을 만들지 않는다.** 설비의 장치 배정은 시설 JSON 이
    정본이고 바인딩은 저장 후 `sync_facility_bindings` 가 파생하므로, 여기서
    만든 바인딩은 다음 시설 저장이 지운다(실측). 행을 누르면 그 설비를
    인스펙터에서 열어 주기만 한다 — 고치는 것은 드롭다운이다.
    지도판이 배정 모달을 여는 것과 갈리는 지점이고, 갈리는 이유가 이것이다.
  - `kinds=fitting,actuator` 로 반드시 좁힌다. 좁히지 않으면 지도 구역이 섞여
    들어와 시설 편집기에서 남의 관할을 보게 된다.
  - 목록이 비면 패널 자체를 감춘다. 정상 상태에 늘 켜져 있는 표시는 곧 아무도
    안 보게 된다(CI 13연속 실패에서 배운 것과 같은 이유).
- AI 도구 ✅ (2026-08-08): `rebind_device`·`list_unbound_slots`.
  `rebind_device` 는 `mutating=True` + 승인 필수 — **`config_only` 면제 절대
  금지**(물리 제어 대상이 바뀌는 결정이다). SSOT 스냅샷은 같은 커밋에서 갱신.
  장치 단위 쓸기는 게이트웨이(`device_binding.rebind_device()`)에 있다 —
  AI 서비스에 두면 마커 충돌 판정·채널 축소 처리·레거시 컬럼 동기화 셋이
  게이트웨이 밖으로 새고, 그중 하나만 빠져도 조용히 깨진다.
  - **마커의 레거시 `device_id` 컬럼도 함께 옮긴다.** 정합성 미학이 아니라
    고장 방지다: `place_device`/`unplace_device` 는 마커를
    `(geo_id, device_id, channel_id)` 컬럼으로 찾으므로, 컬럼이 옛 장치를
    가리킨 채 남으면 새 장치를 배치할 때 마커가 하나 더 생기고 옛 장치를
    치울 때 **새 장치의 마커가 지워진다.** `binding-drift` 는 "레거시에만
    있는 연결"만 보므로 이 어긋남을 못 잡는다.
    C-4 에서 `_move_marker_column()` 은 통째로 사라진다.
  - 구역 폴리곤의 레거시 컬럼은 건드리지 않는다 — 마커 조회에 쓰이지 않아
    고칠 이유가 없고, 고치면 GB-6 명부만 늘어난다.
- graph-async 구간 접합 ✅ (2026-08-08): `aot/aot_flask/utils/utils_series_stitch.py`
  + `/async/...?stitch=1` + `/async_segments/<device>/<measurement>`.
  화면의 「같은 자리의 이전 장치 값도 함께」 체크박스가 켠다.
  - **교체 이력이 하나도 없으면 그 체크박스를 아예 내보내지 않는다.** 지도를
    쓰지 않거나 장치를 바꾼 적 없는 사용자에게는 이어 붙일 것이 없는데, 그런
    사람에게 이 옵션이 보이면 "장치를 교체하면 뭔가 달라지나"로 읽힌다.
    (초기 문구 「장치 교체를 관통해 잇기」는 뜻이 통하지 않는다는 지적을 받아
    바꿨다 — 기능이 아니라 결과를 말하는 문구여야 한다.)
  - **과거 데이터를 새 uuid 로 다시 쓰지 않는다.** 되돌릴 수 없고 "그때 실제로
    측정한 기계"라는 사실을 지운다. 조회 시점에 구간별로 물어 이어 붙인다.
  - 응답 모양(평평한 `[시각, 값]` 목록)을 그대로 둔 이유: 네비게이터·기간
    버튼·요청 순번 배선이 전부 그 모양을 전제로 짜여 있다.
  - **하드웨어 변경 표식은 별도 축이다** — `/async_segments` 의 `events` 가
    깃발(flags 시리즈)로 찍힌다. 값이 어느 시점에 한 단 뛰었을 때 그것이
    환경의 변화인지 기계가 바뀐 탓인지는 사람이 판단해야 한다.
    - **두 출처를 한 줄에 모은다.** 다른 장치로 옮긴 교체는 바인딩 구간
      경계에만 남고(uuid 가 바뀐다), 같은 모델로 갈아끼운 교체는 바인딩에
      흔적이 없다(uuid 가 그대로다) — 그쪽의 유일한 기록이 `maintenance`
      노트다. 하나만 보면 절반을 놓친다.
    - **접합 여부와 무관하게 그린다.** 접합은 데이터를 잇는 선택이고 이
      표식은 사실이다 — 잇지 않고 보더라도 "여기서 기계가 갈렸다"는 알아야
      한다.
    - `maintenance` 외의 노트 범주는 찍지 않는다. 일반 관찰까지 올리면 이
      줄은 곧 배경 소음이 되고 아무도 안 본다.
    - 깃발 위 글자는 장치 이름(10자에서 자름)이고 노트 본문은 툴팁이 맡는다.
      아이콘은 쓰지 않는다(프로젝트 규칙).
  - **접합하지 않는 것이 정상인 경우가 둘.** 이력이 하나뿐이면 빈 목록을
    돌려주고 기존 단일 장치 경로로 떨어진다(같은 결과를 두 코드 경로로 만들지
    않는다). 그리고 접속정보만 갈아끼운 교체는 uuid 가 그대로라 애초에 끊기지
    않는다 — 그 흔적은 maintenance 노트에 남는다.
  - 대응 측정값을 못 찾은 구간은 조용히 빼지 않고 `has_data=False` 로 돌려준다.
    말없이 건너뛰면 그래프가 "그 기간엔 아무 일도 없었다"고 거짓말한다.
  - 그룹핑 간격은 전체 범위 기준이다. 구간별로 계산하면 점 밀도가 교체 시점마다
    튀고 사람은 그 계단을 데이터의 변화로 읽는다.
...
## 그래프 선택 목록의 두 축 (2026-08-09)

측정값 목록은 설치가 커질수록 쓸모없어진다 — 로컬 개발 DB 조차 입력만 117개다.
"1-1 하우스의 온도"를 찾으려고 117줄을 훑는 것은 목록이 아니라 벌이다.
공간 계층이 이미 있으므로 그것으로 좁힌다.

- **구역 필터**(`device_membership.area_choices` / `device_ids_in_area`).
  고르지 않으면 전체다. 지도를 쓰지 않는 설치에서는 목록이 비어 드롭다운 자체가
  뜨지 않는다.
  - **필지와 구역을 종류별로 나눠 보여준다.** 이름순으로 섞으면 '1-1'~'1-9'
    같은 구역이 위를 다 차지하고 필지는 아래로 밀려, 화면만 보고는 필지가
    아예 없는 줄 안다(실제로 그렇게 보고받았다). `area_choices()` 가 `kind` 를
    함께 준다.
  - **고를 것이 아무것도 없는 구역은 내보내지 않는다.** 로컬 실측에서 36개 중
    25개가 장치도 노트도 없었는데, 그런 항목을 고르면 모든 목록이 통째로 비어
    "필터가 고장났다"로 읽힌다. 판정은 마커·바인딩·공간 노트만 보는 싼 것으로
    한다(참조는 이미 걸린 장치가 있어야 성립하므로 비어 있음을 뒤집지 못한다).
    장치가 없어도 노트가 있으면 남긴다 — "3-2 구역의 기록만 보기"는 고를 값이
    있는 선택이다.
  - 이름 없는 도형은 뺀다. 고르는 수단이 이름뿐이라 `(site 1)` 같은 자리표시는
    무엇인지 알려주지 못하면서 이름 있는 것들을 밀어낸다(로컬 실측 12건).
  - **`<select>` 를 쓰지 않는다.** `.aot-modern-select` 는 layout 이
    bootstrap-select 로 승격시키는데, 그러면 wrapper 와 내부 버튼이 각각 알약
    테두리와 화살표를 그려 테두리 두 겹·화살표 두 개가 된다
    (`aot-settings.css` 가 설정 페이지에서 따로 처방을 둔 것과 같은 증상).
    이 줄의 나머지가 전부 알약 드롭다운이라 select 하나만 이질적이기도 하다.
    같은 셸을 단일 선택(radio)으로 쓴다.
  - `device_ids_in_area` 는 도형을 못 찾으면 **`None`(거르지 않음)** 을 준다.
    빈 집합으로 접으면 지워진 구역을 필터로 들고 있을 때 화면이 통째로 빈다.
  - 복합장치가 구역 안에 있으면 그 하위 Input/Output 도 함께 넣는다. 하위
    장치는 보통 자기 마커가 없어(그릇만 지도에 놓는다) 기하 판정만으로는
    빠지는데, 그러면 "이 구역의 값"에서 정작 값을 재는 것들이 사라진다.
  - **소속 판정은 네 겹이다.** 하나만 쓰면 목록이 통째로 빈다:
    마커(폴리곤 안의 위치 점) · 바인딩(그 안의 구역 폴리곤·시설 설비가 맡긴
    장치) · 그릇(복합장치의 하위 Input/Output) · 참조(그 장치들을 읽거나
    움직이는 PID·함수). 처음엔 마커만 봤는데, 로컬 실측이 출력 16개 중 마커
    1개 · PID 2개 중 0개 · 함수 17개 중 1개라 구역을 고르면 목록이 거의
    비었다 — 마커는 "지도에 점을 찍었다"는 뜻일 뿐이다.
  - **노트 태그도 거른다.** 태그 자체는 구역에 속하지 않지만("관수"는 농장
    전체의 어휘다) 노트는 속한다. 그 구역에 노트가 하나라도 있는 태그만
    남긴다 — 아니면 골라도 아무것도 안 뜨는 태그가 남는다.
    노트의 소속은 셋으로 본다: 공간에 붙은 것(`target_id` 가 도형·시설 —
    실측상 이쪽이 다수다) · 장치에 붙은 것 · 좌표로 찍힌 것.
  - **목록만 좁히고 표시를 안 좁히면 필터가 거짓말이 된다.** `/async/...` 의
    노트 시리즈에도 `area` 를 넘겨 같은 집합으로 거른다.
- **복합장치 단위**(`utils_general.choices_composite_devices`). 복합장치는 값을
  재지 않는 **그릇**이라, 고르면 그 안의 측정값 전부를 뜻한다. 선택값은
  `<uuid>,ALL,device` 하나이고 `routes_page._expand_device_choices` 가 편다.
  - 하위 판정은 **소유(`parent_device_id`)뿐**이다. `/device/<id>/summary` 와
    `device_binding.expand_device()` 가 쓰는 기준과 같게 둔다 — 셋이 갈리면
    같은 장치가 화면마다 다른 것을 담는다.
  - 편 결과로 체크박스를 그리지 않는다(`selected_raw` 를 따로 넘긴다). 사람이
    고른 것은 장치 하나인데 편 결과로 그리면 장치를 골랐던 사실이 사라진다.
  - 측정값이 하나도 없는 장치는 목록에 넣지 않는다 — 골라도 아무것도 안 나오는
    항목은 사람에게 "고장났다"로 읽힌다.

## 결정 — 장치 교체 3건 (2026-08-08 확정)

셋 다 `rebind_device` 의 계약이 걸려 있어 도구 구현 전에 확정했다.
`rebind()` 자체는 슬롯 하나만 다루므로 1·2 는 **장치 단위 교체**(장치 A 의
모든 슬롯을 장치 B 로) 경로에서만 발생한다.

### 1. 채널 축소 교체 → **매핑 불가 채널은 미배정 전환 + 경고**

8채널 릴레이를 4채널로 교체하면 5~8번 슬롯에는 앉을 자리가 없다.

- **거부하지 않는다.** 작은 보드로 바꾸는 것은 정당한 교체다. 통째로 막으면
  사람은 다시 "새로 만들고 옛것 삭제"로 돌아간다 — 이 전체 작업이 없애려는
  바로 그 행동이다.
- **조용히 버리지 않는다.** 5~8번이 바인딩을 유지한 채 남으면 "장치가 붙어
  있는데 아무 명령도 안 가는" 자리가 된다. 가장 위험한 결말이다.
- 그래서 **미배정 슬롯으로 전환**하고 경고를 반환한다. 미배정 슬롯은 지도의
  「장치 없는 자리」 뷰(`#btn-unbound-slots`)에 이미 뜨므로 사람이 발견할
  경로가 이미 있다.
- 종료 사유는 **`'replaced'`**. 후임이 없어도 끝난 원인은 교체다.
  `'unbound'` 로 적으면 사람이 직접 뗀 것과 구분되지 않는데, 그쪽이 더 큰
  정보 손실이다. 후임이 없다는 사실은 "그 슬롯에 현재 바인딩이 없다"가
  이미 말한다.
- 새 장치의 채널 목록은 `OutputChannel`/`InputChannel` 행이 정본이다.
  `expand_device()` 는 복합장치를 하위 장치로 펴는 함수라 여기에 쓰지 않는다.

### 2. 마커 충돌 → **거부하되, 어디가 충돌인지 알려준다**

교체 대상 장치가 같은 지도에 이미 위치 마커를 갖고 있는 경우.

- I2 유니크 인덱스(`geo_id, device_id, COALESCE(channel_id,'0')`,
  `type='aot_device'` 한정)가 DB 에서 이미 막는다. 게이트웨이가 먼저 막지
  않으면 IntegrityError 가 500 으로 새어 나온다.
- **옛 마커를 자동으로 지우지 않는다.** 그 마커는 측량된 좌표를 들고 있고,
  지우면 그 자리를 다시 그려야 한다 — 장치 삭제 정책을 "도형 존속"으로
  뒤집은 이유와 같다.
- **자동으로 옮기지도 않는다.** 두 좌표 중 무엇이 맞는지는 사람만 안다.
- 그래서 거부하고, 오류에 **충돌하는 마커의 이름과 지도**를 담는다. 거부만
  하고 어디가 걸렸는지 말하지 않으면 사람은 찾을 수 없다.
- 구역 폴리곤(`type='device'`)은 이 규칙 밖이다. 한 장치가 여러 구역을 맡는
  것은 정상이고(밸브 하나가 두 구역에 관수) I2 도 마커에만 걸린다.

### 3. 이름 동기화 → **현행 유지. 애초에 충돌이 없었다**

초안은 "슬롯 이름 보존"이었으나, 코드를 보니 전제가 틀렸다.

- `sync_device_name()` 이 덮어쓰는 것은 `properties.name` 이다.
...
- 즉 **사람이 지은 이름은 이미 보존되고 있고**, `name` 은 사람이 이름을 안
  ...
  오히려 거짓말이다.
- 결론: 마커·구역 양쪽 모두 `name` 은 계속 장치를 따라간다. 별도 작업 없음.
  `device_placement.sync_device_name()` 의 "미결" 주석은 지운다.

## 미결

1. **Phase A 마이그레이션 배포 시점**: 운영 3서버 토폴로지에서 백필 보고서
   확인 절차(업그레이드 전후 `check_geo_integrity` 실행 관례에 합류).

`MapDependency` 처분은 미결이 아니다 — 「선행 화석」 절에서 결론(파일 삭제,
별도 커밋)까지 정리했고 어느 단계의 게이팅 조건도 아니다.
**2026-09-20 처리 완료** — 모델 파일 삭제, 표는 `p6_71_drop_unread_tables_20260920` 이 지운다.
