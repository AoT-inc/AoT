...
3. **파생 가능한 링크는 저장하지 않는다.** `map_overlay_id`(장치→zone 소속)는 마커
   ...
   피처의 `bbox` 도 같다 — 좌표에서 언제든 다시 구할 수 있으므로 저장하지
   않는다(`geo_overlays._strip_derived_bbox` 가 저장 경계에서 떨군다). 이건
   가정이 아니라 겪은 일이다: `turf.lineSplit` 이 내부 rbush 색인의 검색 키로
   결과 피처에 `bbox` 를 붙여 돌려주는데(그것도 `turf.square()` 로 정사각형까지
   부풀린 값), 그 피처가 그대로 레이어의 정본이 되어 DB 까지 실려 갔다.
   2026-09-06 실측으로 지도 4곳의 `pipe_branch` 130건이 좌표와 어긋난 상자를
   달고 있었고, 위도 35도에서 도 단위 정사각형은 동서로 더 넓어지므로 최악은
   실제보다 75m 넓었다. 게다가 `turf.bbox()` 는 피처에 `bbox` 가 있으면 좌표를
   보지 않고 그 값을 돌려주므로, 한 번 저장되면 이후 모든 독자가 틀린 범위를
   받는다 — 파생값을 저장하면 정본과 갈라진다는 이 원칙의 교과서적 사례다.
   기존 데이터 정리는 `aot/scripts/fix_geo_feature_bbox.py`.
...
`unregistered-facility-shape`(시설로 등록되지 않은 `facility` 도형) 정리는
`aot/scripts/fix_geo_unregistered_facility_shapes.py` 다. **참조 확인을 손이
아니라 도구가 한다** — `GeoFacility.shape_uuid`·`GeoShape.parent_id`·7개
모델의 `map_overlay_id`·`geo_binding`·`geo_plot.facility_uuid`·라벨의
`parent_node_id` 를 훑어 하나라도 걸리면 그 도형은 지우지 않고 보고한다.
서버마다 상황이 다르므로 한 번 확인하고 끝낼 일이 아니다. 실물은 대체로
그리다 만 초안이지만(2026-09-06 김제 4건: 전부 2026-05 생성, 기본 이름
`New facility` 둘·이름 없음 하나·`type='facility'` 인데 이름이 `Main Pipe`
이고 `sub_type='pipe_main'` 인 것 하나), **지도에서 폴리곤이 사라지므로**
dry-run 의 이름·면적을 눈으로 확인하고 반영할 것.

⚠ 검사기의 `duplicate` 는 **같은 좌표를 중복으로 세지 않는다.** 장치를 매단
도형은 장치 신원(`device_id`, `channel_id`)까지 같아야 중복이다. 마커는 장치를
등록할 때 부모 구역의 중심점에 놓이므로(`place_device`), 여러 장치를 한꺼번에
등록하면 서로 다른 장치의 마커가 정확히 같은 좌표에 겹쳐 앉는 것이 정상이다.
좌표만 보던 시절 이 검사는 Japan 지도의 장치 15개(側窓·天窓·バルブ·ポンプ…)를
"중복 15건" 으로 보고하고 `keep_suggestion` 으로 하나만 남기라 권했다 —
그대로 따랐다면 멀쩡한 마커 14개가 지워졌다(2026-09-06). 마커의 진짜 중복
규칙은 I2 인덱스가 DB 에서 막는다.
...
판정의 **기준점은 `geo_hierarchy.containment_point()` 하나**다(2026-08-14 통일).
Point 는 자기 자신, 그 외는 `representative_point()`.

- **완전 포함(`contains(폴리곤)`)을 쓰지 말 것** — 경계를 조금 넘은 도형이 통째로
  빠져 소속이 한 단계 위로 밀린다(실측: 밸브 담당구역 10개가 자기 zone 을 못 찾고
  site 로 밀렸고, zone 하나는 부모를 아예 잃었다).
- **`centroid` 를 쓰지 말 것** — 오목한 폴리곤(ㄷ자 구역·도넛 필지)에서 중심점이
  자기 밖으로 나간다. `representative_point()` 는 내부에 있음이 보장된다.
- **대표점으로 바꿀 때는 "자기보다 작은 도형은 부모가 될 수 없다" 가드가 함께
  필요하다.** 완전 포함이 암묵적으로 막던 것이라, 기준만 바꾸면 자식이 부모로
  뒤집힌다(실측: site '2포장' 이 자기 안의 zone '2-2' 를 부모로 얻었다).
- **bbox 사전필터도 같이 본다.** 폴리곤에 "bbox 전체가 컨테이너 bbox 안" 을 요구하는
  필터가 남아 있으면, 기준을 바꿔도 경계를 넘은 도형이 판정까지 가지 못해 변경이
  조용히 무효가 된다.
