...
| 기존 문제 | 새 기준에서 |
|-----------|-------------|
| 빈 지도 91% 양산 | 원칙 4 — 생성 경로 자체가 없다 |
| 고아 지도 47~60% | 원칙 1·2 — 장치 전용 지도가 존재하지 않는다 |
| 판정 기준 4갈래 | 원칙 1 — 구분할 종류가 없다 |
| 위젯이 방금 만들어진 사설 지도로 끌려감 | 원칙 1 — 모든 지도가 동등 |
| 삭제 경로 비대칭·4종 누락 | 원칙 2 — 장치 삭제가 지도를 건드릴 이유가 없다 |
| 장치 삭제가 공유 지도를 파괴(임실군 62도형) | 원칙 1·2 — 소유하지 않으므로 지울 수도 없다 |
...
```
P1  자동 생성 중단      GET 쓰기 제거(5곳) + 장치 생성/수정 시 생성 중단
...
P5  컬럼 드롭          map_config_id + (앞서 사망 처리한) map_overlay_id
```
