...
### `@scoped` — 선택자에 묶인 커스텀 속성 12곳

`--aot-common-control-width` · `--aot-row-gap` · `--aot-viz-*` ·
`--active-theme-color` 는 `:root` 가 아니라 컴포넌트 선택자 안에서 선언된다.
검사기는 이것을 "정의가 여러 벌" 로 봤지만 **의도적 국소 오버라이드**다 —
`§5-7` 이 이미 그렇게 적어 두었다. 각 선언 자리에 `@scoped` 를 달아 그 의도가
파일 안에서도 읽히게 했다(첫 자리에는 이유까지, 나머지는 한 줄).

### `@kind other` — 색도 간격도 아닌 토큰

`--aot-fw-*`(4) · `--aot-lh-*`(2) · `--aot-z-*`(17) 는 색·간격 축이 아니라
숫자 사다리다. 세 섹션 머리말에 `@kind other` 를 달았다.

⚠ **이 표식의 문법은 아직 확인되지 않았다.** 디자인 쪽 에이전트의 보고에 나온
형태를 그대로 썼을 뿐, 검사기가 실제로 이렇게 읽는지는 다시 올려 봐야 안다.
읽지 못해도 주석이라 해가 없고, 다른 형태를 요구하면 그때 맞춘다.
(`--breakpoint-*` 는 부트스트랩이 선언하는 것이라 손대지 않았다.)
