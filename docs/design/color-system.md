...
## 5-9. 포커스 색 정리 — 노란 테두리·파란 링 (2026-09-07)

디자인 시스템을 claude.ai/design 에 올렸더니 저쪽 에이전트가 팔레트 밖의 색을
짚어 왔다. 확인해 보니 **입력칸 포커스 한 자리에 규칙 위반이 두 개** 겹쳐 있었다.

### 무엇이 틀렸나

| 토큰 | 예전 | 문제 |
|---|---|---|
| `--aot-input-focus-border` | `var(--aot-color-primary)` = `#F2D524` | 노랑은 버튼·액션 전용인데 입력 테두리가 정확히 그 색이었다 |
| `--modal-focus-ring` | `rgba(0, 123, 255, 0.1)` | 부트스트랩 파랑. 팔레트에 없다 |

포커스를 주면 **노란 테두리 + 파란 링**이 동시에 나왔다. 모달 입력칸
(`.aot-modern-input`)·설정 화면(`.aot-settings-select`)·공지 본문 텍스트영역이
전부 그랬다.
...
- `components/aot-drawer.css` — `/* Brand focus ring. NOT --aot-input-focus-border
  (that token = forbidden yellow). */` 라 적고 `--aot-color-brand-primary` 를 직접 썼다.
- 공지 화면 3개 템플릿 + `widget_notice.py` — "노란 테두리로 바꾸지 않으려고"
  테두리를 `--gray` 로 고정하고 링만 빌려 썼다(같은 주석이 네 벌).

**토큰이 규칙을 어기면 우회가 코드에 눌어붙는다.** 토큰 쪽을 고쳐 네 곳의
우회를 전부 지웠다.

### 지금

```
--aot-input-focus-border: var(--aot-color-brand-primary);   /* #13261B */
--aot-focus-ring:         rgba(19, 38, 27, 0.12);
```

이름도 `--modal-focus-ring` → `--aot-focus-ring` 으로 옮겼다(사용처 13곳).
모달 전용이 아니라 앱 전체의 포커스 링이다.

### 다크는 뒤집는다

딥그린(`#13261B`)은 어두운 입력칸(`#2a2a2a`) 위에서 **보이지 않는다.**
`custom-dark.css` 에 밝은 쪽 오버라이드를 넣었고, 값은 이미 다크 팔레트에 있는
보조 텍스트색을 그대로 쓴다(새 색을 만들지 않는다):

```
--aot-input-focus-border: var(--aot-color-text-secondary);  /* #D1D5D5 */
--aot-focus-ring:         rgba(209, 213, 213, 0.18);
```

드로어 composer 도 토큰을 쓰게 바꿨으므로 이 다크 오버라이드를 함께 받는다 —
직접 쓴 브랜드색은 다크에서 안 보였다.

**검증**: 프로브 페이지에서 네 컨트롤(모달 입력·설정 셀렉트·공지 텍스트영역·
드로어 composer)에 **실제 클릭·Tab 으로 포커스를 주고** 계산값을 읽었다.
라이트 8/8 · 다크 8/8 이 위 값으로 풀린다. `transition: border-color 0.2s` 가
있는 자리는 클릭 직후 읽으면 시작색이 나온다 — 전이가 끝난 뒤 읽어야 한다.

### 폴백도 정본으로 맞췄다

`§5-7` 의 "폴백 자리는 바꾸지 않는다"는 **토큰으로 치환하지 말라**는 뜻이지
아무 값이나 적어도 된다는 뜻이 아니다. `var(--aot-color-info, #007bff)` 처럼
정본(`#029ACF`)과 다른 색을 적은 자리가 9곳 있었고 값이 제각각이었다
(`#0d6efd` `#084298` `#2980b9` `#2a6aa8` `#004085` `#0070f3` …).

지금 화면에 그 색이 나오지는 않는다(토큰 파일이 늘 실린다). **렌더 버그가
아니라 문서 버그다** — 코드를 읽는 사람과 색을 훑는 도구에게 거짓말을 한다.
실제로 그렇게 새어 나가서 이 항목이 보고됐다. 그리고 토큰 이름을 바꾸는
순간(방금 `--modal-focus-ring` 이 그랬다) 폴백이 켜지면 그때는 렌더 버그가 된다.

`--aot-color-info` 9곳을 정본으로 맞췄고, 나머지 311곳은 토큰별 상한으로
묶었다(`test_var_fallback_matches_the_token`). 늘지만 않으면 된다.
`#fff` 와 `#ffffff` 는 같은 값으로 본다.

## 5-10. 테두리 정본 통일 · 죽은 팔레트 재선언 삭제 (2026-09-07)

### 같은 용도에 두 색이었다

모달 CSS 가 테두리를 부트스트랩 변수 `--gray` 로 그리고 있었다. 앱의 나머지
**147곳**은 `--aot-border-neutral` 을 쓴다. 값도 다르다 — `#eeebeb` vs `#dddddd`.

**다크에서는 결함이었다.** `--gray` 는 `custom-dark.css` 에 오버라이드가 없어
`#eeebeb` 로 남는다. 어두운 모달(`#1e1e1e`)·입력칸(`#2a2a2a`) 위에 거의 흰 선이
그어지고 있었다. `--aot-border-neutral` 은 다크에서 `#444444` 로 뒤집힌다.

테두리 **14곳**을 정본으로 옮겼다(모달 9 · 공지 템플릿 3 · 위젯 2).
...
`test_borders_do_not_use_the_bootstrap_gray` 가 재발을 막는다.

### 남긴 두 곳 — 테두리가 아니다

- `bootstrap-4-themes/aot.css` 드롭다운 선택 항목 **배경** — 라이트에서는
  옅은 회색 강조로 맞지만, 다크 드롭다운(`#2a2a2a`) 위에서는 흰 블록이 된다.
- `aot-modal-modern.css` 채널 **구분자 글자색** — 흰 바탕에서 거의 안 보인다.

둘 다 다크·라이트에서 어떤 값이어야 하는지가 **정리가 아니라 디자인 결정**이고,
`--aot-border-light`(`#f0f0f0`/`#2a2a2a`)를 쓰면 다크에서 드롭다운 배경과 같은
값이 되어 선택 표시가 사라진다. 맞는 토큰이 아직 없다. 그대로 두었다.

### 아무도 읽지 않던 팔레트 재선언 11개

`aot.css` 의 `:root` 가 부트스트랩 팔레트를 AoT 값으로 다시 잡고 있었다 —
`--blue` `--indigo` `--purple` `--pink` `--red` `--orange` `--yellow` `--green`
`--teal` `--cyan` `--white`. **읽는 곳이 저장소 전체에 한 곳도 없다**
(`var(--blue)` 류 0곳, JS 번들 포함 실측).

부트스트랩의 `:root` 팔레트는 값을 노출할 뿐 자기 규칙에서 `var()` 로 쓰지
않는다. 그래서 여기 다시 적는 것은 아무 일도 하지 않았다. 지웠다.

**A/B 로 확인한 것**: 지운 뒤 `--yellow` 가 `#f2bd1f` → 부트스트랩 기본
`#ffc107` 로 바뀌지만, 그 변화로 **계산값이 달라진 요소가 하나도 없다**.
읽는 곳이 없다는 정적 측정이 렌더에서도 그대로였다.

`--gray` 만 남겼다(위 두 곳이 아직 읽는다). `--gray-dark` `--primary`
`--secondary` `--success` `--warning` `--danger` `--light` `--dark` 는
소비처가 살아 있어 그대로다 — `§5-8` 의 "이름만 별칭이고 값이 다른 여덟 개".

**검증**: 모달 골격 프로브에 `git show HEAD:` 로 꺼낸 수정 전 CSS 를 옆에 두고
라이트·다크로 각각 새로 로드해 계산값을 비교했다. 바뀐 것은 e1~e9 의
**테두리색뿐**이고 배경·글자색·굵기는 그대로다. 이미 정본을 쓰던
`.aot-notice-box-plain`·`.aot-color-preset` 은 차이 0.

## 5-11. 팔레트 밖 파랑 · 라이트 테마 파일 · 안내 상자 대칭 (2026-09-07)

### 파랑 여섯 개를 info 계열로

팔레트에 없는 파랑이 토큰 정의에 남아 있었다. 전부 `--aot-color-info` 계열로 옮겼다.

| 토큰 | 전 | 후 |
|---|---|---|
| `--aot-note-fab-bg` | `#3b82f6` | `var(--aot-color-info)` |
| `--aot-note-search-focus` | `#3b82f6` | `var(--aot-color-info)` |
| `--aot-expand-toggle` | `#3b82f6` | `var(--aot-color-info)` |
| `--aot-expand-toggle-hover` | `#2563eb` | `var(--aot-color-info-hover)` |
| `--color-equipment-mode` | `#007bff` | `var(--aot-color-info)` |
| `--color-equipment-mode-hover` | `#0056b3` | `var(--aot-color-info-hover)` |

**`--aot-color-info-hover` 를 새로 뒀다.** 값은 짓지 않았다 — 라이트는 `#0287b8`
(이미 다크의 `--aot-color-info`), 다크는 `#029ACF`(라이트의 info). 즉 **두 값을
맞바꿔** 쓴다. 라이트에서 hover 는 한 단 어둡고, 다크에서 hover 는 한 단 밝다.
이 다크 오버라이드가 없으면 다크에서 info 와 hover 가 같은 값이 되어 hover
표시가 사라진다(처음 넣었을 때 실제로 그랬고, 계산값에서 잡혔다).

다크의 `--aot-note-fab-bg`(`#2563eb`) · `--aot-note-search-focus`(`#60a5fa`) 두
줄은 지웠다 — 정본이 `var(--aot-color-info)` 라 다크에서 저절로 `#0287b8` 로 풀린다.

### `custom-light.css` 는 선언이 23개였는데 22개가 군더더기였다

이 파일의 자기 규칙은 "**라이트 기본값과 다른 것만** 재정의한다" 이다. 실제로는
23개를 선언하고 있었고, 그중 **22개가 `aot-theme-variables.css` 와 글자까지 같았다**
(노트 토큰 14개 · 표면 4 · 테두리 3 · 모달 backdrop 1).

나머지 하나 `--aot-surface-body` 는 같아야 하는데 **달랐고, 그것이 결함이었다**:

```
aot-theme-variables.css   #F3F6F5   ← 정본. theme_defaults.json ·
                                      forms_settings.py · custom_ui.html 도 이 값
custom-light.css          #f8f9fa   ← 나중에 실려 이겼다
```

`/custom.css` 는 `bd_secondary`(기본 `#F3F6F5`)를 `--aot-surface-body` 로 발행하고
**가장 마지막에 실린다.** 그래서 **설정을 한 번도 저장하지 않은 사용자만**
`#f8f9fa` 를 보고, 설정 화면을 열었다 저장한 사용자는 `#F3F6F5` 를 봤다.
같은 기본 설정인데 화면이 갈렸다. `§3` 의 "기본값 4곳 일치" 계약이 이 토큰에서
깨져 있었던 것이다.

파일을 비우고 정본으로 모았다. **파일은 지우지 않았다** — `layout.html` 이 라이트
테마에서 싣고, 라이트에만 필요한 값이 생기면 갈 곳이 있어야 한다.

### 안내 상자 — 기본형에 톤이 없다

`.aot-notice-box` 는 **기본형이 곧 경고 톤**이라 `-warning` 이라는 이름이 아예
없었다. 변형을 안 붙이면 조용히 노란 경고 상자가 나왔고, `admin/upgrade.html`
에는 "modifier 없이도 경고 톤으로 나온다" 는 주석까지 남아 있었다 — 사고가
아니라 **관습으로 굳은 함정**이었다.

    전:  기본형 = 경고 톤,  변형 -success -danger -info -plain
    후:  기본형 = 톤 없음,  톤 -success -warning -danger -info

- `-warning` 신설. 기본형만 쓰던 호출부 **7곳**에 명시적으로 붙였다.
- `-plain` 삭제 — 기본형이 곧 그것이 됐다. 유일한 호출부(폼을 감싸는 상자)는
  변형을 뗐다.
- 그림자는 **톤이 있는 상자만** 진다. 기본형은 폼을 감싸는 테두리라 뜨지 않는다.

계약 테스트도 표현을 바꿨다. 예전에는 `.aot-notice-box {` 부터 `-plain` 까지를
**한 덩어리로 잘라** 그 안에 네 tint 토큰이 다 있는지 봤는데, 기준점이던 `-plain`
이 없어졌다. 지금은 **톤마다 자기 규칙을 직접 확인한다** — 한 톤의 선택자가 여러
규칙에 나올 수 있으므로(그림자 묶음 규칙) 첫 규칙을 집지 않고 tint 를 담은 규칙을
찾는다. `test_notice_box_base_has_no_tone` 이 기본형의 무톤을 지킨다. 둘 다 일부러
어겨서 잡히는 것을 확인했다.

### ⚠ 여기서 드러난 것 — 틴트에는 다크 값이 없다

`--aot-tint-*-bg/fg/border` 열두 개는 `custom-dark.css` 에 재정의가 없다(그 파일
주석도 그렇게 적고 있다). 그래서 다크에서 안내 상자·상태 배지·틴트 칩이 **밝은
파스텔 그대로** 어두운 배경 위에 앉는다. 이번 작업이 만든 것이 아니라 원래
그랬고, 기본형이 무톤이 되면서 오히려 눈에 덜 띈다. 열두 값의 다크 판을 정하는
것은 별건의 디자인 결정이다.

## 5-12. 디자인 도구가 읽을 수 있게 — 표식 두 종류 (2026-09-07)

claude.ai/design 의 검사기가 매번 보고하던 형식 항목 둘을 정리했다. 화면에는
영향이 없고, **저쪽이 토큰을 제대로 분류하지 못하던 것**이다.

처음 시도는 **둘 다 틀렸고**, 다시 올려서 그것을 알았다. 아래는 정정 후다.

### 선택자에 묶인 커스텀 속성 12곳 — **고치지 않는 것이 맞다**

`--aot-common-control-width` · `--aot-row-gap` · `--aot-viz-*` ·
`--active-theme-color` 는 `:root` 가 아니라 컴포넌트 선택자 안에서 선언된다.
검사기는 이것을 "정의가 여러 벌" 로 본다. 처음에는 `@scoped` 라는 표식을 달아
의도를 알리려 했는데, **그런 억제 주석 문법은 존재하지 않는다.**

그렇다고 `:root` 로 올릴 수도 없다 — `:has()` 로 행 모양을 갈라 폭을 정하는
설계가 통째로 깨진다(`§5-7`). **이 보고는 영구적으로 남는다.** 표식 대신
왜 그대로 두는지를 주석에 적었다. 검사기가 앞으로도 짚을 자리라는 것까지.

### `@kind other` — **선언마다** 후행 주석이어야 한다

`--aot-fw-*`(4) · `--aot-lh-*`(2) · `--aot-ls-*`(1) · `--aot-z-*`(17) 는
색·간격 축이 아니라 숫자 사다리다. 처음에는 세 **섹션 머리말**에 한 번씩
`@kind other` 를 적었는데, 검사기는 **선언별 후행 주석만 읽는다** — 그대로
다시 보고받았다. 24줄에 각각 붙였다:

```css
--aot-fw-regular:  400;  /* @kind other */
--aot-z-base:        0;  /* @kind other · 기본 흐름 콘텐츠 */
```

이미 설명 주석이 있던 z-index 17줄은 처음에 `/* @kind other · 기본 흐름 콘텐츠 */`
로 합쳐 적었는데 **그것도 안 읽혔다**(세 번째다). 검사기는 `@kind` 뒤에 **라벨
하나만** 온다고 본다 — 설명이 붙으면 라벨로 인식되지 않는다. 주석을 둘로 가른다:

```css
--aot-z-base: 0;  /* @kind other */ /* 기본 흐름 콘텐츠 */
```

`--breakpoint-*` 는 **지웠다.** 부트스트랩 것이라 손댈 수 없다고 적었었는데
다시 보니 `bootstrap-4-themes/aot.css` 가 다섯 개를 **재선언**하고 있었다 —
값은 부트스트랩 기본과 똑같고(0·576·768·992·1200) 읽는 곳은 저장소에 한 곳도
없다. `§5-10` 에서 지운 팔레트 11개와 같은 계열이라 같은 이유로 지웠다.
표식을 붙일 게 아니라 없앨 선언이었다. (벤더 `bootstrap.min.css` 안의 원본은
그대로 둔다 — 받아 온 파일이다.)

`--aot-preview-band-h`(`aot-modal-modern.css`, 위젯 드로어의 미리보기 띠
높이)도 `/* @kind spacing */` 을 받았다. 한 줄에 `42vh`·`42dvh` 두 선언이
붙어 있어 표식이 어느 쪽에 붙는지 모호했으므로 **두 줄로 갈랐다** — 순서가
곧 폴백이라(`dvh` 를 모르는 브라우저가 둘째 줄을 버린다) 값과 순서는 그대로다.

### 끝내 남는 한 건 — `--breakpoint-xs`

이제 출처가 `vendor/bootstrap.min.css` 의 `:root` 뿐이다. **받아 온 파일이라
주석을 달 자리가 없다.** 값이 `0` 이라 `spacing` 으로 자동 분류되지도 않는다
(`sm|md|lg|xl` 은 px 값이라 이미 분류된다). 스캔에서 `vendor/` 를 빼거나
알려진 예외로 두는 것이 맞고, **우리 쪽에서 할 일은 없다.** 다시 조사하지 말 것.
