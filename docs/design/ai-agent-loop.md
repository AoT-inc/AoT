...
- `manual_reference`: **서버가 결정적으로 조회한 지식 근거**(설계
  ai-library-redesign.md §6). 아래 3.2 참조 — 이것만 컨텍스트 **맨 앞**에 놓는다.

### 3.2 지식 접지 계약 (`manual_reference`)

@ANCHOR: AGENT_LOOP_KNOWLEDGE_GROUNDING / AGENT_LOOP_UNCONFIRMED_DISCLOSURE

**루프는 지식 검색을 모델의 재량에 맡기지 않는다.** `run()` 이 턴 시작에
`AIAgentService._manual_grounding(command_text)` 을 **한 번** 호출하고, 결과를
매 스텝 컨텍스트 맨 앞에 싣는다. 인용 규약(`_MANUAL_GROUNDING_DIRECTIVE`)은 스텝
프롬프트에 붙고, 최종 답변은 `_finish` 에서 `_enforce_unconfirmed_disclosure` 를
지난다.

지켜야 할 세 가지 — 셋 중 하나만 어긋나도 계약이 조용히 깨진다:

1. **조회는 1회, 적재는 매 스텝.** 검색은 결정적(LLM 비사용)이라 재조회할 이유가
   없다. 반대로 근거를 0스텝에만 실으면, 모델이 3스텝째에 답하기로 정했을 때
   근거 없이 답한다.
2. **컨텍스트 맨 앞.** `base_ai` 는 컨텍스트를 삽입 순서로 직렬화하고 티어 예산에서
   프롬프트 **꼬리를 하드 절단**한다.

   정정(2026-08-24) — 처음 이 항목을 쓸 때 "이 설치는 이미 절단되고 있다" 고
   적었는데 **틀렸다.** `GeminiAI` 가 `get_context_budget()` 을 오버라이드해
   standard 를 600,000자로 잡고, 실측 프롬프트는 197,267자라 지금은 잘리지 않는다
   (절단 경고 로그 0건이 그 증거였다).

   **수정(2026-08-24)**: 그 다섯 엔진의 예산을 고쳤다 — `base_ai` 기본 standard
   를 100,000 → 300,000자로 올리고, 진짜 창이 작은 엔진만 따로 낮췄다
   (`ollama` 24,000 — 로컬 `num_ctx` 가 실제 상한이고 넘기면 Ollama 가
   **앞에서부터** 자른다; `openai_compatible` 100,000 — 어느 엔드포인트인지
   알 수 없다). `minimax` 는 200,000 이라 실측 프롬프트와 여유가 2,700자뿐이어서
   300,000 으로 올렸다. 계약은 `aot/tests/test_engine_context_budgets.py` 가
   고정한다.

   **나머지 절반도 수정(2026-08-24)**: 예산이 맞아도 `groq`(TPM 한도)·`ollama`·
   `openai_compatible` 처럼 **작은 것이 맞는** 엔진에서는 절단이 일어나고, 그때
   없어지는 것이 꼬리 — `SYSTEM INSTRUCTIONS` — 라는 구조 문제가 남아 있었다.
   `base_ai._fit_context_to_budget`(@ANCHOR: PROMPT_BUDGET_FIT)가 이제 들어갈
   자리를 먼저 계산해 **컨텍스트만** 줄인다.

   - **순서는 바꾸지 않았다.** 지시를 앞으로 옮기면 절단이 없는 요청까지 모든
     엔진의 프롬프트가 바뀐다(지시를 뒤에 두는 것은 최신성을 노린 배치이기도
     하다). 자리를 계산해 컨텍스트만 줄이므로 **예산 안에 드는 요청은 바이트
     단위로 종전과 같다** — 이것이 이 방식을 고른 이유이고, 테스트의 첫 항목이다.
   - 문자열을 중간에서 자르지 않고 **큰 최상위 키를 통째로** 뺀다. 잘린 JSON 은
     파싱이 안 되고, 잘린 도구 목록은 모델에게 "도구가 이것뿐" 으로 보인다.
   - 뺀 것은 `_omitted` 로 **이름을 말한다.** 빠진 줄 모르는 모델은 "그런 장치는
     없습니다" 라고 단정한다 — 없는 것과 못 본 것을 구분 못 하는 답이 가장 나쁘다.
   - 요청(`user_command`)과 그 근거(`manual_reference`)는 절대 버리지 않는다.
   - 응답형식 꼬리를 상수(`_RESPONSE_FORMAT_TRAILER`)로 뽑아 자리 계산에 포함했다.
     이 꼬리는 예산 검사 **뒤**에 붙어서, 예전에는 "예산 안" 으로 판정된 프롬프트가
     실제로는 1,673자만큼 예산을 넘겨 나가고 있었다(경고도 안 뜨는 초과였다).
   - 실측 바닥값: 컨텍스트를 완전히 비워도 프롬프트에 **17,645자**가 남는다.
     그보다 작은 예산은 줄일 컨텍스트가 없어 지시부터 자르게 되므로, 그 아래
     값은 '작게 잡았다' 가 아니라 '작동하지 않는다' 는 뜻이다 — `ollama` 의
     standard 를 그 실측에 맞춰 24,000 → 32,000 으로 고쳤다.

   계약은 `aot/tests/test_prompt_budget_fitting.py` 가 고정한다.

   그래도 앞 배치가 옳은 이유는 남는다: 예산은 **엔진마다 다르고** 오버라이드가
   없는 엔진(anthropic·openai·mistral·ollama·openai_compatible)은 base_ai 기본
   100,000자를 쓴다. 그 엔진으로 바꾸면 같은 프롬프트에서 97,767자가 잘려
   `SYSTEM INSTRUCTIONS` 블록과 도구 카탈로그 뒷부분이 사라진다 — 실측으로
   확인했다. 즉 이 배치는 "지금 잘리니까" 가 아니라 **"모델 불가지 시스템에서
   예산은 사용자가 고르는 엔진에 달렸으니까"** 앞에 둔다.
3. **고지는 `_finish` 한 곳에서.** 종료 경로가 여섯 개다. 각 `return` 마다 걸면
   다음에 추가되는 경로가 조용히 빠진다.

이 계약은 **한 번 깨진 적이 있다.** P2/P6 이 fast path·collab 에 구현한 두 장치가
루프 이관 때 옮겨지지 않아, 유닛테스트 65건이 통과하는 채로 서비스에서만 지식이
닿지 않았다(2026-08-24 발견·복구). 그래서 회귀 방어를
`aot/tests/test_agent_loop_knowledge_grounding.py` 에 두었다 — 그 파일이 재는 것은
`knowledge_search` 의 동작이 아니라 **루프가 그것을 부르고 결과를 어디에 싣는가** 다.
