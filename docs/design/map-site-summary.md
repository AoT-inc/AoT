...
**시설도 같이 얹었다 (2026-08-10).** 위 세 겹을 넣을 때 시설 모달(`/overview`)만
빠져 있었다 — 결과적으로 넷 중 가장 느렸다(실측 **534~641ms, 열 때마다**;
같은 시설의 `/runtime` 은 17ms).

| | 조치 | 실측 |
|---|---|---|
| 서버 | `cached_build` 30초 + 단일 비행 | 305ms(콜드) → **6~9ms** |
| 클라이언트 | `modalFetch('facility', …)` | 호버 1건 → 열기 0건 → 다시 열기 0건 |
| 예열 | `facilities-fill` 호버 + **bay 칩 호버** | — |

- **bay 칩 호버가 핵심이다.** 지도에서 시설 모달을 여는 실제 과녁은 도형이
  아니라 칩이고, 칩은 DOM 이라 캔버스 mousemove 가 안 온다. 도형 호버만 걸면
  칩 위로 곧장 간 사용자는 예열을 못 받는다.
- **`?fresh=1` 우회가 반드시 필요하다.** 사진 교체·설명 저장·자동제어 토글
  직후의 재조회(`_loadOverview(uid, uuid, true)`)는 캐시를 타면 안 된다 —
  방금 끈 자동제어가 켜진 채로 보이면 토글이 고장 난 것으로 읽힌다.
  클라이언트 캐시(`invalidateModal`)와 서버 캐시(`force`) **둘 다** 넘어야 한다.
  실증(2026-08-10): 캐시를 데운 뒤 자동제어를 끄고 바로 읽으면 **캐시 경로는
  `function_active: true` 를 그대로 내놓고**(거짓말) `?fresh=1` 만 `false` 를
  냈다. 우회가 장식이 아니라는 증거다.
- **상태를 바꾼 쪽이 캐시를 버린다.** `?fresh=1` 은 *누른 사람의 창*만 구한다 —
  다른 탭·다른 사람·다른 경로(AI 도구·스케줄러)로 바뀌면 그 우회가 없어서
  열려 있는 모달이 최대 30초 동안 옛 운전 상태를 내걸었다. 그래서
  `/function_state` 가 `invalidate_facility_overview()` 를 부른다. 넣은 뒤에는
  **캐시 경로만으로도** 켜짐→꺼짐→켜짐이 즉시 따라온다.

> **검증 함정**: 자동제어를 **끄는** 조작에는 `window.confirm` 확인창이 붙어
> 있다. 자동화로 토글을 눌러도 확인창이 취소되어 `function_state` 가 아예
> 나가지 않는다 — 코드가 안 도는 것으로 오진하기 쉽다(실제로 그랬다).
- 시설 목록(`/api/geo/facility/list`)이 `rep_key` 를 함께 실어, 모달을 한 번도
  안 열어도 지도 칩이 지정된 대표를 낸다.
