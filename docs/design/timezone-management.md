...
## 14. MCP 표면의 시간 — 외부 AI에게는 시계가 없다 (2026-09-16)

§7 의 표시 규칙은 **사람이 보는 화면**을 전제한다. 사람은 지금이 몇 시인지 알고 화면을
보므로, 응답이 "지금"을 말해 주지 않아도 문제가 없다. MCP 로 붙은 외부 AI 는 그렇지 않다 —
대개 날짜까지만 알고 현재 시각은 모른 채, **응답 안에서 시각처럼 보이는 값을 "지금" 으로
집어 든다.**

**실제 사고(2026-09-16).** 외부 AI 가 `get_system_brief` 를 부르고 "지금은 새벽이니 곧
05:30 자동 관수가 시작된다" 고 답했다. 실제 시각은 한낮 14:10 이었다. 브리핑 안에서 시각처럼
보이는 필드가 `anomalies.evaluated_at` 하나뿐이었고 그것이 UTC(05:10) 였다. 같은 서버의 센서
도구는 그때도 `+09:00` 을 정확히 내고 있었으므로 **데이터는 맞고 기준선이 없었던 것**이다.

### 14.1 규약

1. **"지금" 은 데이터가 아니라 봉투에 싣는다.** 모든 도구 응답에 `now = {farm_local, tz}`
   (농장 기본 시간대의 현재 시각)를 붙인다. 붙이는 자리는 `tool_execution._farm_now()` —
   stdio/HTTP 양쪽이 반드시 지나는 단일 지점이라, 서랍 안에 있는 도구까지 예외 없이 달고
   나간다. 도구마다 넣게 하면 반드시 빠지는 도구가 생긴다.
2. **응답 안의 모든 시각은 오프셋을 달고 나간다.** naive ISO 는 읽는 쪽이 자기 시간대로
   읽으므로 한국에서는 9시간 어긋난다. 저장은 UTC-naive 규약 그대로 두고 **내보낼 때만**
   변환한다(`serialize_ts`, 문자열로 이미 굳은 값은 `_local_iso`).
3. **UTC 로 내보내지 않는다.** 오프셋이 붙어 있어도 `+00:00` 은 같은 응답 묶음 안의 센서값
   (장치 현지)과 기준이 갈려, 읽는 쪽이 둘을 나란히 놓는 순간 9시간을 침묵이나 지연으로
   오해한다. 운영 문맥은 장치 현지, 그 밖은 농장 기본 시간대.
4. **서버 instructions 에 적는다.** 도구 설명에 흩어 놓으면 그 도구를 안 부른 AI 는 못 본다.
   `_server_instructions()` 의 TIME 절이 ①`now` 가 유일한 현재 시각이고 ②데이터의 시각은
   과거 사건이며 ③위치별 시각은 `get_local_time` 이라는 세 가지를 못박는다.

### 14.2 이때 함께 고친 것

| 지점 | 전 | 후 |
|---|---|---|
| `get_anomalies.evaluated_at`(→`get_system_brief`) | UTC `+00:00` | 농장 현지 |
| `get_device_freshness.last_seen` | UTC `+00:00` | 장치 현지 |
| `list_advice.created_at`/`reviewed_at` | **오프셋 없음**(naive) | 농장 현지 |
| `get_weather_forecast.published_at`/`age_hours` | KST 벽시계를 UTC 시계와 뺌 → **age 9시간 오차** | 오프셋 부여 후 비교 |

`age_hours` 는 표시가 아니라 **stale 판정의 입력**이었다 — 발표 30분 된 예보가 "8.5시간 전"
으로 보였다. 시간대 결함이 표시에만 머물지 않는다는 실례다.

### 14.3 낡은 값은 "낡았다고 말할 값" 이 아니라 없는 값이다

같은 날 이어서 드러난 문제. 예보를 쓰지 않는 시스템인데도 AI 가 대화마다 "기상예보가
오래되었다" 를 보고했다. 세 가지가 겹쳐 있었다.

1. `aot/aot_flask/static/json/forecast.json` 이 **git 에 추적되고 있었다.** 런타임 생성물이
   초기 커밋에 섞여, 예보 Input 을 쓰지 않는 설치까지 2026-02-13 발표분을 받아 들고
   시작했다 — 앱에서 예보 장치를 지워도 pull 하면 되살아났다. 추적 해제 + `.gitignore`.
2. `get_weather_forecast` 가 그 파일을 `status:"success"` 로 냈다. 발표 215일 지난 값에
   `stale:true` 와 경고를 붙여서. **경고를 붙였기 때문에 오히려** 받는 쪽은 그것을 보고할
   사실로 다뤘다. 24시간(`_FORECAST_UNUSABLE_H`)을 넘겼거나 미래 시각이 하나도 없으면
   내용을 싣지 않고 `unavailable` 로 답한다.
3. `get_system_brief` 의 `how_to_proceed` 가 매번 "Forecast: get_weather_forecast (always
   check the 'stale' flag)" 로 **호출을 지시**했다. 사용자가 하지 말라고 해도 다음 대화의
   브리핑이 다시 시켰다. 이제 예보가 살아 있을 때만(`_forecast_is_usable()`) 안내한다.

교훈은 §14 전체와 같다. **상시 지시는 그 대상이 살아 있을 때만 싣는다**, 그리고 쓸 수 없는
값은 경고를 붙여 넘기지 말고 없는 것으로 답한다 — 경고는 읽는 쪽에게 "보고할 거리" 로 보인다.

---

## 15. 인지 판정 — 시스템·사용자·장치가 모두 다른 시간대일 때 (2026-09-21)

합격 기준: **시스템과 사용자 모두 같은 순간을 정확히 인지한다.** 세 조건으로 푼다.
① 저장은 UTC 절대순간 하나, ② 보이는 시각에는 어느 시간대의 시각인지가 붙는다,
③ 입력된 벽시계는 그 시각의 주인(장치 예약=장치, 사람의 기록=그 사람)의 시간대로 해석한다.

판정 테스트 `aot/tests/test_timezone_perception.py` 가 이 기준을 코드로 적는다(시스템 +6,
사용자 +9·+11, 장치 +3, 위치 없는 장치). 못 미치는 항목은 `xfail(strict=True)` 로 두어
고치는 순간 실패로 바뀌게 했다 — 남은 표시 수가 남은 구멍 수다.

### 15.1 1단계 — 저장할 때마다 밀리던 곳

| 지점 | 원인 | 수정 |
|---|---|---|
| 노트·공지 편집 칸 | UTC 원값을 채우고 시스템 tz 로 해석해 저장 | 채우기·저장이 같은 함수(`utils_notes._wall_tz`/`utc_to_wall`) |
| 캘린더 위젯 드래그·편집 | 보는 사람 시계의 벽시계를 서버가 장치 시계로 재해석, 편집 창은 매번 시각 전송 | 절대순간(`toISOString()`) 전송, 바뀐 경우에만 |
| 지도 출력 예약 휠 | 같은 원인 — 고른 시각이 시차만큼 늦거나 이르게 실행 | 절대순간 전송 |

서버는 `timekit.instant_or_wall_to_utc` 로 받는다: **오프셋이 있으면 절대순간, 없으면 장치
벽시계.** 장치 시각으로 입력하는 화면(스케줄러 새 작업)은 그대로다. `wall_to_utc` 는 오프셋을
떼는 계약이라 건드리지 않았다.

참고: 위치 없는 장치에 복사된 `'UTC'` 는 좌표 리스너(`_on_insert`)가 저장 순간 지워 이미
시스템 tz 를 따라간다. 고정되는 것은 `'UTC'` 가 아닌 시스템 값을 복사한 행이다.

### 15.2 2단계 — 순간값이 틀리던 곳

| 지점 | 원인 | 수정 |
|---|---|---|
| 타임라인 가동 막대(`device_helpers.get_device_runtime`) | 컨테이너 지역시각의 naive ISO → 브라우저가 자기 시간대로 읽음 | UTC+오프셋 |
| 노트↔일정 링크 `when`, 조언 위젯·지도 조언 칩 `timestamp` | naive ISO(시각·"N시간 전" 모두 틀림) | `iso_utc` |
| 측정값 내보내기·에너지 사용량 구간 | 칸은 시스템 시각으로 채우고 해석은 `time.mktime`(컨테이너 OS 시계) | `timekit.picker_wall_to_epoch`(채운 시계로 해석) |
| AI "지금 몇 시" 답변 | 라벨 없는 시스템 시각 | 묻는 사람의 시계 + 라벨(`timekit.format_labeled`) |
| 알림 이메일 시각 | 컨테이너 OS 시계, 라벨 없음 | 시스템 시계 + 라벨 |
| 일지 표지 '생성' | UTC 원값 옆에 대상 시간대 이름 | 그 시간대로 변환 |
| 스케줄러 새 작업(대상 없음) 미리보기 | 미리보기는 보는 사람 시계, 서버는 시스템 시계로 해석 | 미리보기도 시스템 시계 |

`format_labeled` 는 '2026-09-21 09:00:00 (Asia/Seoul, UTC+09:00)' 형태 — 서로 다른 시간대의
사람이 같은 글을 읽는 자리(대화·메일)에서 쓴다.

### 15.3 3단계 — 사람이 적는 시각은 쓴 사람의 시계, 읽는 시각은 보는 사람의 시계 + 라벨

"보는(쓰는) 사람의 시계" = `timekit.current_user_tz()`(User.timezone, 없으면 시스템).

| 지점 | 전 | 후 |
|---|---|---|
| 노트·공지 입력(추가·편집·위젯 작성) | 시스템 시계로 해석, 라벨 없음 | 쓴 사람의 시계(`utils_notes._wall_tz`), 칸 아래 `시간대: Asia/Seoul (UTC+09:00)` |
| 노트·공지 목록, 감사로그 | 시스템 시계, 라벨 없음 | 보는 사람의 시계 + `(UTC+09:00)` (`utc_to_local_time`) |
| 편집 칸을 채우는 값(`data-date`, 공지 JSON `publish_at`/`expire_at`) | 목록 표시값 재사용 | 라벨 없는 벽시계(`utc_to_wall_str`) — 라벨이 붙으면 다시 파싱되지 않는다 |
| 노트 보고서(PDF) | 기간은 시스템 시계, 경계 변환과 어긋남 | 만드는 사람의 시계 하나, '생성' 줄에 라벨 |
| 노트 CSV | 시스템 시계, 오프셋 없음 | 내보내기에 오프셋, 가져오기는 오프셋을 따르고 없으면(옛 파일) 시스템 시계 |
| 노트 구간 → 예정 | 기본값을 브라우저 시계로 계산 | `GET /notes/<id>/schedule` 이 저장과 같은 시계(장소, 없으면 시스템)와 그 시계의 다음 정시를 준다, 칸 아래 라벨 |
| 데몬·앱 로그 | 시스템 시계, 오프셋 없음 | `logging_setup.TzLogFormatter` — `2026-09-21 06:00:00,123+06:00`, 로그 화면 상태줄에 `시간대: UTC+06:00` |

라벨 문구는 기존 번역 `Time zone` 만 쓴다(23개 언어 모두 있음) — 새 문구 없음.

**운영 메모:** 로그 줄 형식이 바뀌었다(시각 끝 오프셋). `log_reader._RE_AOT` 는 두 형식을 모두 읽는다.

### 15.4 4단계 — 위치 없는 장치는 '시스템 추정' 이라고 적고, 시스템 시간대를 따른다

| 지점 | 전 | 후 |
|---|---|---|
| 장치 생성(출력·입력·함수 5곳)·복제 | 시스템 시간대를 복사하고 출처를 비움(복제는 `'inherited'`) → `resolve_tz` 가 `'explicit'` 로 봄 | `device_tz.apply_system_tz_fallback` — 출처 `'system'` |
| 좌표 리스너 `_on_insert` | `'UTC'` 복사본을 지움 | `'system'` 복사본은 그대로 둔다(저장 때 따라가야 하므로) |
| 설정 → 일반 저장 | 값이 바뀔 때만 `Misc.timezone` 갱신 | 값이 같아도 `device_tz.sync_system_tz_copies` — `'system'` 행만 새 값, 감사로그에 수 |
| 예약 앵커 출처 | 위치를 몰라도 `'device'` | `device_tz.resolve_location_tz_and_source` — 모르면 `'system'` |
| 스케줄러 목록·새 작업 이중시계 | "장치 현지" | 추정이면 "위치 미지정 — 시스템 시간대로 추정" |
| `GET /api/timezone/device/<id>` | 값이 있으면 `'explicit'` | `resolve_tz` 의 출처 그대로 |

**이행(p6_72, 값은 그대로 — 결정 (가)):** 출처 없음+값 있음+좌표 없음 → `'system'`, 좌표 없고 어떤 도형에도
연결 안 된 `'inherited'` → `'system'`, 출처 없음+좌표 있음 → `'coords'`, 값 없는 `'coords'` → 출처 비움.
로컬 복사본 실측: 7개 표 112행 분류, 그중 `'system'` 59행. 이 행들은 관리자가 시스템 시간대를 저장할 때 새 값을
따른다. 값을 이행 때 바꾸지 않은 이유: 운영자가 옛 시계에 맞춰 넣어 둔 예약이 옮겨질 수 있다.

**남은 한계:** 이미 돌고 있는 컨트롤러(예: `controller_trigger_sequence` 는 시작 때 `device_tz` 를 읽어 둔다)는
다시 켜기 전까지 옛 시간대로 동작한다.

### 15.5 현지 시간 원칙과 5단계 A — 값이 틀리던 곳, 장소 노트

**원칙(2026-09-21 재확인):** 장치·장소에 관한 시각은 **그 장소의 현지 시각**으로 표시한다(§7 운영 문맥).
장치가 +9 면 +9 로 보인다. 사람은 지도에서 그 위치를 보며 그곳의 시각을 읽는다. 장소와 무관한 것
(감사로그, 로그, 장소 없는 노트·공지)만 보는 사람의 시계다. 3단계의 노트 규칙도 이에 맞춰, **장소에 붙은
노트는 그 장소의 시계**로 입력받고 보인다(`utils_notes._wall_tz(target_id)`, 목록·편집 칸·라벨 모두 노트마다).

| 지점 | 원인 | 수정 |
|---|---|---|
| 기상예보 위젯 | 예보 `now`(장치 벽시계)와 브라우저 벽시계를 빼서 시차만큼 다른 예보를 '현재' 로 | `forecast.json` 에 `tz`, 위젯은 그 시계로 '지금'·낮밤·제목 |
| 일지 종료일 기본값 | `toISOString()` UTC 날짜 | `target_info` 가 그 대상의 `today` 를 준다 |
| 구획 시작일 기본값(5개 화면) | 브라우저 날짜 | 공용 폼 `AoTPlotForm.wire` 가 `ctx.placeId` 로 `/api/timezone/today` 를 물어 손대지 않은 칸을 바로잡는다 |
| 지식 라이브러리 추가일 | naive ISO | `iso_utc` |
| 지도 '다가오는 일정' 정렬 | 오프셋이 다른 ISO 를 문자열 정렬 | 순간으로 정렬 |

`resolve_location_tz_and_source` 가 시설(GeoFacility)과 지도 uuid 도 푼다 — 전에는 시설에 건 일정·구획의
시계가 시스템 시간대로 떨어졌다. `AoTTz.todayIn(tz)`·`AoTTz.placeToday(id)` 신설.

**남은 것(보고됨, 미착수):** 서버의 `date.today()`(컨테이너 날짜) 사용 15곳 이상 — 구획 생육일수·단계 판정
(`plot_context`·`plot_accumulation`·`plot_io`·`coordinator_plot`). 같은 원칙으로 그 구획의 현지 날짜여야 한다.
