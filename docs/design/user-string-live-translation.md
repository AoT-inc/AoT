...
`is_enabled()` 는 **표시**만 본다 — 사전에 있는 번역을 화면에 쓸 것인가.
`can_auto_translate()` 만 AI 를 요구한다 — 엔진을 불러 새 번역을 만들 것인가.

처음에는 둘을 `ai_enabled` 하나에 걸었는데, 그러면 **AI 없는 설치에서 관리
화면에 번역을 입력해도 사전이 빈 채로 나간다** — 저장은 되는데 화면은 그대로라,
사용자에게는 고장으로 보인다. 번역본을 보여주는 데에 LLM 은 필요 없다.

AI 를 쓰지 않는 설치에서도 이렇게 쓸 수 있다: 이름 수집(`sync_sources`)은 DB 만
읽으므로 그대로 돌고, 관리 화면에 목록이 올라오면 자기가 쓰는 언어만 손으로
채운다. 22개 언어를 다 채울 일은 없다.
