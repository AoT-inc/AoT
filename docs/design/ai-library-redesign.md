...
클라이언트와 조회 계층까지다. **프리셋 등록(`ext_openmeteo`)이 아직이라 화면에서
켤 수 없고, 따라서 AI 도 아직 못 쓴다.** 이어서 할 것:

- `routes_ai_library.py` 에 `ext_openmeteo` 프리셋 — `needs_api_key: False`,
  좌표 기본값은 `Misc.map_latitude/map_longitude` 에서 채운다(모델이 매번
  위경도를 적지 않아도 되게).
- `get_weather_forecast` 의 `unavailable` 응답이 이 소스를 가리키게 한다.
  지금은 한국 밖에서 "예보 없음" 으로 끝나므로, 대안이 생겨도 모델이 못 찾는다.
