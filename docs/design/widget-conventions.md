...
페이지 CSS 와 AoT 테마 파일은 `aot/tests/test_css_conventions.py` 가 같은
규칙 함수로 본다(2026-09-06 확장, 상한을 두고 내려가는 방식).
...
**타일·카드 모서리도 사다리가 생겼다** (2026-09-07). 위젯 CSS 에 8·12·6·5·4px
가 흩어져 있던 것을 앱 전체와 함께 다섯 단으로 모았다:
...
**모양은 사다리가 아니다.** 원형은 `50%`, 알약은 `var(--aot-btn-pill-radius)`,
없으면 `0`. 시퀀스 요일 타일이 알약이 되면 "한 주" 로 안 읽히는 것이 그 예다 —
타일은 사다리에서, 알약은 알약 토큰에서 고른다. 높이 2px 짜리 막대의 둥근 끝도
알약이지 4px 모서리가 아니다(브라우저가 짧은 변의 절반으로 깎는다).

`aot/tests/test_css_conventions.py` 가 글자 크기와 같은 방식으로 지킨다.
...
**`!important` 를 뗄 때는 브라우저에서 재 보고 뗀다.** 중복 규칙을 하나로
합치면 대개 특정도만으로 이기지만, `aot-base-ui.css` 의 알약 버튼 규칙처럼
`:not()` 사슬로 특정도를 (0,9,0) 까지 올려 둔 전역 규칙이 있다. 톱니 버튼의
`padding: 0` 이 그것에 져서 아이콘이 19.2px 안으로 밀렸고, 브라우저 기본
글꼴이 큰 환경에서는 겹치지 않아 눈에 띄지도 않았다.
