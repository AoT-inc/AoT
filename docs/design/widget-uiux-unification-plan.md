...
- 대비: 사용자의 실제 테마가 적용된 페이지에서 새 선언을 얹어 재측정 —
  위젯 안 텍스트의 AA 위반이 3건 → **0건**(남은 1건은 측정 오류로 밝혀졌다 — 아래).

> **~~미해결 1건~~ → 해결: 결함이 아니라 측정 오류였다 (2026-09-06 후속).**
> `AoT_plot` 의 구획 선택 상자가 `#999999`(2.85:1)로 측정된 것은 **브라우저
> 패널이 화면에 그려지지 않는 동안 CSS 트랜지션이 멈춰 있었기 때문**이다.
>
> 확인 절차: 시트를 하나씩 꺼도 색이 안 바뀌고, 전부 끄면 검정이 되고,
> **같은 마크업의 복제본은 정상색이 나왔다** — CSS 가 아니라는 뜻이다.
> `getAnimations()` 를 보니 `color` 트랜지션이 `playState: "running"` 인데
> `currentTime: 0` 으로 **한 밀리초도 진행하지 않은 채** 멈춰 있었다.
> 키프레임은 `rgb(153,153,153) → rgb(94,107,100)`, `anim.finish()` 를 부르면
> 곧바로 `rgb(94,107,100)`(#5E6B64)가 된다.
>
> 즉 `#999999` 는 bootstrap-select 의 `.bs-placeholder` **시작색**이다.
> 위젯 JS 가 실제 선택값을 넣으면 그 클래스가 떨어지고 Bootstrap 의
> `.btn { transition: color .15s }` 를 따라 0.15초에 걸쳐 `#5E6B64` 로 간다
> (흰 바탕 대비 5.13:1 — AA 통과). 사람이 보는 화면에서는 그 전환이 정상적으로
> 끝나므로 **고칠 것이 없다.**
>
> ⚠ 이 패널에서는 같은 이유로 문서 전체 애니메이션 68개가 전부 멈춰 있었다.
> 세션 중 `.seq-day-btn.is-selected` 의 배경이 투명으로 읽힌 것도 같은 원인이다
> (그 요소도 `transition: background .15s` 를 갖는다).
> **트랜지션이 걸린 속성을 이 패널에서 잴 때는 `getAnimations()` 로 먼저
> 끝내고 재야 한다.** 이번에 그렇게 다시 재 보니 제목 크기·굵기, 카드
> 테두리색, 두 대비 수정값은 **전부 그대로**였다(트랜지션이 없는 속성들이다).
...
> **~~미해결~~ → 해결(2026-09-06 후속).** WP4 절 끝에 적었다 — 결함이 아니라
> **브라우저 패널의 멈춘 트랜지션을 잰 것**이었다. 실제 화면에서는 0.15초 만에
> `#5E6B64`(5.13:1, AA 통과)로 간다.
...
- ~~**타일·카드 모서리.**~~ ✅ 2026-09-07 해결 — 앱 전체와 함께
  `--aot-radius-xs/sm/md/lg/xl`(4·8·12·16·20px) 다섯 단으로 모았다.
  여기서 걱정한 대로 알약은 사다리에 넣지 않았다(`--aot-btn-pill-radius`).
  자세한 것은 `widget-conventions.md` §3 과 `aot-theme-variables.css`.
- **알약 버튼 좌우 여백** — 정본 14px vs 센서라벨 계열 12px.
- ~~**`aot-pill-btn-sm`**~~ ✅ 2026-09-07 해결 — 치수를 지어내지 않고
  **이음매만** 만들었다. `--aot-btn-height-sm` 을 읽는 정의를 두었고, 그 토큰이
  지금 base 와 같은 값이라 화면은 그대로다(/plots 실측: 46개 버튼 32x54 불변).
  더 작은 단이 필요해지면 토큰 한 곳만 갈라 놓으면 호출부가 함께 따라온다.
...
| ✅ | **WP4** 색 정합 | 중 | 낮 | 완료 — 대비 2건 수정 (선택상자 건은 측정 오류였음) |
