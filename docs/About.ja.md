...
センサーで環境を観測し、デバイスを遠隔操作するためのオープンソースシステムであり、特定の用途や場所の種類に縛られません。

[Raspberry Pi](https://en.wikipedia.org/wiki/Raspberry_Pi)のようなシングルボードコンピュータにネイティブで動作するほか、一般的なサーバーやPC上ではDockerで動作します。

提供するものは次のとおりです。

- 実績のある**入力(Input)・出力(Output)・機能(Function)**の制御モデル。AoT が出発点とした Mycodo プロジェクトから受け継いだものです([由来](#origins)を参照)。
- **GIS 地図** — デバイス・センサー・構造物を地図上の実際の位置に置き、そこから確認・操作できます。
- **MCP(Model Context Protocol)ベースのAIレイヤー** — システムを読み取り、診断し、操作できるアシスタントです。ハードウェアを動かす操作には必ず承認を求めます。

どれを使うかは利用者が決めます。

## GIS — 地図・施設・情報

地図ウィジェットには、デバイス・センサー・構造物・区域が実際の位置どおりに置かれます。使うかどうかは利用者が決め、地図なしの構成でも他の機能は同じように動作します。

- **空間階層** — サイト・ゾーン・施設・植栽エリアが実際の階層を成しているため、「東棟で何が起きているか」といった問いに明確な答えが出せます。
- **3D施設** — 施設の外形をポリゴンとして定義して3Dで可視化し、換気窓やカーテンなどの構成要素はその形状と一体になっていて、そこから直接制御できます。
