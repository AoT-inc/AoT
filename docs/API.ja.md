...
1人のユーザーが**複数のAPIキーをそれぞれ名前付きで**持てます。**管理 → システム管理 → ユーザー**でユーザーを編集し、**APIキー**セクションで発行します。そのキーを使うクライアントが分かるよう名前を付け(例: 「ChatGPT」「Claude Desktop」)、**生成**を押します — 新しく発行しても既存のキーは上書きされないため、同じユーザーで複数の連携を同時に使え、1つのキーだけを個別に無効化しても他のキーには影響しません。

発行時には**権限**を選びます: **フルアクセス**(デフォルト — そのユーザーアカウントと同じ権限)か、**読み取り専用**(データの取得はできますが、状態を変更するリクエストはすべて`403 Forbidden`で拒否されます)です。権限は発行前に決める必要があり、既存のキーの権限は変更できません(無効化して再発行してください)。

キーは128バイトのランダムな値で、**生成された瞬間に一度だけ**base64エンコードされた文字列として表示されます — AoTはキー自体ではなく一方向ハッシュのみを保存するため、後から再表示することはできません。紛失した場合は新しく発行してください。詳しくは[セキュリティ](Security.md#api-keys)を参照してください。
...
プログラム側で処理できる4xxエラー(たとえばカードが拒否された場合など)には、報告された問題を簡潔に説明するエラーコードが含まれます。このAPIに固有の例を1つ挙げると、**読み取り専用**のAPIキーで状態を変更するリクエスト(`GET`/`HEAD`/`OPTIONS`以外のすべてのリクエスト)を送ると、そのユーザーであれば本来許可される内容であっても`{"error": "Forbidden", "message": "..."}`とともに`403 Forbidden`が返されます。
