...
## 오류 개선 (Bug Fixes)
- **지도 위젯**: 지도 레이어(Overlay) 상태가 새로고침 후에도 유지되도록 개선 (1단계).
- **보안 업데이트**: 모든 위젯의 AJAX/fetch 요청에 CSRF 토큰 주입을 의무화하여 배포 서버의 403 오류 해결.
- **Input 시스템**: 입력 설정 저장 및 비동기 동기화 오류 개선.
- **I2C 시스템**: I2C 장치 통신 안정성 및 예외 처리 강화.
...
### 지오 시스템 (Geo System)
- **Geo Design**: 지도 유형, 데이터 추출, 도형 그리기 및 장치 배치 기능 추가.
- **Geo Input**: 다양한 GIS를 추가하고 기본적인 설정을 제공합니다.
