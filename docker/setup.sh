...
case "${1:-''}" in
    ...
    ;;
    "test")
        docker exec -ti aot_flask "${INSTALL_PATH}"/env/bin/python -m pip install --upgrade -r /home/aot/aot/install/requirements-testing.txt
        docker exec -ti aot_flask "${INSTALL_PATH}"/env/bin/python -m pytest /home/aot/aot/tests/software_tests
    ;;
    "clean-all")
