...
class WidgetController(AbstractController, threading.Thread):
    ...
    def widget_add_refresh(self, unique_id):
        ...
        if widget is None:
            self.logger.error(f"widget_add_refresh: Widget {unique_id} not found in DB")
            return
        if widget.graph_type not in self.dict_widgets:
            self.logger.error(f"widget_add_refresh: Unknown widget type '{widget.graph_type}' for {unique_id}")
            return
        ...
        try:
            ...
            widget = db_retrieve_table_daemon(Widget, unique_id=unique_id)
            if widget is None:
                self.logger.error(f"widget_add_refresh: Widget {unique_id} disappeared after module load")
                return
