...
WIDGET_INFORMATION = {
    ...
    'widget_name': lazy_gettext('Camera'),
    'widget_library': '',
    'no_class': True,

    'message': lazy_gettext('Displays a camera image or stream.'),
    ...
    'custom_options': [
        ...
        },
        {
            ...
            'default_value': 'relay',
            'options_select': [
                ('relay', lazy_gettext('Server Relay (MJPEG)')),
                ('direct', lazy_gettext('Direct Client Connection (HLS/Direct)'))
            ],
    ...
    $.ajax(url, {
      success: function(data, responseText, jqXHR) {
        if (jqXHR.status === 204) {
          ...
          let timestamp_str = '';
          if (image_type_str === 'still') timestamp_str = '{{_("Still")}}: ';
          else if (image_type_str === 'timelapse') timestamp_str = '{{_("Timelapse")}}: ';
          ...
          if (filename === 'max_age_exceeded') {
            ...
            if (document.getElementById(dashboard_id + "-timestamp")) document.getElementById(dashboard_id + "-timestamp").innerHTML = timestamp_str + "{{_('Max Age Exceeded')}}";
          } else if (filename === 'file_not_found') {
            // No image was found in the directory
            document.getElementById(dashboard_id + "-image-src").src = "/static/img/image_error.png";
            document.getElementById(dashboard_id + "-image-href").href = "/static/img/image_error.png";
            if (document.getElementById(dashboard_id + "-timestamp")) document.getElementById(dashboard_id + "-timestamp").innerHTML = timestamp_str + "{{_('File Not Found')}}";
          } else {
            ...
            document.getElementById(dashboard_id + "-image-src").src = "/static/img/image_error.png";
            document.getElementById(dashboard_id + "-image-href").href = "/static/img/image_error.png";
            if (document.getElementById(dashboard_id + "-error")) document.getElementById(dashboard_id + "-error").innerHTML = "{{_('Error')}}: " + data[0] + "<br>";
        }
      },
      error: function(jqXHR, textStatus, errorThrown) {
        document.getElementById(dashboard_id + "-image-src").src = "/static/img/image_error.png";
        document.getElementById(dashboard_id + "-image-href").href = "/static/img/image_error.png";
        if (document.getElementById(dashboard_id + "-error")) document.getElementById(dashboard_id + "-error").innerHTML = "{{_('Error Getting Image')}}<br>";
      }
    });
  }
  ...
  function repeat_get_image_cam(dashboard_id, camera_unique_id, period_sec, image_type, max_age, stream_method) {
    ...
    if (image_type === 'stream') {
        ...
        $.ajax('/api/cameras/info/' + camera_unique_id, {
                ...
                if (stream_url.endsWith('.m3u8')) {
                  ...
                  if (window.Hls) {
                    ...
                    var interval = setInterval(function() {
                      if (window.Hls) {
                        ...
                        clearInterval(interval);
                        if (document.getElementById(dashboard_id + "-error")) document.getElementById(dashboard_id + "-error").innerHTML = "{{_('Error: HLS library failed to load')}}<br>";
                      }
                      retryCount++;
                    }, 500);
                  }
                  ...
                  if (isRtsp) {
                    ...
                    if (document.getElementById(dashboard_id + "-error")) {
                      document.getElementById(dashboard_id + "-error").innerHTML = "{{_("Error: Browser doesn't support direct RTSP. Use 'Relay' method instead.")}}<br>";
                    }
                    return;
                  }
                 ...
                 if (document.getElementById(dashboard_id + "-error")) document.getElementById(dashboard_id + "-error").innerHTML = "{{_('Error: No streaming URL configured')}}<br>";
              }
            } else {
          ...
          error: function() {
            if (document.getElementById(dashboard_id + "-error")) document.getElementById(dashboard_id + "-error").innerHTML = "{{_('Error fetching camera info')}}<br>";
          }
        });
      if (document.getElementById(dashboard_id + "-timestamp")) document.getElementById(dashboard_id + "-timestamp").innerHTML = '{{_("Live Stream")}}';
    } else {
      imgEl.style.display = 'block';
      hrefEl.style.display = 'block';
      videoEl.style.display = 'none';
