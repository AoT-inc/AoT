...
WIDGET_INFORMATION = {
    ...
    'widget_name': lazy_gettext('Indicator'),
    'widget_library': '',
    'no_class': True,

    'message': lazy_gettext('Displays a red or green circular image based on a measurement value. Useful for showing if an Output is on or off.'),
    ...
    if (measure_type === "output") {
      ...
      $.ajax(url, {
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          // 빈 문자열 src 는 브라우저가 '현재 페이지 URL'(대시보드 HTML)을
          // 다시 요청하게 만든다 — 위젯 수 × 폴링마다 1.3MB 페이지 재요청이
          // 발생해 워커 풀을 포화시킨다. 투명 1x1 데이터 URI 로 대체한다.
          document.getElementById('value-' + widget_id).src = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
