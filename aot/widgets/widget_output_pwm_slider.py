...
WIDGET_INFORMATION = {
    ...
    'widget_name': lazy_gettext('Output (PWM Slider)'),
    'widget_library': '',
    'no_class': True,

    'message': lazy_gettext('Displays and allows control of a PWM output using a slider.'),
...
<div class="pause-background" id="container-output-{{each_widget.unique_id}}" style="height: 100%; text-align: center">
  ...
  <span class="aot-w-caption" id="timestamp-{{each_widget.unique_id}}"></span>
  {%- else -%}
  ...
  <span class="aot-w-value" id="value-{{each_widget.unique_id}}"></span> %
  {%- else -%}
...
<span class="aot-w-value" id="range_val_{{each_widget.unique_id}}"></span> <input id="range_{{each_widget.unique_id}}" type="range" min="0" max="100" step="1" value="0" style="width:80%;" oninput="showVal('{{each_widget.unique_id}}', this.value)" onchange="PWMSlidersendVal('{{each_widget.unique_id}}', '{{device_id}}', '{{channel_id}}', this.value)">
