...
WIDGET_INFORMATION = {
    ...
    'widget_name': lazy_gettext('Graph (Synchronous) [Highstock]'),
    'widget_library': 'Highstock',
    'no_class': True,

    'message': lazy_gettext('Displays a synchronous graph (all data is downloaded for the selected period on the x-axis).'),
    ...
    'custom_options': [
        ...
        {
            'type': 'message',
            'default_value': lazy_gettext('Hold down the <kbd>Ctrl</kbd> or <kbd>&#8984;</kbd> key to select more than one'),
        }
    ],
...
""",
    ...
    'widget_dashboard_configure_options': """
        <div class="row small-gutters" style="padding: 0.5em">
          ...
          <div class="col-auto">
            <label class="control-label">{{_('Enable Custom Colors')}}</label>
        ...
        <div class="row small-gutters" style="padding: 0.5em">
          ...
          <div class="col-auto">
            ...
            <label class="control-label" for="color_number{{index}}">{{_('Color')}}</label>
            <div>
          ...
          <div class="col-auto">
            ...
            <div class="input-group-text">
              <select id="series_type-{{widget_variables['colors_graph'][n]['measure_id']}}" name="series_type-{{widget_variables['colors_graph'][n]['measure_id']}}">
                ...
                <option value="step-left" {% if widget_variables['colors_graph'][n]['series_type'] == "step-left" %} selected{% endif %}>{{_('Line (Step, Left)')}}</option>
                <option value="step-center" {% if widget_variables['colors_graph'][n]['series_type'] == "step-center" %} selected{% endif %}>{{_('Line (Step, Center)')}}</option>
                <option value="step-right" {% if widget_variables['colors_graph'][n]['series_type'] == "step-right" %} selected{% endif %}>{{_('Line (Step, Right)')}}</option>
        ...
        <div class="row small-gutters" style="padding: 0.5em">
          ...
          <div class="col-auto">
            <label class="control-label">{{_('Enable Y-Axis Align Ticks')}}</label>
            <div class="input-group-text">
          ...
          <div class="col-auto">
            <label class="control-label">{{_('Enable Y-Axis Start On Tick')}}</label>
            <div class="input-group-text">
          ...
          <div class="col-auto">
            <label class="control-label">{{_('Enable Y-Axis End On Tick')}}</label>
            <div class="input-group-text">
      ...
      {% for each_yaxis in widget_variables['y_axes'] if each_yaxis in dict_units %}
        ...
        <div class="row small-gutters" style="padding-left: 0.5em">
          ...
          <div class="col-auto">
            <label class="form-check-label" for="custom_yaxis_min_{{index}}">{{_('Y-Axis Minimum')}}</label>
            <div>
          ...
          <div class="col-auto">
            <label class="form-check-label" for="custom_yaxis_max_{{index}}">{{_('Y-Axis Maximum')}}</label>
            <div>
      ...
      function(data, responseText, jqXHR) {
        if (jqXHR.status !== 204) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            // Data is not guaranteed sorted — track the newest timestamp, not the last
            // element. Storing an older one makes later polls re-fetch and re-add the
            // whole window, growing the series without bound.
...
{% set graph_note_tag_ids = widget_options['measurements_note_tag'] %}
  ...
  {% endfor %}
    ...
    rangeSelector: {
      ...
      inputStyle: { color: 'var(--aot-color-text-primary, #333333)' },
      buttons: [{
        count: 1,
        type: 'minute',
        text: '{{_("1m")}}'
...
def dict_custom_colors(widget_options):
    ...
    """
    color_count = []
    # 전역 차트 시리즈 팔레트로 통일 (aot.config 단일 소스 + custom_ui chart_1..6 오버레이)
