...
WIDGET_INFORMATION = {
    ...
    'widget_name': lazy_gettext('Measurement (2 Values)'),
    'widget_library': '',
    'no_class': True,

    'message': lazy_gettext('Displays two measurement values and timestamps.'),
    ...
    'custom_options': [
        ...
        },
        {
            ...
            'default_value': 1.0,
            'constraints_pass': constraints_pass_positive_value,
            'name': lazy_gettext('Value Size (multiplier)'),
            'phrase': lazy_gettext('Multiplier applied to the base value font size (1.0 = default)')
        },
        {
            'id': 'font_em_unit',
            'type': 'float',
            'default_value': 1.0,
            'constraints_pass': constraints_pass_positive_value,
            'name': lazy_gettext('Unit Size (multiplier)'),
            'phrase': lazy_gettext('Multiplier applied to the base unit font size (1.0 = default)')
        },
        {
            ...
            'default_value': 1.0,
            'constraints_pass': constraints_pass_positive_value,
            'name': lazy_gettext('Timestamp Size (multiplier)'),
            'phrase': lazy_gettext('Multiplier applied to the base timestamp font size (1.0 = default)')
        },
        {
            'id': 'enable_unit',
            'type': 'bool',
    ...
    'widget_dashboard_title_bar': """<span class="widget-title-bar aot-w-title" style="padding-right:0.5em">{{each_widget.name}}</span>""",
  ...
  {#- 단위가 's'(지속시간)면 값은 HH:MM:SS 로 찍고 단위 접미사는 안 붙인다. -#}
  {%- set aot_value_unit_1 = dict_units[dict_measure_units[measurement_id_1]]['unit']
        if dict_measure_units[measurement_id_1] in dict_units else '' -%}
  {%- set aot_value_unit_2 = dict_units[dict_measure_units[measurement_id_2]]['unit']
        if dict_measure_units[measurement_id_2] in dict_units else '' -%}
  ...
  {%- for each_input in input if each_input.unique_id == device_id_1 and measurement_id_1 in device_measurements_dict -%}
    <span class="widget-measurement-value aot-w-value" id="1-value-{{each_widget.unique_id}}" data-unit="{{aot_value_unit_1}}"></span><span class="widget-measurement-unit aot-w-unit">
        {%- if dict_measure_units[measurement_id_1] in dict_units and
               dict_units[dict_measure_units[measurement_id_1]]['unit'] and
               aot_value_unit_1 != 's' and
  ...
  {%- for each_input in input if each_input.unique_id == device_id_2 and measurement_id_2 in device_measurements_dict -%}
    <span class="widget-measurement-value aot-w-value" id="2-value-{{each_widget.unique_id}}" data-unit="{{aot_value_unit_2}}"></span><span class="widget-measurement-unit aot-w-unit">
        {%- if dict_measure_units[measurement_id_2] in dict_units and
               dict_units[dict_measure_units[measurement_id_2]]['unit'] and
               aot_value_unit_2 != 's' and
               widget_options['enable_unit'] -%}
  ...
  <div>
    {%- if widget_options['enable_timestamp'] and measurement_id_1 in last_measure_dict -%}
      <span class="aot-w-caption" id="1-timestamp-{{each_widget.unique_id}}">{{last_measure_dict[measurement_id_1].timestamp.strftime('%Y-%m-%d %H:%M:%S')}}</span>
    {%- endif -%}
  </div>
  <div>
    {%- if widget_options['enable_timestamp'] and measurement_id_2 in last_measure_dict -%}
      <span class="aot-w-caption" id="2-timestamp-{{each_widget.unique_id}}">{{last_measure_dict[measurement_id_2].timestamp.strftime('%Y-%m-%d %H:%M:%S')}}</span>
    {%- endif -%}
  </div>
  ...
  // 단위가 's'(지속시간)면 초를 그대로 찍지 않고 HH:MM:SS 로.
      ...
      success: function(data, responseText, jqXHR) {
        if (jqXHR.status === 204) {
          if (document.getElementById(measurement_num + '-value-' + widget_id)) {
            document.getElementById(measurement_num + '-value-' + widget_id).innerHTML = '{{_("NO DATA")}}';
          }
          if (document.getElementById(measurement_num + '-timestamp-' + widget_id)) {
            document.getElementById(measurement_num + '-timestamp-' + widget_id).innerHTML = '{{_("MAX AGE EXCEEDED")}}';
          }
        }
        else {
          ...
          if (document.getElementById(measurement_num + '-value-' + widget_id)) {
            const value_el = document.getElementById(measurement_num + '-value-' + widget_id);
