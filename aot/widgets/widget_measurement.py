...
WIDGET_INFORMATION = {
    ...
    'widget_name': lazy_gettext('Measurement (1 Value)'),
    'widget_library': '',
    'no_class': True,

    'message': lazy_gettext('Displays a measurement value and timestamp.'),
  ...
  <br/><span class="aot-w-caption" id="timestamp-{{each_widget.unique_id}}"></span>
  {%- endif -%}
  </div>
""",
      ...
      success: function(data, responseText, jqXHR) {
        if (jqXHR.status === 204) {
          if (document.getElementById('value-' + widget_id)) {
            document.getElementById('value-' + widget_id).innerHTML = '{{_("NO DATA")}}';
          }
          if (document.getElementById('timestamp-' + widget_id)) {
            document.getElementById('timestamp-' + widget_id).innerHTML = '{{_("MAX AGE EXCEEDED")}}';
          }
        }
        else {
