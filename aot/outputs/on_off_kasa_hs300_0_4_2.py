...
class OutputModule(AbstractOutput):
    ...
    # device on a timer and clears output_setup when the query fails, then a
    # reconnect restores it. That signal simply never reached the UI. These two
    # methods expose it, so an unreachable device now reads as offline instead
    # of silently showing its last known on/off.
    # ------------------------------------------------------------------ #
    def comm_capable(self):
        """True: the device is re-queried over the network (status_update)."""
        return True
