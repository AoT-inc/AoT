...
runSelfUpgrade() {
  ...
  if [ -d "${CURRENT_AOT_DIRECTORY}" ] ; then
    ...
    CURRENT_VERSION=$("${CURRENT_AOT_INSTALL_DIRECTORY}"/AoT/env/bin/python3 "${CURRENT_AOT_INSTALL_DIRECTORY}"/AoT/aot/utils/github_release_info.py -c 2>/dev/null)
    ...
    printf "Could not find a current version of AoT installed. Check the symlink /var/aot-root that is supposed to point to the install directory"
    exit 1
  fi
  ...
  # --- GIS edition compatibility guard ---
  # The legacy 2D-map edition stores config as a single module (aot/config.py),
  # while this GIS edition uses a config package (aot/config/__init__.py). The two
  # editions have incompatible database schemas (separate alembic lineages), so an
  # in-place upgrade from 2D to GIS would corrupt the database. If the currently
  # installed version is the 2D edition, stop before anything is modified.
  if [ -f "${CURRENT_AOT_DIRECTORY}"/aot/config.py ] && [ ! -f "${CURRENT_AOT_DIRECTORY}"/aot/config/__init__.py ] ; then
    printf "\n\n#### UPGRADE BLOCKED: incompatible edition ####\n"
    printf "The installed version is the legacy 2D-map edition, but this release is the GIS\n"
    printf "edition, which uses a different and incompatible database schema. An in-place\n"
    printf "upgrade would corrupt your database, so it has been stopped before any change was\n"
    printf "made. Your current installation is untouched and still running.\n\n"
    printf "To move to the GIS edition, perform a FRESH INSTALL on a clean system and re-create\n"
    printf "your configuration. See: https://aot-inc.github.io/AoT/Upgrade-Backup-Restore/\n\n"
  ...
  THIS_VERSION=$("${CURRENT_AOT_DIRECTORY}"/env/bin/python3 "${THIS_AOT_DIRECTORY}"/aot/utils/github_release_info.py -c 2>/dev/null)
  ...
  mkdir -p "${THIS_AOT_DIRECTORY}"/aot/databases
  ...
  if [ "$RELEASE_WIPE" = true ] ; then
    ...
    if [ -f /opt/AoT/aot/databases/aot.db ] ; then
      printf "Removing database at /opt/AoT/aot/databases/aot.db..."
      if ! rm -f /opt/AoT/aot/databases/aot.db ; then
        printf "Failed: Error while trying to delete database at /opt/AoT/aot/databases/aot.db.\n"
      fi
      printf "Done.\n"
    fi
