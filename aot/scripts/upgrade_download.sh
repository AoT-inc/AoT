...
runDownloadAoT() {
  ...
  CURRENT_VERSION=$("${INSTALL_DIRECTORY}"/AoT/env/bin/python3 "${INSTALL_DIRECTORY}"/AoT/aot/utils/github_release_info.py -c 2>/dev/null)
  ...
  if [ "$UPGRADE_TYPE" == "upgrade-release-major" ] || [ "$UPGRADE_TYPE" == "upgrade-release-wipe" ] ; then
    ...
    if [ -n "$UPGRADE_MAJ_VERSION" ]; then
      ...
      UPDATE_VERSION=$("${INSTALL_DIRECTORY}"/AoT/env/bin/python3 "${INSTALL_DIRECTORY}"/AoT/aot/utils/github_release_info.py -m "$UPGRADE_MAJ_VERSION" -v 2>/dev/null)
      UPDATE_URL=$("${INSTALL_DIRECTORY}"/AoT/env/bin/python3 "${INSTALL_DIRECTORY}"/AoT/aot/utils/github_release_info.py -m "$UPGRADE_MAJ_VERSION" 2>/dev/null)
