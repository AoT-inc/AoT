...
def assess(
    ...
    # ⚠ **창은 사이클보다 넉넉해야 한다** (2026-08-26).
    # 고정 300초였는데, 이 값보다 사이클이 길면 매 주기 직전 점이 창 밖으로
    # 밀려나 점이 **하나만** 남는다. 회귀는 점 2개 미만이면 0 을 돌려주므로
    # 추세가 **구조적으로 영원히 0** 이 된다 — 화면은 "→ 0 °C/min" 을 띄우고
    # 사용자는 환경이 안 변하는 줄 안다. 실측(2026-08-26): 사이클 600초인
    # 온실이 실내 온도 22.8 → 27.1 °C 로 오르는 동안 세 축 모두 0.0 이었다.
    # 최소 3점이 남도록 사이클에 비례해 넓힌다(기본 300초는 하한).
    ts.window_sec = max(300.0, float(cycle_sec or 60.0) * 3.0)
