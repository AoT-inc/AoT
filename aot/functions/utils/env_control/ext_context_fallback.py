...
외부 센서(기상 스테이션, 외부 온습도 등)가 만료됐을 때:
  ...
  2. 개구부는 **더 열지 않는다** — 강우·풍속을 잃으면 안전 게이트가
     `vent_open_ceiling` 을 걸고(마지막 값이 위험이면 그 값으로 계속 닫는다),
     온습도까지 지어낸 것이면 코디네이터가 제자리에 둔다(2.6).
     ⚠ 2026-09-19 전에는 "게이트가 개구부·차광막을 강제 폐쇄한다" 였다. 그
       전제로 아래 wind/rain 을 0 으로 둔 것이다 — 이제 0 이 안전한 이유는
       폐쇄가 아니라 "더 열지 않음" 이다. 강우 판정은 이 dict 를 보지 않는다
       (게이트는 원래 external 을 본다).
  3. 내부 전용 액추에이터(난방·냉방·CO₂·가습)는 L1-L3 를 계속 실행한다.

fallback 원칙 (보수적 기본값):
  - wind  : 0.0 m/s  (판정용이 아니다 — 게이트는 이 값을 보지 않는다)
  - rain  : 0.0      (판정용이 아니다 — 위와 같다)
...
def build_fallback_context(
    ...
    fallback = {
        # 기상 조건 — 게이트는 이 값을 보지 않는다(강우·풍속을 잃으면
        # 게이트가 "더 열지 않음" 을 건다, 모듈 머리말 2)
        'wind':     0.0,
        'rain':     0.0,
