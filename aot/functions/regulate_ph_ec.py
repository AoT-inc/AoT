...
FUNCTION_INFORMATION = {
    ...
    'function_name': lazy_gettext('pH, EC Regulation'),
    'measurements_dict': measurements_dict,
    'enable_channel_unit_select': False,

    'message': lazy_gettext('This function uses two pumps (acid and base solutions) to regulate pH, and can use up to four pumps (A, B, C, D nutrient solutions) to regulate electrical conductivity (EC). You only need to configure the nutrient solution outputs you intend to use. Outputs that are not configured are not activated during EC adjustment, and you can use from one to four pumps. Outputs can operate in units of duration (seconds) or volume (ml), and each output type must be matched to the selected output channel (an on/off output channel for duration control, a volume output channel for volume control). The mixing ratio of the nutrient solutions is determined by the duration or volume setting of each EC output. '
               'If you enter an email address (or multiple addresses separated by commas) in the email notification field, a notification email is sent in the following cases. '
               '<br>1) when the pH value goes outside the configured danger range, 2) when the EC value is too high and water must be added to the storage tank, 3) when no measurement can be found in the database within a given Max Age range. '
               '<br>Each email notification type has its own timer so that the same notification is not sent repeatedly, and the same notification is not sent for the duration of the configured email timer. '
               '<br>Once this duration elapses, the timer resets automatically, allowing a new notification to be sent. You can also manually reset the email timers using the Custom Commands below. '
               '<br>When the function is enabled, status text is displayed at the bottom of the screen showing the regulation information and the total duration/volume of each output.'),
