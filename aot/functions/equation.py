...
FUNCTION_INFORMATION = {
    ...
    'message': lazy_gettext('This function retrieves a measurement, applies it to a user-defined equation, and stores the result with the selected measurement and unit.'),
...
class CustomModule(AbstractFunction):
    ...
    def loop(self):
        ...
        if last_measurement:
            try:
                equation_output = safe_eval(
                    self.equation, {'x': last_measurement[1]})
            ...
            self.logger.debug("Equation: {} with x={} -> {}".format(
                self.equation, last_measurement[1], equation_output))
