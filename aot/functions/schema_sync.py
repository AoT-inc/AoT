...
class ConflictError(Exception):
  """Raised when cfg_rev optimistic-lock check fails (HTTP 409)."""
  pass
...
def read_ns(controller_id: str, ns: str) -> Dict[str, Any]:
  """Return controller-level namespace dict with ensured `cfg_rev` int."""
...
def write_ns(controller_id: str, ns: str, payload: Dict[str, Any], expect_rev: Optional[int] = None) -> int:
  """
  Persist controller-level namespace dict with optimistic lock on `cfg_rev`.
  - If `expect_rev` is not None and != current rev → ConflictError.
...
def read_ns_channels(controller_id: str, ns: str) -> List[Dict[str, Any]]:
  """
  Return a list of per-channel namespace dicts ordered by FunctionChannel.id.
  Each item at least contains {"index": <1-based index>}.
  """
...
def write_ns_channels(controller_id: str, ns: str, items: List[Dict[str, Any]]) -> None:
  """
  Persist per-channel namespace dicts.
  - Does not create/delete FunctionChannel rows.
  - Maps incoming items to existing rows in DB-id order.
  - Ignores extra items beyond existing rows.
  """
  q = db_retrieve_table_daemon(FunctionChannel).filter(FunctionChannel.function_id == controller_id)
  rows = q.order_by(FunctionChannel.id.asc()).all()
...
def get_namespace_schema(controller_id: str, ns: str) -> Dict[str, Any]:
  """
  Convenience: read both controller+channels for a namespace.

  Returns:
    {
      "cfg_rev": int,
      "global": dict,       # controller-level payload incl. cfg_rev
      "channels": [dict],   # list of channel payloads with "index"
    }
  """
...
def set_namespace_schema(
  ...
  """
  Convenience: write controller-level namespace (with optimistic lock) and,
  if provided, write channel-level namespace payloads in one go.

  Returns the new cfg_rev.
  """
...
def load_namespace(
  ...
  """
  Read the given namespace (and channels if requested) from DB and pass them
  to `apply_fn(func_obj, global_dict, channels_list)` so that each Function
  can map settings into its own runtime attributes however it sees fit.

  Returns the schema dict from get_namespace_schema().
  """
