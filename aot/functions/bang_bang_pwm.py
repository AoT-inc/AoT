...
FUNCTION_INFORMATION = {
    ...
    'function_name': lazy_gettext('Bang-Bang Hysteretic (PWM) (Raise/Lower/Both)'),
    'function_name_short': 'Bang-Bang (PWM, Raise/Lower/Both)',

    'message': lazy_gettext('A simple Bang-Bang control method that uses a single input value to control one PWM output. '
               'Select the input, enter the PWM output, Setpoint, and **Hysteresis**, then choose the Direction. '
               '\t•\tRaise mode (e.g., heating): the output turns on when the input is at or below (setpoint - hysteresis), and turns off when the input is at or above (setpoint + hysteresis). '
               '\t•\tLower mode (e.g., cooling): the opposite of the above, turning the output on to lower the input. '
               '\t•\tBoth mode: adjusts Raise and Lower to keep the input at the setpoint. '
               'Note: this output only works with a PWM (Pulse Width Modulation) output.'),
