...
FUNCTION_INFORMATION = {
    ...
    'function_name': lazy_gettext('PID Autotune'),

    'message': lazy_gettext('This function attempts to automatically tune a PID controller. That is, it enables the output and measures the response from the sensor multiple times to calculate the P, I, and D gain values. '
               'Updates on the operating status are written to the daemon log, and when autotuning completes successfully a summary is also stored in the daemon log. '
               'Only raising the current measurement is supported; lowering the measurement may require some modification of the controller code. '
               'To monitor whether the output is normally raising the measurement above the setpoint, it is recommended to graph the measurement and output on the dashboard. '
               'The autotune feature is experimental and not fully developed. It is likely it will not generate proper PID gains, so it is recommended not to rely on this function for accurate PID controller tuning.'),
