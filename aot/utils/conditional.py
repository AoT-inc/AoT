...
"""
        ...
        if test:
            cmd_test = 'mkdir -p {dir}/.pylint.d && ' \
                       ...
                       '{py} -m pylint -d I,W0621,C0103,C0111,C0301,C0327,C0410,C0413,R0912,R0914,R0915 {path}'.format(
                           dir=INSTALL_DIRECTORY, py=sys.executable, path=file_run)
            cmd_out, cmd_err, cmd_status = cmd_output(cmd_test, user='root')
            if cmd_err:
                err_str = cmd_err if isinstance(cmd_err, str) else cmd_err.decode('utf-8', errors='replace')
                if 'No module named pylint' in err_str:
                    cmd_status = None
                else:
