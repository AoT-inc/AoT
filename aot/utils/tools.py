...
def create_measurements_export(influxdb_version, save_path=None):
    try:
        ...
        if influxdb_version.startswith("1."):
            cmd = "/usr/bin/influxd backup -database {} -portable {}".format(
                shlex.quote(settings.measurement_db_dbname),
                shlex.quote(influx_backup_dir))
            out, err, status = cmd_output(cmd, user='root')
        elif influxdb_version.startswith("2."):
            cmd = "/usr/bin/influx backup --org aot {}".format(
                shlex.quote(influx_backup_dir))
            out, err, status = cmd_output(cmd, user='root')
        else:
            msg = f"Could not determine influxdb version: {influxdb_version}"
            logger.error(msg)
            return 1, msg
        ...
        if not status:
            ...
            msg = f"influxd backup command failed (status={status}): {err}"
            logger.error(msg)
            return 1, msg
