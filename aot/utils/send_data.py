...
_default_logger = logging.getLogger("aot.notification")
...
def send_email(smtp_host, smtp_protocol, smtp_port, smtp_user, smtp_pass,
               ...
               attachment_file=None, attachment_type=False, logger=None):
    """
    ...
    :param logger: logger to use (defaults to a shared logger gated at
        ERROR level); pass the caller's own logger so INFO/DEBUG lines
        follow that device's/function's log_level_debug setting
    ...
    try:
        ...
        if attachment_file:
            ...
            for file in attachments:
                try:
                    ...
                    log.error("Unable to open one of the attachments. "
        ...
        if smtp_port:
            ...
            log.error("Could not determine port to use to send email. Not sending.")
            return 1
        ...
        if smtp_protocol in ['unencrypted', 'unencrypted_no_login']:
            log.warning("Sending email without encryption (protocol=%s). This is not recommended.", smtp_protocol)
        try:
            if smtp_protocol == 'ssl':
                ...
                log.error("Unrecognized protocol: {}".format(smtp_protocol))
                return 1
        except Exception as exc:
            log.error("Failed to connect/login SMTP (%s:%s, protocol=%s): %s",
                         smtp_host, port, smtp_protocol, exc)
            return 1

        if response_login:
            log.debug("Email login response: {}".format(response_login))

        # Send the email
        try:
            envelope_from = smtp_email_from or smtp_user
            response_send = server.sendmail(envelope_from, recipients, composed)
            log.info("Email sent via %s:%s protocol=%s to=%s",
                        smtp_host, port, smtp_protocol, recipients)
            log.debug("Email send response: {}".format(response_send))
            return 0
        except Exception as exc:
            log.error("Failed to send email via %s:%s to %s: %s",
