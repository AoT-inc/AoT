...
def method_builder(method_id):
    ...
    # Create new method
        ...
        flash(gettext("Invalid method ID"), "error")
        return redirect('/method')
...
def method_delete(method_id):
    ...
    try:
        ...
        # Cloned/migrated methods may never have been added to the display
        # order, so guard against removing an id that isn't present.
        if method_id in display_order:
            display_order.remove(method_id)
            DisplayOrder.query.first().method = list_to_csv(display_order)
        db.session.commit()
        flash(gettext("Success: %(action)s", action=action), "success")
    except Exception as except_msg:
        flash(gettext("Error: %(action)s: %(err)s", action=action,
                      err=except_msg),
              "error")
    return redirect(url_for('routes_method.method_list'))
