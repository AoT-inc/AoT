...
from aot.config import (ALEMBIC_VERSION, BACKUP_LOG_FILE, CAMERA_INFO,
                           DAEMON_LOG_FILE,
...
def page_camera():
    ...
    for each_cam in camera:
        if each_cam.library == 'opencv':
            try:
                from aot.devices.camera import count_cameras_opencv
            ...
            if mod_camera.stream_started:
                try:
                    camera_stream = import_module(
                        'aot.aot_flask.camera.camera_{lib}'.format(
