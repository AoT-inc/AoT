    ...
    if 'custom_options' in dict_options[device]:
        for each_option in dict_options[device]['custom_options']:
            ...
            if (request_form and
                ...
                # For multi-select types: absence from form = user intentionally cleared (HTML behavior).
                if (request_form and
                        each_option['type'] in ['select_multi_measurement', 'channel_selector', 'select_multi_device']):
                    dict_options_return[each_option['id']] = []
                # Keep existing DB value if present; only fall back to default_value for new entries
                elif each_option['id'] in dict_options_return:
...
def user_is_admin():
    ...
    Used to gate notice-board moderation (pin/edit-any/delete-any).
    """
    if not flask_login.current_user.is_authenticated:
        return False
    user = User.query.filter(User.name == flask_login.current_user.name).first()
    return bool(user and user.role_id == 1)
...
def reorder(display_order, device_id, direction):
    if direction == 'up':
        ...
        reord_list = gettext("Unknown command")
...
def custom_command(controller_type, dict_device, unique_id, form):
    ...
    if controller_type == "Output":
        ...
        messages["error"].append(gettext("Unknown controller type: {}").format(controller_type))
        return messages

    if not controller:
        messages["error"].append(gettext("Could not find button ID."))
        return messages

    if controller_type != "Output" and not controller.is_activated:
        messages["error"].append(gettext("Please activate the controller before executing a custom command."))
        return messages
                ...
                for value in form.getlist(key):
                    if key in options:
                        if options[key]['type'] == 'integer':
                            ...
                            messages["error"].append(gettext("Unknown key type. Key: {}, Type: {}").format(key, options[key]['type']))
