    ...
    try:
        ...
        if trigger.trigger_type == 'trigger_edge':
            ...
            if form.period.data:
                ...
                if p_res:
                    new_period = p_res['total_seconds']
                else:
            ...
            # 폼에서 고친 창을 **정본(JSON)에** 반영한다. 예전에는 레거시 컬럼만
            # 고쳐, 저장 성공 메시지까지 나오면서 실제 동작은 하나도 안 바뀌었다
            # (데몬은 JSON 을 본다 — 실측 2026-09-05).
            _sched = sequence_schedule.load(trigger)
            if sequence_schedule.set_window(_sched,
                                            start=form.timer_start_time.data,
                                            end=form.timer_end_time.data,
                                            period=new_period):
                # 저장 시점에 막지 않으면 데몬이 읽을 때 JSON 을 거부하고 레거시로
                # 폴백한다 — 창이 조용히 잘리고 **요일별 설정이 통째로 사라진다.**
                if sequence_schedule.save(trigger, _sched):
                        ...
                        for w in all_widgets:
                            try:
                                ...
                                if opts.get('function_id') == form.function_id.data:
                                    ...
                                    # 주기는 per_day 면 요일마다 다르다 — 레거시
                                    # 컬럼(첫 활성 요일)이 아니라 오늘 값을 JSON 에서
                                    # 계산한다(sequence_schedule.today_period).
                                    from aot.utils.device_tz import get_device_tz
                                    opts['sequence_period'] = sequence_schedule.today_period(
                                        sequence_schedule.load(trigger),
                                        get_device_tz(trigger), trigger.period or 3600)
...
def trigger_del(trigger_id):
    ...
    from aot.aot_flask.access import scope
    if not scope.can_operate_device(trigger_id):
        messages["error"].append(scope.deny_message())
        return messages
