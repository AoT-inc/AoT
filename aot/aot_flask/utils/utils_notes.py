...
def show_notes(form):
    ...
    for each_error in error:
        flash(gettext('Error: %(err)s', err=each_error), 'error')
            ...
            for each_file in os.listdir(tmp_folder):
                if each_file.endswith('_notes_exported.csv') and not found_csv:
                    ...
                    if (count_notes + count_attach) == 0:
                        ...
                        flash(gettext("Imported %(notes)s notes and %(attach)s "
                                      "attachments", notes=count_notes,
                                      attach=count_attach),
                              "success")
...
def datetime_time_to_utc(datetime_time):
    ...
    users enter time in Misc.timezone, that drifted the stored value by the tz
