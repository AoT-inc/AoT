...
export default function NotesInput({ target }) {
  ...
  // Safe ASCII base name (+.jpg) — the server rejects non-ASCII filenames
  // because secure_filename() strips them down to just the extension.
  const safeJpgName = (name) => {
    const base = (name || '').replace(/\.[^.]+$/, '').replace(/[^A-Za-z0-9_-]+/g, '').slice(0, 40);
  ...
  const resizeImage = (file, maxWidth = 1920, maxHeight = 1920) => {
    return new Promise((resolve) => {
      ...
      reader.onload = (event) => {
        const img = new Image();
        // If the browser can't decode this image (e.g. HEIC on Chrome), send the
        // original — the backend accepts it (and stores it under a UUID name).
        ...
        img.onload = () => {
          ...
          // Always export as JPEG so HEIC / odd formats become a displayable
          // image, with a clean ASCII .jpg name.
          canvas.toBlob((blob) => {
            if (!blob) { resolve(file); return; }
            const resizedFile = new File([blob], safeJpgName(file.name), {
              type: 'image/jpeg',
  ...
  return (
    <div className="relative">
       ...
       {/* Composer row — shared shape with the AI chat composer (.aot-composer-*) */}
       <form onSubmit={handleSubmit} className="aot-composer-row">
          <button
            type="button"
