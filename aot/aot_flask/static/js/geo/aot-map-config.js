...
function populateMapConfigSelects(uid, data, savedMapId) {
    ...
    var updateSelect = function(domId, items) {
        ...
        items.forEach(function(item) {
             ...
             var txt = item.name + (item.has_coords === false ? ' [' + (window._ ? window._('No Coordinates') : 'No Coordinates') + ']' : '');
    ...
    if ($mapSel.length) {
        ...
        var defaultText = $mapSel.data('placeholder') || (window._ ? window._('Select Map') : 'Select Map');
