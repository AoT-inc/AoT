...
;(function() {
    ...
    window.initAoTMapWidget = function(uniqueId) {
        ...
        // Decide whether to show toast messages based on the server settings (AoTGlobalSettings).
        // Do not modify arbitrarily. (Requested by User)
        function showToast(msg, type = 'info') {
          if (typeof window.showToast !== 'undefined') {
            window.showToast(msg, type);
            return;
          }
          // 1. Check server global settings
        ...
        function initWidget() {
            ...
            // Detailed debug - show which dependency is missing
                ...
                marker.bindPopup(() => {
                     ...
                     return `
                        <div style="min-width: 200px; padding: 10px; font-family: 'Inter', sans-serif;">
                            ...
                            <div style="font-size: var(--aot-fs-title); font-weight: var(--aot-fw-semibold); color: #000; margin-bottom: 12px; word-break: break-all;">
                                ${safeTagName}
                            </div>
                            ...
                            <div id="note-row2-view-${noteId}" style="display: flex; flex-direction: column; gap: 10px;">
                                ...
                                <div style="font-size: var(--aot-fs-body); color: #666; line-height: var(--aot-lh-base); word-break: break-all; max-height: 60px; overflow-y: auto; padding: 4px 0;">
                            ...
                            <div id="note-row2-edit-${noteId}" style="display: none; flex-direction: column; gap: 8px;">
                                <input type="text" id="rename-input-${noteId}" value="${safeTagName}" class="form-control" 
                                    style="height: 30px; font-size: var(--aot-fs-body); padding: 4px 8px; border: 1px solid #ddd; border-radius: 4px; width: 100%;">
                                <div style="display: flex; gap: 8px; justify-content: flex-end;">
            ...
            function computeFeatureStyle(feature, dev) {
                ...
                if (isSprinkler) {
                    ...
                    if (t === 'device' || t === 'aot_device' || t === 'input' || t === 'output' || t === 'function') {
                       ...
                       // [Fix] If live device data exists, use it exclusively — do NOT fall back to stale p.is_active from GeoJSON.
                       // Stale stored p.is_active=true causes inactive devices to appear as ON.
                       const isActive = dev ? isLiveON : (p.is_active === true || p.is_active === 'true' || p.status === 'active' || p.status === 'on' || p.status === 'ON');
            ...
            labelMarker.bindPopup(() => {
                 ...
                 return `
                    <div style="min-width: 180px; padding: 5px;">
                        ...
                        <div class="aot-popup-title" style="font-size: var(--aot-fs-value); font-weight: var(--aot-fw-bold); margin: 0; line-height: var(--aot-lh-tight); word-break: break-all; text-align: left; padding-top: 15px; margin-bottom: 8px; color: #333;">
                            ${text}
                        </div>
                        ...
                        <div style="display: flex; flex-direction: column; gap: 6px;">
                            <button class="btn btn-primary" style="border-radius: 14px; height: 28px; width: 100%; font-size: var(--aot-fs-body); display: flex; align-items: center; justify-content: center; padding: 0;"
                                onclick="${openNoteAction}">
                                <i class="fas fa-clipboard mr-2"></i> ${window._('Open Notes')}
                            </button>
                            <div id="last-note-title-${tId}" style="font-size: var(--aot-fs-label); color: #888; padding-left: 4px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">
                                ${window._('Loading...')}
                            </div>
                        </div>
                    </div>
        ...
        function bindDevicePopup(marker, dev, vars) {
            marker.bindPopup(() => {
                ...
                const noteSectionHtml = `
                    ...
                    <div id="${notePreviewId}" style="font-size: var(--aot-fs-body); color: #666; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; min-height: 1.2em; line-height: var(--aot-lh-base); margin-top: 6px;">
                        ...
                        devMeas.forEach(m => {
                            ...
                            bodyHtml += `
                                <div class="aot-popup-row" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; border-bottom: 1px dotted #eee; padding-bottom: 4px;">
                                    <span style="font-weight: var(--aot-fw-regular); font-size: var(--aot-fs-title); color: #333; flex: 1; padding-right: 4px; line-height: var(--aot-lh-tight); min-width: 0; word-break: break-word;">${mName}</span>
                                    <span style="text-align: right; white-space: nowrap; flex: 0 0 auto;">
                                        <span id="popup-val-${dev.id}-${m.id}" class="aot-w-value" style="font-size: var(--aot-fs-title); color: #000;">${mVal}</span>
                                        <span class="aot-popup-unit aot-w-unit" style="font-size: var(--aot-fs-body); margin-left: 2px; color: #555;">${unitStr}</span>
                                    </span>
                                </div>
                            `;
                        });
                ...
                const headerHtml = `
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; gap: 10px;">
                        <div class="aot-popup-title" style="font-size: var(--aot-fs-value); font-weight: var(--aot-fw-bold); margin: 0; line-height: var(--aot-lh-tight); word-break: break-all;">${dev.name}</div>
                ...
                let infoHtml = `
                    <div style="border-top: 1px solid #eee; padding-top: 8px;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                            ...
                            <span id="${durId}" class="aot-timer-display" style="font-family: var(--aot-font-mono); font-size: var(--aot-fs-title); font-weight: var(--aot-fw-bold);">00:00:00</span>
                        </div>
                        <!-- Last Time -->
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-weight: bold; color: #555;">${window._('Last Work Time')}</span>
                            <span id="last-dur-${dev.id}" style="font-family: var(--aot-font-mono); font-size: var(--aot-fs-title); color: #777;">${lastDurStr}</span>
                        </div>
                    </div>
                `;
            ...
            if (marker.drawMeta) {
                ...
                const finalLabelContent = `
                        ...
                        <span class="dev-val-group" style="display: ${valueDisplay}; margin-left: 4px;">
                            ...
                            <span class="dev-unit aot-w-unit" style="font-size: var(--aot-fs-caption); margin-left: 2px;">${unitStr}</span>
                        </span>`;
        ...
        function initRainViewerController(uniqueId, widgetMap, vars) {
            ...
            function updatePlayButton(playing) {
                ...
                if (btn) {
                    ...
                    if (window.AoTSetTitle) window.AoTSetTitle(btn, playTitle); else btn.title = playTitle;
                }
            }
