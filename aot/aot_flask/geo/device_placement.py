...
def sync_device_name(device_id, new_name, channel_id=None, commit=True):
    ...
    마커·구역 **양쪽 다** 갱신한다(2026-08-08 확정, 설계 문서 「결정」 3번).
    한때 이것이 사람이 지은 이름을 덮는다고 보아 미결로 두었으나 전제가
    틀렸다: 사람이 지은 이름은 `properties.label_name` 에 따로 살고 이 함수는
    그 키를 건드리지 않으며, 렌더 경로가 전부 `label_name || name` 순으로
    읽는다. 여기서 고치는 `name` 은 사람이 이름을 안 지었을 때의 폴백이고,
    그 폴백이 교체 뒤에도 옛 장치 이름으로 남는 것이 오히려 거짓말이다.
    """
    from aot.aot_flask.geo import device_binding
