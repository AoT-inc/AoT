...
from aot.databases.models import GeoShape, Input, Output, OutputChannel, PID, Trigger, Conditional, CustomController, Function
        ...
        try:
            ...
            if target_type:
                if target_type == 'equipment':
                    # [Optimization] Fetch both Individual (Legacy) and Collection (Bundle) types
            ...
            for s in shapes:
                ...
                # Only overwrite aot_type if missing (Preserve subtype for mixed buckets like infra_blob)
                if 'aot_type' not in feat['properties']:
