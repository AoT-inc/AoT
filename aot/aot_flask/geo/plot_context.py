...
# ---------------------------------------------------------------------------
# 기하·치수·소속·센서 — plot_geometry.py 로 뗐다. 83개 함수 중 이 묶음(기하
# 헬퍼·치수·소속·센서)만 밖으로 나가는 참조가 0인 순수 기반층이었다 — 나머지는
# 서로 순환 의존이 있어 떼지 않았다. 여기서 다시 가져와 재노출한다 — 소비자
# 35개 파일이 `from ...plot_context import <이름>` 형태로도 가져오므로 이름이
# 여기 그대로 있어야 한다.
# ---------------------------------------------------------------------------
from aot.aot_flask.geo.plot_geometry import (
    _CAPACITY_UNITS, _FLAT_LAYOUT_ASK, _POLY_TYPES, _SHAPE_WARN_NOTE,
    _SHAPE_WARN_RATIO, _mean_lat, _only_output_ids, _only_sensor_ids,
    _shapely, _to_local_m, allocation_view, area_m2, bay_capacities,
    capacity_estimate, dimensions, facility_brief, facility_geometry,
    facility_sensor_ids, geometry_of, local_frame, shapely_area_m2,
    zone_for_plot,
)
...
# ---------------------------------------------------------------------------
# GDD(적산온도)·DLI(일적산광량) — plot_accumulation.py 로 뗐다. 밖으로 나가는
# 참조가 `plot_sources`·`aot.utils.*` 뿐이라 안전했다. 재노출.
# ---------------------------------------------------------------------------
from aot.aot_flask.geo.plot_accumulation import (
    _DLI_LIGHT_MEASURES, _EXTREMES_CACHE, _EXTREMES_MAX_DAYS,
    _EXTREMES_MAX_KEYS, _EXTREMES_TODAY, _EXTREMES_TODAY_TTL,
    _EXTREMES_TTL, _GDD_MIN_COVERAGE, _GDD_TEMP_MEASURE, _daily_extremes,
    _light_channel_sum, _local_day_of, _query_daily_extremes,
    _today_extremes, dli_accumulated, gdd_accumulated,
)
