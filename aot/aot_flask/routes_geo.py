...
# ---------------------------------------------------------------------------
# [New] GIS Search Routes
# ---------------------------------------------------------------------------

@blueprint.route('/api/geo/search', methods=['POST'])
@login_required
def api_geo_search():
    """
    ...
    try:
        ...
        if not layer_id:
            # [Fix] Default provider if none specified (e.g. initial setup)
            # Try to get from global settings or default to OpenStreetMap
        ...
        # 1. Resolve Input Module Instance
        # Try finding Active Layer first
        ...
        if layer:
            ...
            # Try finding by Input Type Name (e.g. 'gis_vworld')
            dict_inputs = parse_input_information()
        ...
        # We need to check if the module requires a key and if it's in global settings
        ...
        # Get Module Info for Key Field mapping
        dict_inputs = parse_input_information()
        ...
        if 'key_field' in layer_def:
            ...
            # If not in custom_opts, try global
        ...
        # Even here, returning 500 might be scary, but it IS an exception. 
        # Let's keep 500 for actual crashes, but the above case was CHECKED error.
        return jsonify({'ok': False, 'message': str(e)}), 500
...
# ---------------------------------------------------------------------------
# [New] Device Location & Mapping API
# ---------------------------------------------------------------------------

@blueprint.route('/api/geo/device/location', methods=['POST'])
@login_required
def api_geo_device_location_save():
    """
    Saves device location directly to SQL columns (latitude, longitude).
    Also performs basic Spatial Join (Point-In-Polygon) to link device to Zone/Site.
    
    Payload: { unique_id, type, lat, lng, map_config_id, map_uuid }
    """
    try:
        current_app.logger.info(f"[GeoAPI] /device/location hit! Data: {request.get_json()}")
        data = request.get_json()
        unique_id_raw = data.get('unique_id')
        dev_type = data.get('type')
        lat = data.get('lat')
        lng = data.get('lng')
        # map_config_id is usually the map this device belongs to. 
        # But 'map_uuid' from frontend is the ACTIVE map we are designing on.
        map_uuid = data.get('map_uuid') 
        
        if not unique_id_raw or not dev_type:
            return jsonify({'ok': False, 'message': 'Missing unique_id or type'}), 400

        # [Fix] Handle Composite IDs (uuid::channel) from channel-based input list
        ...
        model_map = {
            ...
            'function': CustomController,  # "function" type maps to CustomController usually
            'custom': CustomController,
            'generic_function': Function   # Explicit "Function" model
        }
        
        # Determine model class
        ModelClass = model_map.get(dev_type.lower())
        if not ModelClass:
            # Fallback for 'function' ambiguity
            if dev_type.lower() == 'function':
                 # Try finding in CustomController first, else Function?
                 # Actually, usually 'function' in frontend = CustomController in this system.
                 # But if we support distinct 'Generic Function', we need precise type.
                 # For now, let's assume 'function' maps to CustomController as per existing pattern.
                 # If user adds 'generic_function', we handle it.
                 ModelClass = CustomController
            else:
                 return jsonify({'ok': False, 'message': f'Unknown device type: {dev_type}'}), 400

        # [Fix] Handle Aggregated 'function' type (PID, Trigger, Conditional, CustomController all map to 'function' in utils_geo)
        model_candidates = []
        ...
        for M in model_candidates:
             ...
             if target_device: 
                 # Correct the type for subsequent logic if needed (optional)
                 break
             
        if not target_device:
             current_app.logger.warning(f"[GeoAPI] Device not found for ID: {unique_id} Type: {dev_type}")
             return jsonify({'ok': False, 'message': 'Device not found'}), 404

        # Update SQL columns directly
        # [Fix] Only update global device coordinates for Channel 0 (Primary)
        ...
        # [New] Spatial Join & Map Association Logic
        if map_uuid:
            # 1. Update map_config_id (Link device to this Map Design)
            if hasattr(target_device, 'map_config_id'):
                target_device.map_config_id = map_uuid
            
            # 2. Find Containing Shape (Site/Zone)
            # Use GeoOverlayManager's helper
            from aot.aot_flask.geo.geo_overlays import GeoOverlayManager
            # [Fix] Correct Argument Order: lat, lng, map_uuid
            ...
            if hasattr(target_device, 'map_overlay_id'):
                if containing_shape:
                    ...
                    # [Fix] Safe Name Access from Feature Properties
                    shape_name = 'Unnamed'
            ...
            # [New] Update GeoShape overlay for this map design
            ...
            if lat is not None and lng is not None:
                ...
                geo_shape.feature = {
                    ...
                    "properties": {
                        ...
                        "name": target_device.name # Initial name fallback
                    }
                }
                geo_shape.updated_at = datetime.utcnow()
            elif geo_shape:
                # [Fix] Explicitly remove GeoShape overlay if device is removed from map
