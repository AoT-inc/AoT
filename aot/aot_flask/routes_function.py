...
def page_function_submit():
    ...
    if not messages["error"]:
        if form_add_function.function_add.data:
            ...
            if not tab_id:
                # 화면(`page_function`)은 스코프로 걸러진 탭을 보여준다. 제출의 기본값도
                # 같은 기준이어야 한다 — 전역 position 0 을 쓰면 사용자가 보고 있던
                # 탭이 아닌 남의 탭이 대상이 되어, 조작할 수 없다며 거부된다.
                current_tab = (TabService.default_visible_tab('function') or
                               TabService.get_default_tab('function'))
...
def page_function():
    ...
    tabs = TabService.visible_tabs_for_page('function')
    ...
    if tab_id:
        ...
        if not current_tab or current_tab not in tabs:
            current_tab = TabService.default_visible_tab('function')
    else:
        current_tab = TabService.default_visible_tab('function')
