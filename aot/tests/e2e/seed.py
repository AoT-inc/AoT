...
def _purge():
    ...
    # 것뿐 아니라, 여정이 만들었다 남긴 빈 대시보드와 — `/dashboard-add` 가 GET
    # 으로 열리기만 해도 하나씩 만들던 때(L0·L1 이 그 주소를 매번 열어 실행마다
    # 탭이 둘씩 늘었다) 쌓인 것까지 거두기 위해서다. 그 주소는 이제 POST 전용이라
    # 새로 쌓이지는 않지만, 이미 쌓인 스택을 처음 상태로 돌리는 일은 여전히 이
    # 정리의 몫이다. 이름으로는 못 고른다 — 자동 이름은 화면 언어를 따라간다
    # ("대시보드 7" · "Dashboard 7").
