...
#
#   무엇                          에이전트    MCP     도구
#   직전                           31,800   51,950  115 / 127
#   일지 MCP 3종 정비               32,093   52,698  116 / 128
#
# 늘어난 것은 도구 **하나**(`create_plot_journal`)와 기존 일지 도구 둘의 설명이다.
#
#  (1) 생성 도구를 열었다. 그전에는 읽기 둘뿐이라 **AI 가 남이 만든 일지를 읽을
#      수만 있고, 사용자가 만들어 달라고 하면 화면으로 안내하는 수밖에 없었다** —
#      기능의 주 동작이 도구 표면에서 통째로 빠져 있었다.
#  (2) `get_plot_journal` 의 설명이 길어졌다. 짧게 두었더니 6일짜리 일지도
#      측정값이 0개로 나갔다 — 응답 캡이 가장 큰 필드(`env`)를 떨어뜨리는데
#      호출자가 그것을 모르니 좁힐 생각을 못 한다. "기간이 사흘을 넘으면
#      granularity 를 주라" 는 한 줄이 그 실패를 막는다.
#  (3) `list_plot_journals` 가 대상 없이도 돌게 되면서 설명이 그만큼 늘었다.
#
# 문구가 부풀어서가 아니라 도구가 늘고, 늘어난 설명이 각각 **실측된 실패를
# 막는 것**이라 자릿수를 맞추자고 지울 것이 아니다(2026-08-20 재기준선과 같은
# 판단). 여유는 예전과 같이 좁게 둔다.
...
#   무엇                          에이전트   도구
#   직전                           32,150    116
#   목표 되먹임 두 줄               32,188    116
#
# **도구는 늘지 않았다.** 기존 도구 둘의 설명에 한 줄씩 붙었고, 둘 다 "그 자리에
# 없으면 모델이 존재를 모르는" 종류다(`get_plot` 독스트링의 §guidance 가 적어 둔
# 그 실패 — payload 에 있는 것만으로는 LLM 이 쓰지 않는다).
#
#  (1) `get_plot` — `target_check` 가 최근 며칠을 함께 본다는 한 줄. 이것이
#      없으면 응답에 실린 `recent` 를 모델이 읽을 이유를 모르고, "이 목표가 이
#      현장에서 계속 안 맞는다" 는 발견이 그대로 묻힌다(실측: 야간온도가 잰 날
#      전부 목표 초과인데 답은 늘 "지금 19.5도" 였다).
#  (2) `save_plot_schedule_as_program` — `adopt_targets`. 단계 일수는 실측으로
#      갱신되는데 목표값만 갱신할 길이 없던 되먹임 고리의 나머지 반쪽이라,
#      파라미터가 표면에 없으면 그 고리가 여전히 닫히지 않는다.
#
# 여유는 예전과 같이 좁게 둔다.
AGENT_MANIFEST_TOKEN_CEILING = 32_200
