...
from aot.config import FUNCTIONS
from aot.databases.models import (PID, Actions, Conditional,
                                     CustomController, Dashboard, Function,
                                     Input, Output, Trigger, User, Widget)
from aot.aot_flask.utils.utils_general import (choices_custom_functions,
                                                     generate_form_input_list,
                                                     generate_form_output_list,
                                                     generate_form_widget_list)
from aot.tests.software_tests.conftest import login_user
from aot.tests.software_tests.factories import UserFactory
...
@mock.patch('aot.aot_flask.routes_authentication.login_log')
...
def test_api_docs_when_not_logged_in(testapp):
    ...
    assert 'AoT API' in response, "Unexpected HTTP Response: \n{body}".format(body=response.body)
...
def create_user(aot_db, role_id, name, password):
