...
3. 사용자가 직접 부르는 경로는 이 게이트를 타지 않는다.
   모델을 막 등록한 사람이 스위치를 켜기 전에 시험해 볼 수 있어야 한다.
   (여기서는 `generate_system_summary` 가 여전히 `ai_enabled` 만 보는 것으로
   확인한다 — 에이전트가 없으면 ERROR 가 아니라 조용한 skip 이어야 한다.)
