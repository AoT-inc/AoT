...
class TestUnknownTagDoesNotHideEverything(unittest.TestCase):
    """없는 태그로 거르면 **전부가 걸러진다** (2026-08-25 실사용 실패).

    태그는 운영자·AI 가 자유롭게 붙이는 값이라 정해진 어휘가 없는데, 모델은
    그것을 모르고 그럴듯한 말을 지어 넣는다. 실측: "가을 무는 어떻게 키워야
    되지?" 에 모델이 tags='crop' 을 붙여 불렀고, 이 라이브러리의 실제 태그는
    '무,가을무,김장무,재배' 라 하나도 안 맞아 **답을 담은 바로 그 항목이 필터에
    잘려 나갔다.** 서버가 접지로 같은 내용을 이미 넣어 줬는데도 모델은 자기 빈
    검색 결과를 믿고 "정보가 없습니다" 로 끝냈다.

    조용히 빈손을 주는 대신 필터를 버리고 무엇이 있는지 알려 준다 — 모델이
    스스로 고칠 수 있어야 한다.
    """
    ...
    def setUp(self):
        ...
        s = AIGlobalSettings()
        db.session.add(s)
        src = AIContextSource(facility_id='global', source_name='S', source_type='ai_curated',
                              parameter_name='p', config_json='{}', sync_interval_min=0,
                              is_active=True, is_enabled=False)
        db.session.add(src)
        db.session.commit()
        db.session.add(AIKnowledgeChunk(
            source_id=src.source_id, source_name='S', section_title='가을무 재배 기준',
            digest_text='가을무는 파종 후 약 80일에 수확한다.', raw_excerpt='x',
            content_hash=hashlib.sha256(b'x').hexdigest(), provenance='ai_curated',
            context_state='system_generated', is_enabled=True, tags='무,가을무,재배'))
        db.session.commit()
