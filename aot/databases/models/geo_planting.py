...
class GeoPlanting(CRUDMixin, db.Model):
    ...
    **동시 유효한 구획끼리 기하가 겹쳐도 막지 않는다** — 유니크 인덱스나
    ...
    # ── 배치 ──────────────────────────────────────────────────────────
    # 두둑 폭 / 고랑 폭 (cm). 둘 다 NULL 이면 "모른다" 이지 "평평하다" 가 아니다
    # — 그 구분이 사라지면 조회가 균일 배치 숫자를 아무 말 없이 내밀게 된다
    # (고랑에는 아무것도 안 심으므로 20~30% 과다추정).
    #
    # **여기 있는 이유**: 두둑 규격은 밭의 성질이지 질문의 성질이 아니다. 조회
    # 파라미터로만 두면 사람이 대화마다 다시 말해야 하고, 말하지 않은 대화에서는
    # 조용히 틀린 숫자가 나온다. 작물·기간과 달리 이 값은 갈아엎기 전까지 그대로다.
    #
    # 기하(feature)와 달리 **종료된 작기에서도 수정할 수 있다** — 나중에 규격을
    # 알게 되어 채워 넣는 것은 이력을 고치는 것이 아니라 비어 있던 칸을 메우는
    # 것이다. VP-6 이 막는 것은 "그 자리에 뭐가 있었나" 를 바꾸는 일이다.
    bed_width_cm = db.Column(db.Integer, nullable=True)
    # 0 은 유효한 값이다(두둑을 붙여 만드는 배치). NULL 과 다르다.
    path_width_cm = db.Column(db.Integer, nullable=True)
