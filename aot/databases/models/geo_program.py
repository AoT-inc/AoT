...
class GeoProgram(CRUDMixin, db.Model):
    ...
    **지금은 이 표가 정본이다** — 제어가 매 사이클 여기를 읽고(코디네이터에는
    ...
    # ── 자동 승인은 여기 없다 (P7 → P8, p6_56 에서 구획으로 이전) ──────
    #
    # `auto_advance` 가 잠깐 여기 있었다. 틀린 축이었다 — 자동 승인이 묻는 것은
    # "이 작물의 단계 모델이 정확한가" 가 아니라 **"이 자리를 사람 눈 없이 믿을
    # 수 있는가"** 이고, 그것은 작물이 아니라 구획의 성질이다. 같은 토마토
    # 프로그램을 쓰는 두 구획이 서로 다른 답을 갖는 것이 정상인데, 토글이 여기
    # 있으면 그 둘을 나누려고 작물 지식을 한 벌 더 복제하게 된다(§P6 가 피하려던
    # 바로 그 자리다).
    #
    # ⚠ 되살리지 말 것. 양쪽에 두고 OR 로 합치면 "왜 넘어갔나" 의 답이 두 곳이
    #   된다. 정본은 `GeoPlot.auto_advance` 하나다.
