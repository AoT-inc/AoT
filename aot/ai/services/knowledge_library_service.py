...
def summary():
    ...
    for prov, in db.session.query(AIKnowledgeChunk.provenance).filter(
            AIKnowledgeChunk.is_enabled.is_(True)).all():
        # 화면에 그대로 나가는 값이므로 **여기서** 사람이 읽는 이름으로 바꾼다.
        # 항목 목록은 `provenance_label` 을 쓰는데 요약줄만 원시 키를 내보내고
        # 있었다(실측: "ai_curated: 2 · user_provided: 1"). 같은 화면에서 같은
        # 것을 두 이름으로 부르면 안 된다.
        label = _PROVENANCE_LABELS.get(prov, prov)
        out['by_provenance'][label] = out['by_provenance'].get(label, 0) + 1
        out['total'] += 1
    return out
