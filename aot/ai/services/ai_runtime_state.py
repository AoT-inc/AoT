...
사용자가 직접 보내는 채팅/조언/명령 요청은 `ai_autonomy_enabled()` (1 AND 2) 를
거친다 — 2단계(AI 페이지의 "작동 시작")가 "LLM 모델이 설정되고 작동 중"인
상태를 뜻하므로, 이게 꺼져 있으면 채팅도 동작하지 않는다. (예전에는 1 만 보고
동작했으나, 2단계의 의미가 "LLM 모델 작동 여부"로 확정되면서 채팅도 여기 묶는
쪽으로 바뀌었다.)

외부 LLM 클라이언트(Claude Desktop 등)가 쓰는 MCP 서버(`aot_mcp_server.py`)는
이 판정과 완전히 별개다 — 그쪽은 `mcp_http_enabled` 로만 게이팅되고, AI 서비스
활성화 여부와 무관하게 항상 켜질 수 있다.
