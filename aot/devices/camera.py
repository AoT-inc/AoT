...
def camera_record(record_type, unique_id, duration_sec=None, tmp_filename=None):
    ...
    if settings.library == 'picamera':
        ...
        try:
            ...
            from urllib.request import urlretrieve, HTTPPasswordMgrWithDefaultRealm, HTTPBasicAuthHandler, build_opener, install_opener

            # Inject auth credentials into URL if configured
            _url_still = settings.url_still
            _username = getattr(settings, 'auth_username', '') or ''
            _password = getattr(settings, 'auth_password', '') or ''
            if _username:
                _parsed = urlparse(_url_still)
                _netloc = f"{_username}:{_password}@{_parsed.hostname}"
                if _parsed.port:
                    _netloc += f":{_parsed.port}"
                _url_still = urlunparse(_parsed._replace(netloc=_netloc))

            if record_type in ['photo', 'timelapse']:
                path_tmp = "/tmp/tmpimg.jpg"

                # Get filename and extension, if available
                a = urlparse(_url_still)
                ...
                try:
                    urlretrieve(_url_still, path_tmp)
        ...
        try:
            ...
            _username = getattr(settings, 'auth_username', '') or ''
            _password = getattr(settings, 'auth_password', '') or ''
            _auth = (_username, _password) if _username else None
            ...
            if record_type in ['photo', 'timelapse']:
                ...
                try:
                    r = requests.get(settings.url_still, headers=headers, auth=_auth, verify=False)
    ...
    elif settings.library in ('opencv-python', 'opencv-python-headless', 'ip_camera'):
        # New-style library name: RTSP/HTTP streaming URL camera
        try:
            import cv2
            from urllib.parse import urlparse, urlunparse

            stream_url = getattr(settings, 'url_stream', '') or getattr(settings, 'url_still', '')
            if not stream_url:
                logger.error("opencv-python: No stream URL configured for camera %s", unique_id)
            else:
                # Inject credentials into URL for RTSP/HTTP streams
                _username = getattr(settings, 'auth_username', '') or ''
                _password = getattr(settings, 'auth_password', '') or ''
                if _username and '://' in stream_url:
                    _parsed = urlparse(stream_url)
                    if not _parsed.username:
                        _netloc = f"{_username}:{_password}@{_parsed.hostname}"
                        if _parsed.port:
                            _netloc += f":{_parsed.port}"
                        stream_url = urlunparse(_parsed._replace(netloc=_netloc))
                ...
                if record_type in ['photo', 'timelapse']:
                    ...
                    if cap.isOpened():
                        ...
                        if ret and frame is not None:
                            ...
                            logger.error("opencv-python: Failed to read frame from stream")
                    else:
                        logger.error("opencv-python: Could not open stream")
        except Exception:
            logger.exception("opencv-python capture")

    elif settings.library == 'python-onvif-zeep':
        # ONVIF IP camera: RTSP snapshot via OpenCV with auth injection
        try:
            import cv2
            from urllib.parse import urlparse, urlunparse
            ...
            stream_url = getattr(settings, 'url_stream', '') or getattr(settings, 'url_still', '')
            if not stream_url:
                logger.error("python-onvif-zeep: No stream URL configured for camera %s", unique_id)
            else:
                _username = getattr(settings, 'auth_username', '') or ''
                _password = getattr(settings, 'auth_password', '') or ''
                if _username and '://' in stream_url:
                ...
                if record_type in ['photo', 'timelapse']:
                    ...
                    if cap.isOpened():
                        ...
                        if ret and frame is not None:
                            ...
                            logger.error("python-onvif-zeep: Failed to read frame from stream")
                    else:
                        logger.error("python-onvif-zeep: Could not open stream")
        except Exception:
            logger.exception("python-onvif-zeep capture")
