...
MEASUREMENTS = {
    ...
    'power_reactive': {
        ...
        'units': ['var']},
    'precipitation': {
        'name': lazy_gettext('Precipitation'),
        'meas': 'precipitation',
    ...
    'specific_enthalpy': {
        ...
        'units': ['kJ_kg']},
    'snowfall': {
        'name': lazy_gettext('Fresh Snowfall'),
        'meas': 'snowfall',
        'units': ['cm']},
    'snr': {
    ...
    'sky_condition': {
        'name': lazy_gettext('Sky Condition'),
        'meas': 'sky_condition',
        'units': ['none', 'percent']},    
    'temperature': {
...
UNIT_CONVERSIONS = [
    ...
    # ── W_m2 <-> umol_m2_s ────────────────────────────────────────────────
    # **`W_m2` 는 전천일사(일사계, 약 300~2800 nm)로 정의한다.**
    #
    # 예전 계수는 4.57 이었는데 그것은 **PAR 대역(400~700 nm) W/m²** 을
    # μmol·m⁻²·s⁻¹ 로 바꾸는 값이다. 일사계나 기상 API 의 전천일사에 그대로
    # 곱하면 전천일사 중 PAR 이 에너지 기준 45~50% 뿐이라 PPFD 가 약 2.26 배로
    # 나온다 — 맑은 날 정오 800 W/m² 가 3,656 μmol 이 되는데 지표면에서
    # 물리적으로 불가능한 값이다(실제 상한이 약 2,000).
    #
    # 4.57 × 0.45 ≈ 2.06, × 0.50 ≈ 2.29. 널리 쓰이는 값 **2.02** 를 택한다
    # (일 적산으로 보면 DLI[mol/m²/일] ≈ 2.02 × 일사[MJ/m²/일] 라는 관례와
    # 같은 계수다).
    #
    # ⚠ **PAR 센서는 `umol_m2_s` 로 등록할 것.** 대부분의 양자센서가 μmol 로
    # 출력하므로 자연스럽고, PAR 대역 W/m² 로 등록하면 여기서 절반으로 줄어든다.
    # 그런 센서를 W/m² 로 쓸 수밖에 없다면 채널 변환(Conversion)으로 ×2.26 을
    # 걸어 보정한다.
    #
    # 이 계수는 **태양광 기준**이다. LED·HPS 보광은 스펙트럼이 달라 완전히
    # 다른 값이 되므로, 보광 아래 PPFD 가 필요하면 PAR 센서를 쓸 것.
    ('W_m2', 'umol_m2_s', 'x * 2.02'),
    ('umol_m2_s', 'W_m2', 'x / 2.02'),

    # J/cm² <-> umol_m2_s (1초 집적 시간 가정: 10000 × 2.02)
    ('J_cm2', 'umol_m2_s', 'x * 20200'),
    ('umol_m2_s', 'J_cm2', 'x / 20200')
